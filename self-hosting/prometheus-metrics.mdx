...
- `providerSlug`: The Model Catalog provider / virtual key that served the request, distinct from `provider` (the upstream provider name) — **opt-in**, disabled by default (add `providerSlug` to `PROMETHEUS_EXTRA_LABELS`)
...
This exports:
- `apiKeyName` — the name of the Portkey API key used (as configured in the Portkey dashboard)
- `configName` — the name of the Portkey gateway config used for the request
