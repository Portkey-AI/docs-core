...
Two additional opt-in labels expose the request's config slug and API key name on every custom metric. Both default to `false` to keep cardinality bounded.

| Environment Variable | Default | Description |
|----------------------|---------|-------------|
| `PROMETHEUS_INCLUDE_CONFIG_NAME_LABEL` | `false` | Adds the `config_name` label populated from the config slug passed via headers. Falls back to `N/A` when an inline config object is sent |
| `PROMETHEUS_INCLUDE_API_KEY_NAME_LABEL` | `false` | Adds the `api_key_name` label populated from the API key name on the request |

```bash
PROMETHEUS_INCLUDE_CONFIG_NAME_LABEL=true
PROMETHEUS_INCLUDE_API_KEY_NAME_LABEL=true
```
