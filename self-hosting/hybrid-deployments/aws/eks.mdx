...
To whitelist, add an inbound rule to the Load Balancer's security group allowing connections from the Portkey Control Plane's IP (`54.81.226.149`, `34.200.113.35`, `44.221.117.129`) on NLB listener port.
...
service:
  type: LoadBalancer
  port: 8787                                                 # Customize this parameter if you require a Load Balancer listener for a custom port.
  annotations: 
    service.beta.kubernetes.io/aws-load-balancer-type: "nlb"
