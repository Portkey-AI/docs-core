...
### Outbund Connectivity (Data Plane to Control Plane)

Portkey supports the following methods for integrating the Data Plane with the Control Plane for outbound connectitvity:
- AWS PrivateLink
- Over the Internet
...
1. Contact Portkey and provide AWS account ARN so it can be whitelisted by the control plane to initiate the AWS PrivateLink connection.
2. Once Portkey confirms that whitelisting is complete, go to the [VPC Console](https://console.aws.amazon.com/vpc/).
...
7. After all details are filled in, click on **Create endpoint**.
8. Wait for the Status to change to `Available`.
9. Once the status changes to `Available`, click on **Actions** > **Modify private DNS name** > Select **Enable for this endpoint**.
10. Update the `values.yaml` file with following config.
    ```yaml
    environment:
        create: true
        secret: true
...
To whitelist, add an inbound rule to the Load Balancer's security group allowing connections from the Portkey Control Plane's IP (`44.221.117.129`) on NLB listner port.
