...
1. (Optional) Create a subnet in region `us-east4`. Skip if you already have a subnet in `us-east4`.
  ```sh
  ...
  gcloud compute networks subnets create psc-endpoint-subnet \
    ...
    --region=us-east4
  ```
2. Create an IP for endpoint to be created.
  ```sh
  # If you already had a subnet in us-east4 and you skipped last step,
  # Replace psc-endpoint-subnet that name if subnet you had.
  gcloud compute addresses create psc-endpoint-ip \
    --region=us-east4 \
    --subnet=psc-endpoint-subnet 
3. Contact Portkey teamm share your gcp PROJECT id and request for private connectivity. 
4. Once your project have been whitelisted and Portkey have shared you SERVICE_ATTACHMENT_URI for PSC connection, create the forwarding rule for Control Plane's private endpoint.
  ```sh
  # Replace <VPC_NAME> with name of vpc in which your GKE cluster is created. 
