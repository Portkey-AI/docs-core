...
2. Proxy service (if enabled)
3. Log store (S3/GCS/Azure Blob/S3-compatible)
4. Local filesystem path
5. Bundled configs in image
...
## Gateway Configuration (Required for Both Options)
...
| Variable | Description |
| -------- | ----------- |
| `MODEL_CONFIGS_PROXY_FETCH_ENABLED` | Enables fetching pricing from Backend |

### Backend

| Variable | Description |
| -------- | ----------- |
| `MODEL_CONFIGS_PROXY_FETCH_ENABLED` | Enables proxy service fetching |
| `MODEL_CONFIGS_PROXY_URL` | Proxy base URL |
| `MODEL_CONFIGS_PRICING_LOG_STORE_PATH` | Log store pricing path |
| `MODEL_CONFIGS_CAPABILITIES_LOG_STORE_PATH` | Log store capabilities path |
| `MODEL_CONFIGS_PRICING_LOCAL_PATH` | Local pricing directory |
| `MODEL_CONFIGS_CAPABILITIES_LOCAL_PATH` | Local capabilities directory |
...
- [Enterprise Architecture](/self-hosting/hybrid-deployments/architecture)
