...
def evaluate_interaction(customer_query, agent_response):
    ...
    response_schema = {
        ...
        "properties": {
            ...
            "improvement_areas": {
                "type": "array",
                "items": {"type": "string"},
                "description": "Specific areas where the response could be improved"
            }
        },
    ...
    result = response.choices[0].json_object
    
    # Log the evaluation result to Portkey for analytics
    portkey.feedback.create(
        trace_id=customer-support-eval-run-1,  # Use the trace ID from the evaluation
        value=1 if result["outcome"] == "good" else 0,  # Convert outcome to numeric rating
        metadata={
...
<Frame>
    <img src="/images/guides/prompts/evaluation-dashboard.png"/> # TODO
</Frame>
...
Once your LLM-as-a-Judge system is operational, you can scale it to evaluate thousands of interactions:
