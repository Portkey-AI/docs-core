...
2. Click **Virtual Keys** and then **Create**
   ...
   2. Hit **Create**
3.  the provider slug from the **KEY** column
...
```json
{
    ...
    "targets": [
        {
            "provider": "@stability-ai-virtualkey",
            "override_params": {"model": "stable-diffusion-v1-6"}
        },
        {
            "provider": "@open-ai-virtual-key",
...
These configs tell the AI gateway to follow an `fallback` strategy, where the primary target to forward requests to is _Stability AI_ (automatically inferred from the provider slug) and then to _OpenAI_. The `override_params` let’s you override the default models for the provider. Finally, surprise surprise! — we also enabled caching with just one more key-value pair.
