...
The gateway caches STS credentials per unique combination of `(roleArn, externalId, region, sourceCredentialHash, tagsHash)`. Different metadata tag sets produce different cache keys, so:

- Two requests with **identical metadata** reuse the same cached credentials (no extra STS call).
- Two requests with **different metadata** trigger separate `AssumeRole` calls and produce distinct STS sessions.

This ensures cost attribution accuracy while minimizing STS API usage.
...
```
User: arn:aws:sts::884994594706:assumed-role/ecsTaskContainerRole/...
is not authorized to perform: sts:TagSession on resource:
arn:aws:iam::642137768492:role/portkey-bedrock-invokemodel
```
