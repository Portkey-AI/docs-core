...
portkey = Portkey(api_key=PORTKEYAI_API_KEY)
```
...
This configuration instructs Portkey to use a \_fallback \_strategy with the requests. The \_targets\_array lists the provider slugs of LLMs in the order Portkey should fallback to an alternative.
...
```json
{
    "strategy": {"mode": "fallback"},
    "targets": [
        {"provider": "openai", "api_key": "sk-xxxxxxxxpRT4xxxx5"},
        {"provider": "azure-openai", "api_key": "*******"}
    ]
}
```
