...
description: "Every Portkey routing strategy — conditional, load balancing, fallback — can be nested inside any other. This guide covers the four most powerful combinations."
---
...
This guide shows the four most useful patterns with real configs.

---

## Route by Model, Distribute by Provider

**Pattern: Conditional → Load Balancer**

Use conditional routing to match a model alias, then route that alias to a load balancer spread across multiple providers. Traffic for `claude-sonnet` distributes evenly across Anthropic, Vertex AI, and Bedrock — each with independent rate limit buckets, effectively tripling throughput.
...
## Per-Branch Fallbacks in a Conditional Router
...
## Fall Back to a Conditional Router
...
## Distribute with a Fallback Safety Net

**Pattern: Fallback → Load Balancer (primary)**

The primary target is an anonymous load balancer across multiple providers. Individual provider failures are handled by the load balancer. Only when **all** providers in the cluster fail does the outer fallback activate. This avoids over-triggering cross-model fallbacks while still guaranteeing zero downtime.
...
<Tip>
To add fallback trigger conditions, set `on_status_codes` on the outer fallback strategy: `"strategy": { "mode": "fallback", "on_status_codes": [429, 500, 502, 503, 504] }`. Without this field, the fallback uses the default set (429, 500, 502, 503, 504). If the cluster returns a 400 or 403, the fallback will not activate regardless.
</Tip>

---

## Per-Leg Fallbacks in a Load Balancer

**Pattern: Load Balancer → Fallback (per leg)**
...
| Pattern | Best for |
|---------|----------|
| **Conditional → Load Balancer** | High-volume model aliases that need rate limit multiplication across providers |
| **Conditional → Fallback (per branch)** | Different model families that each need their own independent recovery sequence |
| **Fallback → Conditional Router** | Smart failover with compliance requirements (e.g., EU data-residency even in failure) |
| **Fallback → Load Balancer** | Protecting a distributed cluster without over-triggering cross-model fallbacks |
| **Load Balancer → Fallback (per-leg)** | Multi-model distribution where each family needs independent failure isolation |
