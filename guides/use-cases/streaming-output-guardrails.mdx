...
function evaluateGuardrailResults(guardrailResults) {
    ...
    for (const guardrail of guardrailResults) {
        if (!guardrail.verdict) {
            const failedChecks = (guardrail.checks || [])
                .filter((check) => !check.verdict)
                .map((check) => check.id);
            console.log(
                `Guardrail '${guardrail.id}' failed. Failed checks: ${failedChecks}`
            );
            return false;
        }
    }
...
```javascript Node
async function streamWithLogging(messages) {
    const { collectedContent, guardrailResults } =
        await streamWithGuardrailsCustom(messages);
    ...
    if (guardrailResults) {
        for (const guardrail of guardrailResults) {
            const logEntry = {
                ...
                execution_time_ms: guardrail.execution_time,
                checks: (guardrail.checks || []).map((c) => ({
                    id: c.id,
                    verdict: c.verdict,
                    execution_time_ms: c.execution_time,
                })),
            };
            // Send to your logging/observability platform
    ...
    return collectedContent; // Always return the content
}
```
</CodeGroup>
...
```javascript Node
async function robustStreamHandler(messages) {
    ...
    for (const model of models) {
            ...
            if (guardrailResults) {
                for (const g of guardrailResults) {
                    console.log(
                        ...
                        `guardrail=${g.id} verdict=${g.verdict}`
                    );
                }
            }

            if (evaluateGuardrailResults(guardrailResults)) {
                return { content, model, attempts: attempt + 1 };
            }
        ...
        console.log(`Model ${model} exhausted retries, falling back...`);
    }
