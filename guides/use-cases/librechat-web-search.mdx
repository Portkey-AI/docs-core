...
Transform your LibreChat experience by adding web search capabilities to any LLM - whether it's GPT-4, Claude, Llama, or any of the 1,600+ models supported by Portkey. This guide shows you how to combine LibreChat, Portkey, and Exa to create a powerful AI chat interface with real-time internet access.
...
The config uses the `@provider-slug/model` format. Add your OpenAI credentials in [Model Catalog](https://app.portkey.ai/model-catalog) first.
...
```json Config
{
    ...
    "retry": {"attempts": 3},
    "override_params": {"model": "@openai-prod/gpt-4o", "temperature": 0.7}
}
```
