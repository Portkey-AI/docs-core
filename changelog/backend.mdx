...
### Organisation Guardrails — Workspace Exclusions
- Individual workspaces can now be exempted from organisation-level default guardrails via `PUT`/`GET /v1/workspace-exclusions/input-guardrails` and `/v1/workspace-exclusions/output-guardrails`. Updates are bulk by default and accept `override_existing` to replace the current exclusion list instead of merging
- API key details now return the workspace's guardrail exclusions so the Gateway can skip enforcement
- [Documentation](/product/administration/enforce-orgnization-level-guardrails#excluding-workspaces)

<Note>Workspace exclusions are currently available via the API only. Dashboard support is coming shortly.</Note>

### Organisation Defaults API
- New `GET`/`PUT /v1/admin/organisation/defaults` to read and set the organisation's default `input_guardrails` and `output_guardrails`. `GET` returns each guardrail's `id` and `slug`
- Update semantics now match the workspace defaults pattern, and the response follows the Admin API contract

### New API Scopes
- `organisation_settings.read` / `organisation_settings.update` — organisation settings, defaults, workspace overrides, and properties
- `organisation_guardrails.create` / `.read` / `.list` / `.update` / `.delete`
- `organisation_exclusions.list` / `organisation_exclusions.update`
- [API Key Scopes Documentation](/product/enterprise-offering/org-management/api-keys-authn-and-authz#admin-api-key-permission-scopes)

### SSO & Authentication
- OIDC settings accept **`token_endpoint_auth_method`** and a **`jwks`** key set, adding `private_key_jwt` support. Self-hosted single-org deployments configure the same through `OIDC_TOKEN_ENDPOINT_AUTH_METHOD` and `OIDC_CLIENT_JWKS`
- [SSO Documentation](/product/enterprise-offering/org-management/sso)

### Model Catalog
- `GET` provider details accepts `include_configurations=true` (system auth only); AWS, Azure Entra, and Oracle secrets are masked in the response

### Security
- Audit log masking extended to camelCase field names

### Self-Hosting
- New **`DB_POOL_SIZE`** environment variable controls the MySQL connection pool size (default `12`, previously fixed at `6`)

### Fixes and Improvements
- **Workspaces**: Removed a redundant user loop during workspace creation
- **Deployments**: Default gateway deployments can now be deleted

</Update>

<Update label="1.21.0" description="2026-07-27">

## v1.21.0
---

### Log Export Field Restrictions
- New **`export_settings.deniedFields`** setting blocks sensitive fields (for example `request`, `response`, `metadata`) from being exported. Set it at the organisation level, or per workspace once the `export_settings` workspace override is enabled
- Export create and update requests are rejected when they ask for a denied field
- New `GET /v1/logs/exports/field-restrictions?workspace_id=...` returns the effective restrictions for a workspace
- [Logs Export Documentation](/product/observability/logs-export#restricting-exportable-fields)

### Model Catalog
- **AWS Bedrock** integrations support a new `apiKey` auth type — supply a Bedrock API key and `aws_region` instead of access keys or a role
- Custom models accept a per-model `custom_headers` string-to-string map on `PUT /v1/integrations/:id/integration-models`
- Refreshed model and pricing configs across Bedrock, OpenAI, Vertex AI, Azure AI, Azure OpenAI, Anthropic, Claude Platform (AWS), Google, xAI, Workers AI, Fireworks, DeepInfra, OpenRouter, Together, Moonshot, Z.ai, and Zhipu
- [AWS Bedrock Documentation](/integrations/llms/bedrock/aws-bedrock)

### Security
- SCIM passwords are masked in audit logs
- Dependency updates to patch security vulnerabilities

### Fixes and Improvements
- **SCIM**: Fixed user reactivation on update
- **Users**: `GET` user details now returns all organisation workspaces for org owners and admins
- **OAuth**: Token introspection responses now include `display_name`
- **Workspaces**: Fixed duplicate workspace-user slug checks
- **MCP**: Fixed tool annotation sync
- **Custom Hosts**: Relaxed validation to allow Kubernetes-internal TLDs on integration models
- **Observability**: Fixed SigNoz OTel tracing metrics

</Update>

<Update label="1.20.0" description="2026-07-13">

## v1.20.0
---

### Guardrails Updates
- **PANW Prisma AIRS**: `profile_name` is no longer required — when omitted, the profile linked to your API key in Strata Cloud Manager is used. Added `profile_id`, `app_name`, `scan_scope`, and `strip_scaffolding` parameters
- [Prisma AIRS Documentation](/integrations/guardrails/palo-alto-panw-prisma)

### New Provider — Scenario AI
- **Scenario AI** added to the Model Catalog (`scenario-ai`) for AI-powered image, video, and asset generation
- [LLM Integrations](/integrations/llms)

### SSO & Authentication
- New **`disable_auto_provision_user`** auth setting restricts SSO login to invited users whose email belongs to a verified allowed domain
- Login accepts an optional `sso_client_id` so users linked to multiple SSO configurations can target a specific one
- [SSO Documentation](/product/enterprise-offering/org-management/sso)

### Usage Limit Policies
- Raising a policy's `credit_limit` now reactivates already-exhausted keys instead of leaving them blocked until the next reset window
- [Budget & Usage Policies Documentation](/product/enterprise-offering/budget-policies)

### Security
- OAuth authorization-code exchange is atomic, enforcing single use

### Fixes and Improvements
- **Guardrails**: Fixed validation of default guardrails when updating organisation and workspace defaults
- **Workspaces**: Deleting a workspace is now blocked when it still has shared prompts or partials; fixed workspace lookup by slug
- **Prompts**: Prompt version listing is now cached, with invalidation on update, delete, and default-version change
- **Integrations**: Fixed SageMaker validation for null fields
