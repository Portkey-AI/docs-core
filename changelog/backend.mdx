...
### Guardrails Updates
- Updated **Lasso Security** guardrail with a new **Classifier** check (`lasso.classify`) powered by Lasso's Deputies v3 API
  - Supports `messages` array input with role-based content
  - Optional `conversationId` (ULID) to group related calls into a dialogue session
  - Optional `userId` for tracking individual usage
  - Returns detailed findings with action types (BLOCK, WARN, AUTO_MASKING) and severity levels
  - Runs on both `beforeRequestHook` and `afterRequestHook`
- Updated Lasso integration schema to support custom `apiEndpoint` for self-hosted deployments
...
### DB Migrations
<Note>
Helm chart update required — run the following migrations when upgrading:
- `2026-04-09-lasso-update.sql`
- `2026-04-08-api-keys-rotate-scope.sql`
- `2026-03-26-ws-integrations-security-settings.sql` (Knex migration)
</Note>
