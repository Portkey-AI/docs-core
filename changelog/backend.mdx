...
### Security
- **Mandatory domain verification** for SSO login, with additional SAML/OIDC hardening and rate-limiting on the domain verification flow
- New **`TRUSTED_CUSTOM_HOSTS`** env var — comma-separated allowlist of hosts that bypass private/reserved SSRF checks (cloud-metadata/IMDS endpoints remain blocked)
- DNS-rebinding protection and hardened redirect handling on outbound fetches
- Safe Regex validation for user-supplied regexes
- Workspace names sanitised on response
- Updated dependencies to patch security vulnerabilities

### Fixes and Improvements
- **AWS**: Fixed EKS pod-identity credential cache JSON read; added the missing `AWS_CONTAINER_AUTHORIZATION_TOKEN_FILE` parameter
- **API Keys**: Key details now return the masked key
