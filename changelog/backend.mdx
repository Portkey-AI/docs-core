...
### API Key Rotation — Org-Enforced Period
...
### API Key List & Updates
- `GET /v1/api-keys` now accepts a `status` query parameter (`active`, `exhausted`, `expired`, comma-separated) to filter keys by lifecycle state — defaults to all three
- API key updates on **expired** keys are now rejected (rotate or recreate the key instead)

### Configurable API Key Prefix
...
- Key validation accepts both unprefixed (28 chars) and prefixed (`<prefix>` + 28 chars) formats
...
- Domain SSO lookups are now in-memory cached for 60s
...
- **Audit Noise Reduction**: SCIM `GET` calls are no longer recorded in audit logs
...
- **Private-Link Endpoint Resolution**: Deployments with `use_private_link_proxy` now correctly resolve `gateway_base_url` ↔ `private_link_endpoint` and forward `x-portkey-private-link-endpoint` headers
- **Multi-Org Resync**: New `POST /organisations/resync-multi` admin route (auth via `RESYNC_SERVICE_AUTH_HEADER` / `RESYNC_SERVICE_AUTH_TOKEN`) for bulk subscription/policy resync
- **Cosign-Signed Images**: Release Docker images are now signed with [Sigstore Cosign](https://docs.sigstore.dev/cosign/overview)
- **MCP Tokens Storage**: `mcp_server_user_connections.tokens` column upgraded to `MEDIUMTEXT` to support larger OAuth payloads
...
- Hardened SSO encryption settings handling on the SSO callback path
- Hardened `reqHydrator` middleware to prevent unhandled exceptions from invalid log exports / api-keys / SCIM payloads
- Migrated all outbound `fetch` calls to `externalServiceFetch` with abort-controller timeouts
- Security dependency updates and vulnerability fixes (`package-lock.json`)

### DB Migrations
- `mcp_integrations.secret_mappings` (JSON column)
- `integration_models.description` (text column)
- `integration_models.configurations` and `integrations.pricing_adjustments` (JSON columns)
- `usage_limits_policies.periodic_reset_days`, `next_usage_reset_at`, `last_reset_at`
- `rotation_policies.rotation_period_days`
- `organisation_email_domains.allow_subdomains`
- `mcp_server_user_connections.tokens` → `MEDIUMTEXT`
- New tables: `account_deletions`, `account_deletion_logs`, `agent_*` (agent gateway tables)

### Fixes and Improvements
- Fixed user API key removal when users are removed from org / workspace (previously left orphan keys)
...
- Fixed expired API-key TTL update flow
- Fixed workspace usage-limits validation when invalid values were passed
- Fixed feature-flag guard middleware and Stripe subscription data hardening
- Fixed `last_updated_by` and missing fields on guardrails list API response
...
- Fixed `save user sso settings` flow
