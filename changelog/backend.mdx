...
### New Providers
- **Claude Platform (AWS)** — New `claude-platform-aws` provider in Model Catalog for Claude full platform capabilities via AWS (`anthropic_aws_workspace_id` + AWS access key, assumed role, or service role auth)
...
- Fixed workspace update when identified by slug; SAML org callback ignores organisation ID in private deployments
- Fixed organisation deletion ordering for finetune cleanup
- Gateway deployment limit counts only **active** deployments
...
- Sanitized health-check ping responses; Signoz trace/log push fixes
- Model and pricing config sync (Anthropic, Bedrock, OpenRouter, Vertex AI, and others)
...
### SSO / Authentication
- **PKCE**: OIDC and SAML flows now support dynamic `code_challenge_method` (`S256` / `plain`) with encrypted relay state; SAML ACS URL validation uses the hardened custom-host checks
...
### Fixes and Improvements
- Fixed usage-limits policy exhaustion marking on hosted deployments (enterprise cache path) and added a safe-check for exhausted status under KV cache unreliability
- Fixed usage-limits policy reset to update Durable Object counters on hosted (non–private-self-cache) deployments
- Removed deprecated v1 `/datasets` and `/finetune` routes (v2 APIs remain)
...
### DB Migrations
- Seeds `organisationAdminsViewLogs` and `organisationAdminsViewLogMetadata` (default `true`) on existing organisations and workspaces for admin log visibility settings
...
### Fixes and Improvements
- Fixed ClickHouse analytics time-series queries to use `LIMIT` instead of `TOP` (ClickHouse-compatible syntax)
...
- **Credential Masking**: Sensitive credential fields (`apiKey`, `clientSecret`, `AIRS_API_KEY`) inside guardrail check `credentials` are now masked on `GET /guardrails` and `PUT /guardrails` responses for non-system users
