...
- **Guardrails Now GA**: Production-ready guardrails to enforce LLM behavior in real-time, with support for PII detection, moderation, and more — are now generally available. [(*Docs*)](/product/guardrails)

- **Enterprise Momentum**:
  - Refreshed Portkey's [enterprise offering](https://portkey.ai/docs/product/enterprise-offering) with enhanced security features.
...
- **Platform Enhancements**:
  ...
  - Enhanced cost tracking and monitoring capabilities.
...
- Also updated the API specs for Prompt Completions [API](/api-reference/inference-api/prompts/prompt-completion), Prompt Render [API](/api-reference/inference-api/prompts/prompt-render), and Insert Log [API](/api-reference/admin-api/data-plane/logs/insert-a-log)
...
- **Caching**: Improved cost calculations for OpenAI & Azure OpenAI cached responses
- **Gemini Updates**:
  - Added support for Gemini JSON mode
  - Support for Controlled Generations for Gemini models, along with with Pydantic support
...
#### New Providers
6 new providers added to Portkey last month, expanding your model hosting and deployment options:
<CardGroup cols={3}>
