...
[Self-Hosting Architecture](/self-hosting/hybrid-deployments/architecture)

### Fixes and Improvements
- **Batches**: Validate the endpoint before submitting batch jobs. [Batches Documentation](/product/ai-gateway/batches)
- **Security**: Added SSRF checks for external calls.
    [Custom Hosts Documentation](/product/ai-gateway/custom-hosts)
- **Prometheus**: Skip Prometheus client initialization when env values are not set. [Prometheus Metrics](/self-hosting/prometheus-metrics)
