...
sidebarTitle: "Enterprise Gateway [1.16.5]"
rss: true
---
...
- **VertexAI**: Added support for [https://docs.cloud.google.com/vertex-ai/generative-ai/docs/computer-use](Computer Use).
    ...
    - environment (can either be "ENVIRONMENT_BROWSER" and "ENVIRONMENT_DESKTOP")
...
- Please use the below environment variables to enable this feature:
    - `EXPERIMENTAL_GEN_AI_OTEL_PUSH_ENABLED`: Set to `true` to enable the feature
    - `EXPERIMENTAL_GEN_AI_OTEL_EXPORTER_OTLP_ENDPOINT`: The endpoint to push the traces to
    - `EXPERIMENTAL_GEN_AI_OTEL_EXPORTER_OTLP_HEADERS`: The headers to push with the traces
...
- [Documentation Link](/product/administration/enforce-workspace-budget-limits-and-rate-limits)
