...
sidebarTitle: "Enterprise Gateway [2.25.0]"
rss: true
---
...
<Update label="2.25.0" description="2026-09-24">

## v2.25.0

---

### Decisions API: TypeSafe (Jev) Provider

The gateway now exposes a `/v1/decisions` endpoint for typed-judgment providers, starting with TypeSafe's Jev models. Callers send questions over structured `state` and get back typed judgments (boolean, choice, or score) with probabilities and calibrated confidence, rather than a chat completion. Requests and responses pass through natively, while still getting gateway auth, routing, retries, circuit breaking, caching, logging, and cost tracking. Before-request guardrails can inspect the `state` field on these requests, the same way they do for `embed`.

[Providers Documentation](/integrations/llms)

### Gateway-Local JWT Auth: Workspace Hints & Membership Enforcement

Gateway-local JWT authentication now accepts an `x-portkey-workspace` header (slug or id) to select a workspace at request time. The hint is honored only for requests attributed to a Portkey user via the token's email claim, is ignored for service-key requests, and never overrides a workspace already baked into the token. Whenever a request resolves to a specific workspace — from a token claim, the header hint, or deployment settings — the gateway now also enforces that the user is an active member of it, rejecting the request with a 403 otherwise.

[Gateway-Local JWT Authentication](/product/enterprise-offering/org-management/jwt#workspace-resolution)

### New `startHook` Stage and Header Guardrails

Configs gain a `startHooks` list that runs before every other hook, auth, and routing decision. Two new guardrail checks target it: **Header Check** validates that request headers do or don't contain specific key-value pairs (`all`/`any`/`none` operators), and **Header Transform** removes, sets, or conditionally rewrites request headers before routing — protected and `x-portkey-*` headers can't be touched. The existing `allowedRequestTypes` and `webhook` checks, and JWT header lookup, are now also `startHook`-compatible.

[Guardrails Documentation](/product/guardrails)

### Fixes and Improvements

- **MCP Gateway**: A blocked agent session-init attempt against a cold or expired external OAuth connection is now recorded in the authorization logs with the agent's identity, instead of failing silently. [External MCP Servers](/product/mcp-gateway/external-mcp-servers)
- **MCP Gateway**: Connecting to a cold external MCP upstream no longer revokes the agent's own access token; that purge is now reserved for genuine mid-session credential failures. [External MCP Servers](/product/mcp-gateway/external-mcp-servers)
- **Guardrails**: The PANW Prisma AIRS guardrail now retries transient scan failures (timeouts, 429s, 5xx) with exponential backoff instead of immediately failing the check. [PANW Prisma AIRS Guardrail](/integrations/guardrails/palo-alto-panw-prisma)
- **Pricing**: Voyage, Lightning AI, and Scenario AI pricing cards are now registered for local resolution, so private/on-prem deployments bill these providers correctly instead of silently defaulting to $0. [Model Catalog](/product/model-catalog)
- **Azure**: Config-based conditional routing now reads request parameters from multipart/form-data image edit requests, instead of only from JSON bodies. [Conditional Routing](/product/ai-gateway/conditional-routing)
- **Administration**: Internal routing headers used by the dashboard's logs view and playground are no longer misidentified as inline provider configuration when **Block Inline Configs** is enabled. [Enforcing Saved-Only Resources](/product/administration/enforce-saved-only-config)
- **Security**: Fixed a path traversal vulnerability in the Stability AI provider that let a caller redirect the gateway's outgoing request — carrying the operator's API key — to arbitrary paths on Stability AI's API. [Stability AI Integration](/integrations/llms/stability-ai)
- **Security**: The Prompt Render API now restricts request overrides to the prompt's declared parameters, closing a mass-assignment and prototype-pollution gap where a caller could inject arbitrary fields or reassign the merged object's prototype. [Prompt Render API](/api-reference/inference-api/prompts/render)

</Update>

<Update label="2.24.0" description="2026-09-22">
...
The gateway now logs every intermediate retry attempt, not just the final one, sharing the same trace ID and request context as the primary log. A retry chain like `500 → 404 → 429` previously logged only the last status, hiding the earlier failures that drove a fallback decision; each attempt is now its own log row distinguished by `retry_count`, `response_status`, `response_headers`, and `response_body`.

[Automatic Retries](/product/ai-gateway/automatic-retries) · [Logs](/product/observability/logs)

### Truncated Stream Detection Across Providers

Truncated SSE streams are now detected across all providers on the unified text routes (`/chat/completions`, `/completions`, `/messages`, `/responses`), instead of only on Anthropic. Terminal-event detection now covers `[DONE]`, the Messages API and Responses API event lines (`message_stop`, `response.completed`), and `finish_reason` / `finishReason` chunks for providers like Gemini and Vertex Llama that never send `[DONE]`.

When a stream ends without its terminal event, the gateway appends an in-band `stream_incomplete` error event so the client can tell a cut-off response from a complete one, retains the partial content in the log, and excludes the response from the cache.


### Provider Updates

- **Anthropic**: The `safeguards` field on Claude Code auto-mode requests is now forwarded to Anthropic instead of being dropped, so server-side classifier checks run without incurring extra classifier billing. `safeguard_results` is now also forwarded and logged on the streaming Messages path. [Claude Code Documentation](/integrations/libraries/claude-code)

[Providers Documentation](/integrations/llms)

### Fixes and Improvements

- **MCP Gateway**: A rejected OAuth request to an external MCP server now returns the identity provider's reason instead of a 500, and a provider that is briefly unreachable no longer invalidates the stored connection and forces users back through consent. [Connecting External MCP Servers](/product/mcp-gateway/external-mcp-servers)
- **MCP Gateway**: A new OAuth client is now registered only when the gateway can confirm that none exists for the server, so a transient outage no longer leaves duplicate client registrations in the identity provider.
- **MCP Gateway**: A configuration change now reliably clears a server's cached tokens, and an OAuth refresh now retires the token it replaces, so a rotated-out token stops working immediately.
- **MCP Gateway**: Disabled tools, resources, and prompts now stay disabled when the control plane is briefly unreachable, instead of becoming callable again for up to 5 minutes. [Tool Provisioning](/product/mcp-gateway/tool-provisioning)
- **Pricing**: OpenRouter Decisions API responses (e.g. TypeSafe Jev models) now attribute token usage correctly, instead of always logging zero units and zero cost. [OpenRouter Integration](/integrations/llms/openrouter)
- **Security**: Hardened upstream path and URL construction for SiliconFlow image requests and Cohere batch operations. [SiliconFlow Integration](/integrations/llms/siliconflow) · [Cohere Integration](/integrations/llms/cohere)
- **Security**: Hardened credential validation for the Pangea PII and Text Guard guardrails. [Pangea Guardrail](/integrations/guardrails/pangea)

</Update>

<Update label="2.23.0" description="2026-09-18">

## v2.23.0

---

### ElevenLabs Unified Speech and Transcription Endpoints

ElevenLabs is now natively supported: `/v1/audio/speech` and `/v1/audio/transcriptions` route to ElevenLabs' text-to-speech and speech-to-text APIs directly, with no custom host configuration needed. Voice ids are forwarded as-is rather than remapped from OpenAI's built-in voice names.

[ElevenLabs Documentation](/integrations/llms/elevenlabs)

### Provider Updates

- **OpenAI**: Streaming chat completions now default to `stream_options.include_usage: true`, so usage is reported on the stream unless a custom host is configured. [OpenAI Integration](/integrations/llms/openai)
- **OpenAI**: Pricing and Anthropic-compatible mapping now recognize the `fast` service tier introduced on newer models. [OpenAI Integration](/integrations/llms/openai)
- **Vertex AI**: Rerank requests now honor the configured `vertexRegion` instead of always routing to the global Discovery Engine endpoint. [Vertex AI Rerank](/integrations/llms/vertex-ai/rerank)

[Providers Documentation](/integrations/llms)

### Fixes and Improvements

- **Pricing**: Bedrock requests made through `/v1/responses` now pick up cache-read and cache-write tokens from the Responses usage shape, so cached usage is costed at the correct rate instead of being missed. [Bedrock Integration](/integrations/llms/bedrock/aws-bedrock)
- **Pricing**: Azure AI cache-write tokens are now read from `prompt_tokens_details` when available, so cache-write pricing is applied correctly. [Azure AI Foundry Documentation](/integrations/llms/azure-foundry)
- **Pricing**: `qwen3-asr-flash` on DashScope is now billed by transcribed audio duration instead of falling back to text token counts. [DashScope Integration](/integrations/llms/dashscope)
- **Pricing**: Bedrock SDXL requests over 50 steps are now billed at the premium rate, which previously never applied. [Bedrock Integration](/integrations/llms/bedrock/aws-bedrock)
- **MCP Gateway**: A rejected OAuth `authorize` request now returns the underlying error to the client instead of a 500. [MCP OAuth](/product/mcp-gateway/authentication/oauth)
- **Self-Hosting**: Prometheus metrics support an opt-in `providerSlug` label identifying the Model Catalog provider that served a request. [Prometheus Metrics](/self-hosting/prometheus-metrics)
- **Self-Hosting**: Azure Storage and Key Vault Private Link domains are now trusted by default, so Private Link deployments no longer need to add them to `TRUSTED_CUSTOM_HOSTS` manually. [Custom Hosts](/product/ai-gateway/custom-hosts)
...
[Claude Code Documentation](/integrations/libraries/claude-code)
...
[Azure OpenAI Documentation](/integrations/llms/azure-openai/azure-openai) · [Azure AI Foundry Documentation](/integrations/llms/azure-foundry)

### Provider Updates

- **Anthropic, Bedrock Mantle & Claude Platform on AWS**: `anthropic-beta` and `anthropic-version` are now picked up from the incoming request headers when they aren't set on the provider or in the config, so client-supplied beta features reach the provider. Anthropic requests also no longer carry an automatic `anthropic-beta: messages-2023-12-15` header. [Anthropic Integration](/integrations/llms/anthropic)
- **Claude Platform on AWS**: Fixed a streaming regression where SSE events were concatenated into a single unterminated blob, causing strict SSE clients like Claude Code to fall back to non-streaming. [Claude Platform on AWS Integration](/integrations/llms/claude-platform-aws)
- **Workers AI**: The `thinking` parameter now applies to GLM, QwQ, and Kimi K2.5 models, which expect `enable_thinking` rather than `thinking`. Previously it was silently ignored on these models. [Workers AI Integration](/integrations/llms/workers-ai)
- **Together AI**: The `thinking` parameter is now supported and translated to Together AI's native `reasoning` field, so you can turn reasoning on or off on models like DeepSeek-V4-Flash using the same syntax as every other provider. [Together AI Integration](/integrations/llms/together-ai)
- **Azure PII Guardrail**: A new **PII Categories** setting lets you limit detection to specific entity types, so values like organization names or dates can be left untouched. Leaving it empty continues to detect all default categories. [Azure PII Detection Settings](/integrations/guardrails/azure-guardrails)
- **Alibaba Content Safety Guardrail**: Added the mainland China service types `query_security_check` and `response_security_check`, alongside the existing international ones. [List of Guardrail Checks](/product/guardrails/list-of-guardrail-checks)

[Providers Documentation](/integrations/llms)

### Fixes and Improvements

- **Pricing**: Pricing adjustments can now be set per model on an integration, not just for the integration as a whole. A model-level adjustment fully replaces the integration-level one rather than merging with it. [Pricing Adjustments](/product/model-catalog/pricing-adjustments)
- **Pricing**: Added bundled pricing data for ElevenLabs, so text-to-speech and speech-to-text costs are tracked without a pricing sync. [Model Catalog](/product/model-catalog)
...
- **Guardrails**: Checks that are turned off inside a redaction or transformation guardrail — such as PII redaction — are now correctly skipped. Previously they still ran. Guardrail configs referencing an unknown check ID are also now ignored with a warning instead of being passed through. [Guardrails](/product/guardrails)
- **MCP Gateway**: Fixed a 500 error when loading the OAuth consent screen at `GET /oauth/authorize`. Applies to Prisma AIRS AI Gateway deployments, where the consent screen is served as HTML. [MCP OAuth](/product/mcp-gateway/authentication/oauth)
- **MCP Gateway**: A repeated OAuth callback no longer shows an `invalid_state` error when the connection has already succeeded. [MCP External OAuth](/product/mcp-gateway/authentication/external-oauth)
- **MCP Gateway**: OAuth token refresh failures now return the correct `invalid_grant` error code and 400 status per RFC 6749, instead of a generic `unauthorized` 401. [MCP OAuth](/product/mcp-gateway/authentication/oauth)
...
When a token's email resolves to an existing Portkey user in the target workspace, the request is now attributed to that user and inherits their workspace role, instead of being treated as a generic workspace service key.
...
[MCP Gateway Guardrails Documentation](/product/mcp-gateway/guardrails)
...
[xAI Documentation](/integrations/llms/x-ai)
...
[Self-Hosting Documentation](/self-hosting/hybrid-deployments/aws/eks)
...
[OpenAI Integration Documentation](/integrations/llms/openai)
...
[OpenRouter Documentation](/integrations/llms/openrouter)
...
[MCP Gateway Documentation](/product/mcp-gateway/mcp-registry)
...
[Beta Features Documentation](/product/ai-gateway/beta-features)
...
[MCP Observability Documentation](/product/mcp-gateway/observability)
...
[Rate Limits Documentation](/product/model-catalog/rate-limits)
...
Portkey-registered MCP server tools can now be included in the `tools` array of Responses API and Messages API requests using the `@portkey-mcp` prefix. The gateway fetches tool definitions from the MCP server, injects them into the LLM request, and executes tool calls server-side — so providers that don't natively support remote MCP (e.g., AWS Bedrock, Vertex AI) can still use MCP tools. Gated behind the `x-portkey-beta: server-side-mcp-2026-06-01` header.
...
Gen AI and guardrail OTel spans now carry Portkey tenant attributes — org ID, workspace, and environment — so spans can be filtered, grouped, and alerted on per-tenant in your collector without post-processing.
...
[Observability Documentation](/product/observability)
...
[Vertex AI Documentation](/integrations/llms/vertex-ai)
...
### Portkey Models Endpoint Override
...
[Amazon Bedrock Documentation](/integrations/llms/bedrock/aws-bedrock)
...
[Custom Hosts Documentation](/product/ai-gateway/custom-hosts#trusted-custom-hosts-allowlist)
</Warning>
...
[Claude Platform (AWS) Documentation](/integrations/llms/claude-platform-aws)
...
[Cato Networks Documentation](/integrations/guardrails/cato)
...
[Pricing Adjustments Documentation](/product/model-catalog/pricing-adjustments)
...
[Prometheus Metrics Documentation](/self-hosting/prometheus-metrics)
...
[CrowdStrike AIDR Documentation](/integrations/guardrails/crowdstrike-aidr)
...
[Configs Documentation](/product/ai-gateway/configs#default-and-drop-params)
...
[Vertex AI Batches Documentation](/integrations/llms/vertex-ai/batches)
...
[Realtime API Documentation](/product/ai-gateway/realtime-api)

### MCP Gateway

- **Capability enforcement on `tools/call`**: `tools/call`, `prompts/get`, and `resources/read` repopulate the disabled-capabilities set from the control plane on cold cache instead of silently allowing requests through
...
- **LLM routes**: Workload identity coverage is extended for Azure OpenAI and Azure AI Foundry, including Portkey-specific configuration paths for federated, keyless access.
...
Administrators can attach a **new usage limit policy type** for finer-grained control over consumption limits (aligned with control-plane policy models).
...
Anthropic can return `overloaded_error` as the first chunk of a streaming (`200 OK`) response, which previously slipped past retry/fallback logic. When enabled on an Anthropic integration, the gateway now inspects the first streaming chunk and — if it's an `overloaded_error` — converts the response to HTTP **529**, which is a [retriable status code](/product/ai-gateway/automatic-retries), so your configured retries and fallbacks trigger as expected.
...
Vertex AI is now supported on Portkey's unified `/v1/rerank` endpoint, backed by Google's Discovery Engine ranking API (`semantic-ranker-default`, `semantic-ranker-fast`, `semantic-ranker-512`).

[Vertex AI Rerank Documentation](/integrations/llms/vertex-ai/rerank)

### Amazon Bedrock Mantle (OpenAI-Compatible Bedrock Endpoint)

New `bedrock-mantle` provider for AWS's OpenAI-compatible Bedrock inference engine (`bedrock-mantle.{region}.api.aws`). Supports `/v1/chat/completions`, `/v1/responses`, and Anthropic-native `/v1/messages` via Portkey's unified API, with `apiKey`, `assumedRole`, and `serviceRole` auth modes.

[Amazon Bedrock Mantle Documentation](/integrations/llms/bedrock-mantle)
...
[Anthropic `service_tier` → `speed` mapping](/integrations/llms/anthropic#service_tier--anthropic-speed-mapping)
...
The [Required Metadata Key-Value Pairs](/product/guardrails/list-of-guardrail-checks#access-control-management) guardrail now accepts a `matchType` parameter to control how values are compared:
...
[Semantic Cache Documentation](/product/ai-gateway/cache-simple-and-semantic#set-up-semantic-caching-self-hosted)
...
Published Docker images (release and prerelease) are now signed with [Sigstore Cosign](https://docs.sigstore.dev/cosign/overview) and can be verified with Portkey's public key before deployment.
...
[MCP Forwarding Headers Documentation](/product/mcp-gateway/authentication/forwarding-headers)
...
[Azure Authentication Modes Documentation](/integrations/llms/azure-openai/authentication)
...
[Vertex AI Embeddings Documentation](/integrations/llms/vertex-ai/embeddings)
...
- **Passthrough configs**: Clearer errors when a [passthrough target](/product/ai-gateway/configs#passthrough-targets) can't resolve a provider from the incoming request
...
Run asynchronous Claude jobs through Portkey's unified `/v1/batches` endpoints.

[Anthropic Batches Documentation](/integrations/llms/anthropic/batches)
...
[Sensitive Headers Documentation](/api-reference/inference-api/headers#mask-sensitive-headers-in-logs)
...
[Bedrock `reasoning_effort` Documentation](/integrations/llms/bedrock/aws-bedrock#using-reasoning-effort-parameter)

### Agent Gateway (Preview)

New experimental proxy route `/v1/agent/:agentServerId/*` forwards requests to A2A-registry agent servers with Portkey auth and observability. Reach out to preview.
...
Portkey [metadata](/product/observability/metadata) is now automatically mapped to Vertex AI resource labels across `chat/completions`, `embeddings`, `batches`, and `fine-tuning` — unlocking Google Cloud–native cost attribution. Keys are sanitized and prefixed (default `pk_gateway_`, configurable via `METADATA_MAP_KEY_PREFIX`).

[Vertex AI Custom Metadata Labels](/integrations/llms/vertex-ai#custom-metadata-labels)

### Custom Host per Custom Model

Custom models in the [Model Catalog](/product/model-catalog/custom-models) can now carry their own `custom_host` in the model config, letting different custom models under the same integration route to different upstreams. A header-level `custom_host` still takes priority.
...
- **`cache_control` passthrough**: `/chat/completions` now forwards a top-level `cache_control` parameter to Anthropic, matching `/messages` behavior. For block-level caching, continue using [prompt caching](/integrations/llms/anthropic/prompt-caching)
...
### Provider Updates
- **VertexAI**: Added support for Computer Use. See [Documentation](/integrations/llms/vertex-ai#computer-use-browser-automation-preview).
...
- [Documentation](/product/observability/opentelemetry#experimental-features)
...
### Provider Updates
- **Google and VertexAI**: Updated the Grounding implementation to support their new API signatures. [Docs Link](/integrations/llms/vertex-ai#grounding-with-google-search)
...
- [Docs Link](/product/ai-gateway/batches#batches)
...
- [Docs Link](/integrations/llms/vertex-ai#grounding-with-google-search)
...
- [Docs Link](/integrations/llms/aws-sagemaker)
</Update>
...
- [Docs Link](/product/prompt-engineering-studio/prompt-versioning#prompt-labels)
...
- [Docs Link](/product/ai-gateway/realtime-api)
