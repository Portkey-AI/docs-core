...
### Config Targets: Adapter Re-application

Follow-up to the `default_params` / `override_params` / `drop_params` pipeline in v2.9.0. The full params pipeline (`default_params` → `override_params` → `drop_params`) is now re-applied after Messages and Responses adapter transforms, so adapter-injected fields like `stream_options` are reachable from `drop_params`.
...
- Basic-auth path for Portkey-client authenticated MCP admin routes

[MCP Gateway Documentation](/product/mcp-gateway)

### Logs

Token usage object is now stored in logs in both metrics-only and full logging modes.

[Logs Documentation](/product/observability/logs)
...
- **Auth**: Hardened auth validations across request paths
- **AWS**: Undefined-safe checks for `aws_external_id` and `clientId` on AWS auth flows
- **Azure Redis**: Fixed token refresh on Azure managed-identity Redis connections
- **DNS**: Safe DNS lookup applied per-request via agent bindings to prevent SSRF from cached or stale resolutions
- **Redirects**: Manual redirect handling so upstream 3xx responses don't bypass gateway processing
- **Model parsing**: Reads `model` from `@slug/model` form in the request body, matching header behavior
- **Removed legacy Rubeus mode**: Old request shape that accepted config in the body (deprecated December 2023) has been removed
