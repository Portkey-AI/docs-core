...
sidebarTitle: "Enterprise Gateway [2.23.0]"
rss: true
---
...
ElevenLabs is now a first-class provider: `/v1/audio/speech` and `/v1/audio/transcriptions` route to ElevenLabs' text-to-speech and speech-to-text APIs directly, with no custom host configuration needed. Voice ids are forwarded as-is rather than remapped from OpenAI's built-in voice names.

[ElevenLabs Documentation](/integrations/llms/elevenlabs)

### MCP Agent Attribution

MCP access logs now identify the agent behind a delegated tool call, not just the human it acts for. Authenticated agent tokens add `caller.agent.id`, `caller.agent.name`, `caller.agent.team`, `caller.agent.owner_id`, and `caller.agent.type` alongside the existing `caller.user_id`, so agent traffic can be filtered and attributed separately from a person working by hand.
...
- **Pricing**: Bedrock now correctly bills cached and cache-write tokens for requests made through the `/v1/responses` adapter, which previously resolved to zero. [Bedrock Integration](/integrations/llms/bedrock/aws-bedrock)
- **Pricing**: Fixed Azure AI cache-write token pricing so it's read from `prompt_tokens_details` when available. [Azure AI Foundry Documentation](/integrations/llms/azure-foundry)
...
- **Pricing**: Fixed a typo (`step` vs `steps`) that prevented Bedrock SDXL requests over 50 steps from ever resolving to premium pricing. [Bedrock Integration](/integrations/llms/bedrock/aws-bedrock)
- **MCP Gateway**: A rejected control-plane OAuth `authorize` request now relays the real error to the client instead of surfacing a 500. [MCP OAuth](/product/mcp-gateway/authentication/oauth)
