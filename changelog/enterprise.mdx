...
### Anthropic: Remove `context_management` Pass-Through

The `context_management` parameter added in v2.11.0 has been removed from the Anthropic base transform. Requests that include this field will no longer forward it upstream.
...
- **SSRF**: Internal service calls (batches, etc.) in air-gapped deployments now skip SSRF validation, fixing failures when the gateway routes to its own internal endpoints
- **Security**: Custom-host header is now blocked for the `portkey` provider to prevent host override on first-party requests
