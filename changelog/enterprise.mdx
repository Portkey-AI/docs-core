...
sidebarTitle: "Enterprise Gateway [2.24.0]"
rss: true
---
...
- **Security**: Model names in SiliconFlow image requests are now validated before they are used to build the upstream path. [SiliconFlow Integration](/integrations/llms/siliconflow)
- **Security**: Batch cancellation requests to Cohere now derive the batch id from the URL path alone, so a query string on the incoming request can no longer reach the upstream call. [Cohere Integration](/integrations/llms/cohere)
- **Security**: The Pangea PII and Text Guard guardrails now validate the configured domain before making outbound requests. [Pangea Guardrail](/integrations/guardrails/pangea)
