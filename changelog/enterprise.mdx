...
New Deepgram provider integration for speech-to-text (STT) and text-to-speech (TTS), with full observability and reliability features.

[Deepgram Documentation](/integrations/llms/deepgram)

### OAuth Client Credentials for Upstream Auth

OpenAI-compatible integrations can now authenticate to the upstream provider using the OAuth Client Credentials grant (RFC 6749 §4.4). The gateway acquires and caches the bearer token (Redis with 5-minute TTL, local in-memory with 30-second TTL) from your configured token endpoint and forwards it as `Authorization: Bearer` upstream — no static API key required.

[OpenAI Integration Documentation](/integrations/llms/openai)

### Vertex AI OCR

The `/v1/ocr` endpoint now supports Mistral OCR models served on Google Vertex AI via `rawPredict`, with pricing tracked per page processed.
...
The Qwen provider now supports the Responses and Messages endpoints, along with additional non-OpenAI parameters and `chat_complete_kwargs` forwarding via Dashscope.
...
Billing now prefers the provider-reported cost (`usage.cost`) when available, falling back to the static rate table. OpenRouter is the first provider to benefit, eliminating per-routing-decision cost drift. BYOK requests also capture `upstream_inference_cost` to avoid logging $0.

[OpenRouter Documentation](/integrations/llms/openrouter)

### MCP Registry Proxy

The MCP Gateway now proxies MCP Registry requests, forwarding `/v0.1/*` paths to the registry backend and exposing server listings directly through the gateway.

[MCP Registry Documentation](/product/mcp-gateway/mcp-registry)

### Meshy & Tripo3D Cost Tracking

Credit-based cost tracking is now available for the Meshy and Tripo3D 3D-generation providers, with a 24-hour duplicate-billing protection window for repeated task polls.
...
- **Security**: `x-amz-security-token` masked in Winky logs, prototype-chain guardrail check IDs blocked, `oauth_client_secret` redacted from logs, and dependencies updated to patch audit vulnerabilities
