...
### Azure OpenAI & Azure AI Inference Context-Tier and Service-Tier Pricing

Azure OpenAI and Azure AI Inference requests are now billed at the pricing tier matching their input token count (short vs. long context) and service tier (batch, priority, or standard), instead of always billing at the flat base rate.

[Azure OpenAI Documentation](/integrations/llms/azure-openai/azure-openai)

### Provider Updates

- **Anthropic**: Reasoning and cached token usage are now correctly propagated in streaming Responses API usage details, instead of always reporting zero
- **Claude Platform on AWS**: Fixed a streaming regression where SSE events were concatenated into a single unterminated blob, causing strict SSE clients like Claude Code to fall back to non-streaming
- **Anthropic**: `anthropic-beta` headers are now correctly applied for targets-based configs, not just root-level configs
- **Workers AI**: Thinking is now disabled correctly for GLM, QwQ, and Kimi K2.5 models, which expect `enable_thinking` instead of `thinking` in `chat_template_kwargs`
- **Together AI**: The `thinking` param is now forwarded to disable reasoning on supported models like DeepSeek-V4-Flash
- **Azure PII Guardrail**: The `piiCategories` setting is now exposed in the guardrail configuration form, so redaction can be restricted to specific entity categories instead of redacting everything. [Docs](/product/guardrails/pii-redaction)
- **Alibaba Content Safety Guardrail**: Added two new security type options for the China region, alongside the existing international region types. [Docs](/product/guardrails/list-of-guardrail-checks)

[Providers Documentation](/integrations/llms)

### Fixes and Improvements

- **Pricing**: Model-level pricing adjustments now take full precedence over provider-level adjustments, replacing rather than merging with them. [Docs](/product/model-catalog/pricing-adjustments)
- **MCP Gateway**: Fixed a 500 error on `/oauth/authorize` for MCP servers that serve an inline HTML consent page. [Docs](/product/mcp-gateway/authentication/oauth)
- **MCP Gateway**: External OAuth callbacks are now idempotent — a duplicate callback replays the original redirect instead of showing an `invalid_state` error. [Docs](/product/mcp-gateway/authentication/external-oauth)
...
- **Guardrails**: Added defence-in-depth checks for guardrail mutator checks. [Docs](/product/guardrails)
