...
sidebarTitle: "Enterprise Gateway [1.17.7]"
rss: true
---
...
### New Guardrail
- **Blocked Tools**: New guardrail to control AI tool usage in requests. Block or allow specific tool types (`function`, `web_search`, `file_search`, `code_interpreter`, `computer_use`, `mcp`) and function names using blocklists or allowlists.
    - [Documentation](/integrations/guardrails/blocked-tools)

### Self-Hosting Improvements
- **Configurable Semantic Cache Embedding Dimensions**: Added `SEMANTIC_CACHE_EMBEDDING_DIMENSIONS` environment variable to configure embedding dimensions for semantic cache (default: 1536)
- **Redis Cluster Discovery URL**: Added support for Redis cluster discovery URL with automatic topology discovery via ioredis

### Provider Updates
- **Anthropic**: Enabled anthropic-beta header on all routes for the Anthropic provider, allowing access to beta features across all endpoints

### Fixes and Improvements
- Fixed Gemini response handling to correctly concatenate multiple text parts
- Fixed budget exhausted checks to exclude batch and files GET endpoints
- Improved rate limiter with fixed window implementation for more consistent rate limiting
- Fixed JWT API key validation logic
- Fixed API key ID handling for rate limits
