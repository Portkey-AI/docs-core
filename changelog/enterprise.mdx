...
sidebarTitle: "Enterprise Gateway [2.6.1]"
rss: true
---
...
### Vertex AI: Multi-Layer Custom Pricing Resolution

Custom pricing for Vertex AI now resolves hierarchically through **region → execution mode (standard / batch / priority) → context tier**, replacing the previous flat `pricing_config` lookup. Provider-specific resolvers and a cached `ModelPricingEntry` blob ensure the most specific matching price is used — with bundled defaults applied as a fallback.

This improves cost attribution accuracy for Vertex workloads running across regions, batches, and long-context tiers.
