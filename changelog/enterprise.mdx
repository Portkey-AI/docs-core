...
Truncated SSE streams are now detected across all providers on the unified text routes (`/chat/completions`, `/completions`, `/messages`, `/responses`), instead of only on Anthropic. Terminal-event detection now covers `[DONE]`, Anthropic and Responses event lines, and `finish_reason` / `finishReason` chunks for providers like Gemini and Vertex Llama that never send `[DONE]`.
...
- **MCP Gateway**: A new OAuth client is now registered only when the gateway can confirm that none exists for the server, so a transient outage no longer leaves duplicate client registrations in the identity provider. [External OAuth](/product/mcp-gateway/authentication/external-oauth)
- **MCP Gateway**: A configuration change now reliably clears a server's cached tokens, and an OAuth refresh now retires the token it replaces, so a rotated-out token stops working immediately. [External OAuth](/product/mcp-gateway/authentication/external-oauth)
...
- **Security**: Fixed a path traversal vulnerability in SiliconFlow image request handling. [SiliconFlow Integration](/integrations/llms/siliconflow)
...
- **Security**: Fixed a query parameter injection vulnerability in the Cohere batch cancellation endpoint. [Cohere Integration](/integrations/llms/cohere)
