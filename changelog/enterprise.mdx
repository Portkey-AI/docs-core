...
### Bedrock India Geo Routing & Context-Tier Pricing

Amazon Bedrock requests routed through the India-geo cross-region inference profile now resolve correctly, and `custom_pricing.context_tier_map` is now used to price requests by context length (short vs. long context) at cost-calculation time.

[AWS Bedrock Documentation](/integrations/llms/bedrock/aws-bedrock)

### Unified Gateway + MCP Server Mode

A new `--unified` server flag runs the AI Gateway and MCP Gateway on a single port, routing MCP and OAuth paths internally instead of requiring two separate ports/services.
...
- **Palo Alto Networks Prisma AIRS**: Scan endpoint URL is now configurable via `AIRS_URL`, and model/user/app metadata is resolved from the live request instead of static plugin params
- **Azure AI Foundry**: Fixed cache-token cost attribution for OpenAI-Responses-style usage payloads
- **Azure OpenAI**: Codex models are now routed through the messages-to-responses adapter
...
- **Anthropic**: SSE `ping` events are now forwarded as keepalives on OpenAI-shaped stream paths, preventing long-running streams from being dropped
- **Anthropic**: Fixed `cache_control` being dropped for single-content-part messages sent through the Responses adapter
...
- **Security**: Fixed `custom_host` leaking from one target to other targets in fallback/load-balance configs
- **Security**: The MCP JWT validation cache key now includes the full validation config and uses SHA-256 hashing, closing a stale-auth window after config changes
- **MCP**: Fixed OAuth login for Enterprise Hybrid deployments using a Cloud Authentication Service (CAS) identity provider
- **MCP**: The JWT introspection endpoint now honors the configured content type
- **MCP**: Service-type API keys using local JWT auth skip the unnecessary per-user access check
- **Usage Limits**: Fixed request-count-based limits and JWT-key usage limits not being enforced
- **F5 Guardrails**: Added an opt-in `forwardMetadata` parameter to forward request metadata to the scan request

[MCP Gateway Documentation](/product/mcp-gateway/authentication/oauth)
