...
### Log Store as Control Plane for Hybrid Deployments

Hybrid deployments can now use the log store as a control-plane source, reducing runtime dependency on the Portkey Control Plane for customers running S3-compatible object storage in their own VPC.

[Hybrid Deployment Architecture](/self-hosting/hybrid-deployments/architecture)
