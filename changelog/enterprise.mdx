...
The gateway now detects when a provider's SSE stream ends mid-response without its terminal event, across all providers instead of only Anthropic. A truncated stream now returns an error to the client rather than a silent 200, is logged with its partial content retained, and is excluded from the response cache.
...
- **MCP Gateway**: An upstream OAuth error from an external MCP server now surfaces its underlying reason instead of an opaque 500, and a client is no longer registered with the external identity provider when the control plane can't be reached to check whether one already exists — preventing orphaned registrations during an outage. [Connecting External MCP Servers](/product/mcp-gateway/external-mcp-servers)
- **MCP Gateway**: Fixed a cache key mismatch that caused revoking a server's cached tokens on a config change to silently remove nothing, and a renewed OAuth token now evicts the access token it replaced instead of remaining usable for up to 5 minutes after rotation. [External OAuth](/product/mcp-gateway/authentication/external-oauth)
- **MCP Gateway**: A failed control-plane lookup for a server's disabled tool capabilities no longer caches an empty result, which had let a disabled tool stay callable for up to 5 minutes. [Tool Provisioning](/product/mcp-gateway/tool-provisioning)
...
- **Security**: The Pangea PII and Text Guard guardrails now validate the configured domain before making outbound requests, preventing requests from being redirected to an attacker-controlled host. [Pangea Guardrail](/integrations/guardrails/pangea)
