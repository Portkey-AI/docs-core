...
The MCP Gateway now proxies MCP Registry requests, serving your approved server catalog through a standards-compliant `GET /v0.1/servers` endpoint that governance-aware MCP clients can consume directly.

[Registry API Documentation](/product/mcp-gateway/registry-api)

### Meshy & Tripo3D Cost Tracking

Credit-based cost tracking is now available for the Meshy and Tripo3D 3D-generation providers, with duplicate-billing protection for repeated task polls.
...
- **Amazon Bedrock**: Assumed-role requests can now propagate `x-portkey-metadata` as AWS STS session tags (opt-in via env var), enabling per-application cost and audit attribution on a shared IAM role
...
- **Azure AI Foundry**: Fixed resource resolution and additional OpenCode compatibility fixes
- **Vertex AI**: Corrected metadata prefix handling in the Cloudflare Workers environment
...
- **MCP**: Issuer URL now follows RFC 8414, `authorization_params` are supported for upstream OAuth (e.g. Google refresh tokens), `GET`/`DELETE` requests return `405` since the gateway is stateless, and rejected client credentials now return the correct `4xx` status instead of `502`
