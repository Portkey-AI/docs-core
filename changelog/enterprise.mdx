...
sidebarTitle: "Enterprise Gateway [2.0.0]"
rss: true
---
...
- Set `MODEL_CONFIGS_PROXY_FETCH_ENABLED=ON` in Gateway to fetch pricing from Backend Control Plane
- See [Backend Changelog (1.7.0)](/changelog/backend#private-deployment-pricing) for details
...
- **Models Endpoint**: The `/v1/models` endpoint now accepts API keys with `completions:write` permission in addition to `virtual_keys:list`
...
- **Azure OpenAI**: Added support for batch output retrieval from Azure Blob Storage URLs (`output_blob`, `error_blob`) in addition to file IDs
- **Azure OpenAI**: Fixed Entra scope header handling for custom authentication scopes
