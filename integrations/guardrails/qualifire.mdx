...
<Card title="Get Started with Qualifire" href="https://docs.qualifire.ai" />
...
| Check Name | Description | Parameters | Supported Hooks |
|------------|-------------|------------|-----------------|
| JSON Check | Checks that the model returned a valid JSON object, optionally validated against a schema | `jsonSchema` (optional string) - [For more details](#json-check-with-schema-validation) | `afterRequestHook` |
| SQL Check | Checks that the model returned valid SQL code | None | `afterRequestHook` |
| Javascript Check | Checks that the model returned valid JavaScript code | None | `afterRequestHook` |
| Length Check | Checks that the model's output length meets the given constraint | `lengthConstraint` (string) - [For more details](#length-and-word-count-constraints) | `afterRequestHook` |
| Word Count Check | Checks that the model's output word count meets the given constraint | `wordCountConstraint` (string) - [For more details](#length-and-word-count-constraints) | `afterRequestHook` |
...
### JSON Check with Schema Validation

For the JSON Check, you can optionally provide a JSON schema for validation:
...
For Length and Word Count checks, use constraint strings like:

- `<100` - Less than 100 characters/words
- `=100` - Exactly 100 characters/words  
- `>=200` - At least 200 characters/words
...
- **Code Validation**: Ensuring AI-generated code (SQL, JavaScript, JSON) is syntactically correct
- **Output Control**: Managing response length and word count for specific use cases
