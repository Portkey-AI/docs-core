...
<Steps>
  ...
  <Step title="Configure Valves">
    ...
    2. Open the **Valves** panel and set:
       ...
       - **AUTO_DISCOVER_MODELS** to pull live catalog entries (recommended)
       - **PORTKEY_MODELS** curate the list particular model slugs from Model Catalog. Example: (@openai-provider/gpt-5, @anthropic-provider-slug/claude-sonnet-latest).
  </Step>
  ...
  <Step title="Bring Your Governance">
    ...
    2. Reference [Configs](https://docs.portkey.ai/configs) on API key level to enable routing, fallbacks, caching, or segmentation.
