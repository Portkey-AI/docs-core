...
## 1. Setting Up For Portkey


<Steps>
    <Step title="Create Portkey API Key">
      1. Go to the [**API Keys**](https://app.portkey.ai/api-keys) section on Portkey Sidebar.
      2. Create a new API key.
      3. Allow **all permissions**.
      4. Save and copy it — you'll use this in your OpenWeb UI requests.
    </Step>

  <Step title="Add Your Provider">
    1. Go to **Model Catalog → AI Providers**.
    <Frame>
      <img src="/Screenshot2025-07-21at5.29.57PM.png" alt="Portkey Model Catalog - Add Provider" />
    </Frame>
    2. Click **Create Provider** if this is your first time using Portkeys
    3. **Create New Integration** -> Select your AI service (OpenAI, Anthropic, etc.).
    4. Enter your provider’s API key and other details.
    5. Optionally do worksapce provisioning, and model provisioning
    6. Finally Create Integrations
  </Step>

  <Step title="Get your Model Slugs">
    - Go to the **Model Catlog** -> Models tab .
    - Find your models and save its slug (`@provider-slug/model-name`).
    ...
    <Note>
    Example: `@openai-test/gpt-4o`. This is your unique identifier — you’ll use it in the `model` field of API requests.
    </Note>
...
That's it! Now let's intgrate Portkey AI with OpenWebUI

## 2. Integrate Portkey with OpenWebUI

Now that you have your Portkey components set up, let's connect them to OpenWebUI. Since Portkey provides OpenAI API compatibility, integration is straightforward through OpenWebUI's connection settings.

<Note>
  You need your Portkey API Key from [Step 1](#1-setting-up-portkey) before
  proceeding.
</Note>
...
3. Navigate to the **Admin Panel** section
4. Click on the **Settings** tab
5. Select **Connections** from the sidebar menu

<Frame>
  <img
    src="/images/integrations/openwebui-settings-connection-page.png"
    width="400"
  />
</Frame>
...
<Step title="Enable Direct Connections">
  1. Toggle **Direct Connections** to enable 2. Toggle **OpenAI API** to enable
  3. Click the **+** icon next to Manage OpenAI API Connections
</Step>

<Step title="Configure Portkey Connection">
Fill in the following fields in the Edit Connection dialog:

- **URL**: `https://api.portkey.ai/v1`
- **Key**: Your Portkey API key from Step 1
- **Prefix ID**: `portkey` (or any identifier you prefer)
- **Model IDs**: Add your model identifiers (e.g., `@openai/gpt-4o`, `@anthropic/claude-3-sonnet`) from Model Catalog in Step 1
...
3. Choose your Portkey model (format: `@model-name`)
4. Start chatting!

<Note>
**For Anthropic models**: You must set a max tokens parameter. Click the settings icon in the top right and set the **Max Tokens** field to a valid value (e.g., 1024).
</Note>
</Step>
</Steps>
