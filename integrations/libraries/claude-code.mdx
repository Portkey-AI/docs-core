...
2. Find and copy your provider slug (e.g., `@anthropic-claude-code`)
...
<Note>
This is your unique identifier - you'll need it for the next step. This slug is basically `@your-provider-slug`
</Note>
...
<Note>
    This same JSON will work for Anthropic, Bedrock and Vertex. You will need your Portkey API key created in [Step 1](#1-setting-up-portkey) for this integration
</Note>

You can now use Claude Code with all of Portkey's enterprise features enabled. Monitor your requests and usage in the [Portkey Dashboard](https://app.portkey.ai/dashboard).


### Optional: Using Default Config to use a single model for all Claude Code requests

If you want to lock in a specific model for Claude Code and manage it centrally through Portkey, you can attach a default config to your API key. This is especially useful for enterprise deployments where you want consistent model usage across your team.
...
2. Create new config with:
    ...
    {
        "override_params": {
          "model": "@YOUR_PROVIDER_SLUG/YOUR_MODEL_NAME" // example: @anthropic-claude-code/claude-3-5-sonnet-20241022
        }
    }
    ```
3. Create or edit your API key in [API Keys](https://app.portkey.ai/api-keys)
4. Attach the config you just created to your API key

Once the config is attached, update your Claude Code settings to only include the API key (without the provider):
...
<Warning>
Using a default config allows you to use a single model on Portkey for all your Claude Code requests.
</Warning>
...
If you are using Claude Code inside your organization, you need to consider several governance aspects:
...
<Accordion title="Step 4: Set Routing Configuration">
    Portkey allows you to control your routing logic very simply with its Configs feature. Portkey Configs provide this control layer with things like:
...
- Cost tracking by the engineering team
...
<Note>
When using Max Plan or Anthropic Oauth, you can optionally attach a default config to your Portkey API key (see [Optional: Using Default Config to Fix Your Model](#optional-using-default-config-to-fix-your-model) above) to ensure consistent model usage. The config will automatically apply the model settings you defined in Portkey.
</Note>
