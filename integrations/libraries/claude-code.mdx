...
```json
{
  "env": {
    ...
    "ANTHROPIC_DEFAULT_SONNET_MODEL": "@YOUR_PROVIDER_SLUG/YOUR_PORTKEY_MODEL",
    "ANTHROPIC_DEFAULT_OPUS_MODEL": "@YOUR_PROVIDER_SLUG/OUR_PORTKEY_MODEL",
    "ANTHROPIC_DEFAULT_HAIKU_MODEL": "@YOUR_PROVIDER_SLUG/YOUR_PORTKEY_MODEL"
  }
}
```

<Note>
    This same JSON will work for Anthropic, Bedrock and Vertex. You will need your Portkey API key created in [Step 1](#Setting-up-portkey) for this integration. Note: Currently, Claude Code only supports Anthropic Models. It's recommended set all the models you want to use in your Claude Code setup as shown above to avoid errors.
</Note>

<Tip>
### Using Default Config with Your API Key

If you want to use a specific model with your provider API key and apply advanced Portkey features (routing, fallbacks, guardrails, caching), you can attach a **default config** to your Portkey API key:

1. **Create a Config** in [Portkey Configs](https://app.portkey.ai/configs):
```json
{
  "override_params": {
    "model": "@YOUR_PROVIDER_SLUG/claude-sonnet-4-20250514"
  }
}
```

2. **Attach the Config to your API Key**: Go to [API Keys](https://app.portkey.ai/api-keys), create or edit an API key, and select your config as the default.

3. **Simplified Claude Code Setup**: With the config attached, you can simplify your `settings.json`:
```json
{
  "env": {
    "ANTHROPIC_BASE_URL": "https://api.portkey.ai",
...
The model and routing logic will automatically be applied from your attached config. This approach is where you want centralized control over model selection and routing without modifying individual developer configurations.

<Card title="Enforce Default Configs" icon="shield-check" href="/product/administration/enforce-default-config">
