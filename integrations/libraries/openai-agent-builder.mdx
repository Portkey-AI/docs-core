...
<Note>
    In your model field of your OpenAI Agent SDK code. You need to enter the Portkey Model Slug from Model catalog. Example : @openai-provider-slug/gpt-5, @anthropic-provider/claude-sonnet-latest
</Note>
...
const portkey = new OpenAI({
    ...
    apiKey: process.env.PORTKEY_API_KEY!,
    defaultHeaders: {
        "x-portkey-config": "your-config-id" //or pass the JSON directly
    }
});
setDefaultOpenAIClient(portkey);
```
