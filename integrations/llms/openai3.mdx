...
- Directly use OpenAI API key without Model Catalog: ([Docs](/api-reference/inference-api/headers#1-provider-slug-auth))
- Set budget & rate limits per AI Provider: ([Docs](/product/model-catalog))
</Info>
<Note>
...
<Card horizontal icon="lightbulb" title="More details here →" href="/self-hosting/hybrid-deployments/architecture" />
...
| `responses`           | model, input, tools, max_output_tokens, temperature, top_p, stream, metadata, seed, conversation, modalities                                                                                                                           |
