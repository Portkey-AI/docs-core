...
Portkey supports Together AI's video generation models through the AI Gateway. Generate videos using the Portkey SDK or REST API:
...
const video = await portkey.videos.create({
    model: "@together-ai/minimax/hailuo-02",
    prompt: "A serene sunset over the ocean with gentle waves",
    width: 1366,
    height: 768
...
```sh cURL
curl https://api.portkey.ai/v1/v2/videos \
  -H "Content-Type: application/json" \
  -H "x-portkey-api-key: $PORTKEY_API_KEY" \
