...
With Portkey, you can take advantage of features like fast AI gateway access, observability, prompt management, and more, all while ensuring the secure management of your LLM API keys through a [Provider](/product/model-catalog) system.
...
Portkey also supports the [token counting endpoint for bedrock](https://docs.anthropic.com/en/docs/build-with-claude/token-counting). Checkout the example in this link for more details.
</Card>
...
## Using AWS Bedrock Guardrails in Requests

You can apply AWS Bedrock guardrails directly within your inference requests using the `guardrailConfig` parameter. This allows you to use guardrails you've created in AWS Bedrock without setting them up in Portkey.

<Note>
This is different from [Portkey's Bedrock Guardrails integration](/integrations/guardrails/bedrock-guardrails), which provides additional features like PII redaction and custom actions.
</Note>

### Guardrail Configuration

Pass the `guardrailConfig` object in your request with the following parameters:

- `guardrailIdentifier`: The unique identifier of your AWS Bedrock guardrail (ID or ARN)
- `guardrailVersion`: The version of the guardrail to use (or `"DRAFT"`)
- `trace`: (Optional) Set to `"enabled"` to get detailed information about guardrail actions
...
<Tabs>
    ...
    <Tab title="OpenAI Python SDK">
        ...
        client = OpenAI(
            ...
            base_url=PORTKEY_GATEWAY_URL,
            default_headers=createHeaders(
                provider="@bedrock",  # Your Bedrock Provider Slug
                api_key="PORTKEY_API_KEY"
            )
        )
...
When a guardrail is triggered, the response will include trace information showing what was detected and blocked. The guardrail can:

- **Block the request** if harmful content is detected in the input
- **Block the response** if harmful content is detected in the model output
- **Redact PII** if configured to do so in your AWS Bedrock guardrail settings

For more details on AWS Bedrock Guardrails, see the [AWS documentation](https://docs.aws.amazon.com/bedrock/latest/userguide/guardrails-use-converse-api.html).
