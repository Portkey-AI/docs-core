...
| Model | ARN |
|-------|-----|
| Cohere Rerank v3.5 | `arn:aws:bedrock:us-east-1::foundation-model/cohere.rerank-v3-5:0` |
...
|-----------|------|----------|-------------|
| `model` | string | Yes | The full ARN of the Bedrock rerank model |
| `query` | string | Yes | The search query to rerank documents against |
| `documents` | array | Yes | List of documents (strings) to rerank |
| `top_n` | integer | No | Number of top results to return |

For more details on using the `/rerank` endpoint with other providers, see [Gateway to other APIs](/api-reference/inference-api/gateway-for-other-apis).
