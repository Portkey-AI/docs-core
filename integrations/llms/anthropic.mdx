...
With Portkey, you can take advantage of features like fast AI gateway access, observability, prompt management, and more, while securely managing your API keys through [Model Catalog](/product/model-catalog).
...
<CardGroup cols={3}>
  ...
  <Card title="Multi-Provider Support" icon="check-circle" color="#10b981">
...
<Note>
Anthropic **only accepts base64-encoded images**. Unlike OpenAI, it does not support image URLs. With Portkey, you can use the same format to send base64-encoded images to both Anthropic and OpenAI models.
</Note>
...
Anthropic Claude can process PDFs to extract text, analyze charts, and understand visual content. PDF support is available on:
...
Models like `claude-3-7-sonnet-latest` support [extended thinking](https://docs.anthropic.com/en/docs/build-with-claude/extended-thinking#streaming-extended-thinking). You get the model's reasoning as it processes the request.
...
You need to set `strict_open_ai_compliance=False` to use this feature:
...
You can manage all prompts to Anthropic in the [Prompt Library](/product/prompt-library). All current Anthropic models are supported, and you can easily test different prompts.

Once you're ready, use the `portkey.prompts.completions.create` interface to use the prompt in your application.
