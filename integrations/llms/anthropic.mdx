...
## Using Anthropic's SDKs with Portkey

Portkey now supports using Anthropic models directly on the `/messages` route, making it easy to leverage the latest Anthropic SDKs and API patterns with Portkey’s gateway features (like caching, observability, and virtual keys).

You can use this with cURL, the official Anthropic Python SDK, and the Anthropic TypeScript SDK by simply pointing your requests to Portkey and adding the required headers.
