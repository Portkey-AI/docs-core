...
<Info>
Portkey provides detailed monitoring for all Responses API interactions, including:

- Token usage breakdown by tool and request
- Latency metrics for each tool operation
- Success rates for tool invocations
- Cost tracking per tool call and reasoning step

This data helps optimize performance and costs while providing insights into how different users engage with AI features.


All responses and tool interactions are automatically logged by Portkey with detailed breakdowns of each step, enabling you to trace multi-step tool execution patterns.
