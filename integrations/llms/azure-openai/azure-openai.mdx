...
Azure OpenAI image edit requests use `multipart/form-data` because the request can include binary image inputs. For these requests, keep using your Portkey provider or virtual key configuration for routing, but pass the Azure deployment/model name directly in the multipart `model` field.
...
<Note>
Portkey does not rewrite provider-prefixed model aliases inside multipart payloads. Multipart requests can contain large binary files, so the model value should already match the Azure deployment/model name expected by Azure OpenAI.
</Note>
