...
## Custom Metadata Labels for Cost Attribution

Vertex AI supports adding custom metadata labels to your API calls for cost tracking and attribution. These labels are forwarded to Google Cloud and can be used to filter and analyze costs in your GCP billing reports.

<Info>
Labels are specific to Vertex AI and are ignored when calling other providers through Portkey.
</Info>

### Passing Labels in Requests

You can pass labels directly in your request body:
...
### Passing Labels in Configs

You can also configure labels at the config level using `override_params`. This is useful for automatically applying labels to all requests from a specific service or API key:
...
When you attach this config to an API key, all requests made with that key will automatically include these labels without requiring code changes. This is particularly useful for:

- **Cost attribution by service**: Label requests with unique service IDs to track costs per microservice
- **Environment tracking**: Separate costs between production, staging, and development
- **Team or project allocation**: Attribute costs to specific teams or projects

<Card title="Learn more about configs" href="/product/ai-gateway/configs">
  See the full documentation on Gateway Configs
</Card>

<Note>
Learn more about Google Cloud labels in the [official documentation](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/multimodal/add-labels-to-api-calls).
</Note>
