...
#### Method 1: Sending an Image via a Public URL

This is the recommended approach for images hosted online. The URL must be publicly accessible.
...
--data '{
    ...
    "messages": [{
        ...
        "content": [
            ...
            {
                "type": "image_url",
                "image_url": { "url": "gs://cloud-samples-data/generative-ai/image/scones.jpg" }
            }
        ]
    }]
}'
...
Use this method for local image files. The file is encoded into a Base64 string and sent as a data URI. This is ideal when you don't have a public URL.
...
#### Method 1: Sending Audio via a Public URL

Provide a direct Google Files link to an audio file.
        ...
        "content": [
            {
                "type": "image_url",
                "image_url": {
                    "url": "https://storage.googleapis.com/public-samples/voice.mp3"
                }
            },
...
This is the standard way to process local audio files through the API.
...
```
```sh cURL
# You would first encode your audio file to base64
# For example: base64 audio-example.mp3 > audio.b64
# Then paste the content into the request
curl --location 'https://api.portkey.ai/v1/chat/completions' \
--header 'x-portkey-provider: google' \
--header 'x-portkey-api-key: YOUR_PORTKEY_API_KEY' \
--header 'Authorization: YOUR_GEMINI_API_KEY' \
...
<Card href="https://ai.google.dev/gemini-api/docs/video-understanding#inline-video" title="Gemini Video Understanding Docs">
</Card>


#### Method 1: Sending a Video via YouTube URL

This is the only supported method for video. You can use a direct link to a YouTube video. Base64 encoding for videos is generally not feasible due to large file sizes.
...
This is the standard way to process local video files through the API.
...
```
```sh cURL
# You would first encode your video file to base64
# For example: base64 video-example.mp3 > video.b64
# Then paste the content into the request
curl --location 'https://api.portkey.ai/v1/chat/completions' \
--header 'x-portkey-provider: google' \
--header 'x-portkey-api-key: YOUR_PORTKEY_API_KEY' \
...
#### Method 3: Sending Video via a Public URL
        ...
        "content": [
            {
                "type": "image_url",
                "image_url": {
                    "url": "https://storage.googleapis.com/public-samples/voice.mp4"
                }
            },
            { "type": "text", "text": "Please transcribe the speech in this." }
        ]
    }],
)
```
...
#### Method 1: Sending a Document via a Google Files URL
