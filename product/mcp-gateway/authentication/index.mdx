...
└────────────────────────────────────────────────────────────────────┘
                                  ...
                                  │ (Agent proves identity to Portkey)
                                  │
                                  │ Methods: API Key, Portkey OAuth, External IdP
                                  ▼
┌────────────────────────────────────────────────────────────────────┐
│                        Portkey MCP Gateway                          │
│  ┌──────────────────────────────────────────────────────────────┐  │
...
│  │ • Extracts user identity for forwarding                       │  │
│  │ • Applies rate limits and logging                             │  │
│  └──────────────────────────────────────────────────────────────┘  │
└────────────────────────────────────────────────────────────────────┘
                                  │
                                  │ MCP Server Authentication
                                  │ (Portkey proves identity to MCP Server)
                                  │
                                  │ Methods: OAuth Auto, Client Credentials, Headers
                                  ▼
┌────────────────────────────────────────────────────────────────────┐
│                          MCP Server                                 │
└────────────────────────────────────────────────────────────────────┘
...
<Info>
**How it works:** If your request includes an API key, Portkey uses it. If you include a JWT from an external IdP (with JWT validation configured), Portkey validates that. If neither, Portkey initiates the OAuth flow. You don't need to configure which method to use—just include an API key or don't.
</Info>
...
Use your existing identity provider (Okta, Auth0, Azure AD, Cognito). Configure Portkey to validate tokens from your IdP. Your users authenticate with your existing identity system—they don't need Portkey accounts.
...
You can customize the OAuth client metadata Portkey uses during registration. See [OAuth Client Metadata](/product/mcp-gateway/authentication/oauth-client-metadata).
...
After authenticating a user, Portkey can forward their identity claims to MCP servers. This lets MCP servers make authorization decisions without implementing OAuth themselves.
