...
Your agents need to prove their identity to Portkey before accessing MCP servers. Three options:
...
API keys are scoped to teams. The key determines which MCP servers the request can access.

Create keys in **Settings > API Keys**.

### OAuth 2.1

For browser-based flows where users authenticate interactively. Portkey acts as the OAuth provider. After authentication, your app receives a token to use for MCP requests.
