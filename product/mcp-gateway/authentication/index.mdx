...
```mermaid
flowchart TB
    subgraph Client["Your Agent / Client"]
    end
    
    Client -->|"Gateway Authentication<br/>(Agent proves identity to Portkey)<br/><br/>Methods: API Key, Portkey OAuth, External IdP"| Gateway
    
    subgraph Gateway["Portkey MCP Gateway"]
        GatewayFunctions["• Validates incoming requests<br/>• Checks workspace permissions<br/>• Extracts user identity for forwarding<br/>• Applies rate limits and logging"]
    end
    
    Gateway -->|"MCP Server Authentication<br/>(Portkey proves identity to MCP Server)<br/><br/>Methods: OAuth Auto, Client Credentials, Headers"| MCPServer
