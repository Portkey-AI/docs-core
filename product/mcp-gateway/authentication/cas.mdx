...
1. **MCP client requests access** — The client (e.g., Cursor, Claude Desktop) connects to an MCP server through the AI Gateway.
2. **CAS login** — The user is redirected to the CAS login page, where they authenticate using their organization's identity provider.
3. **User resolved** — Portkey verifies the authenticated identity, resolves the user, and determines workspace access.
4. **Token issued** — An OAuth access token is issued to the MCP client, scoped to the user's workspace permissions.
...
CAS authentication is automatically enabled for organizations in SCM deployment mode. No additional configuration is required on the MCP Gateway side — the gateway detects SCM mode and routes authentication through CAS.
