...
- You want user-level attribution without managing API keys yourself
- You need a quick way to get started without configuring an external IdP
...
MCP clients like Claude Desktop and Cursor handle OAuth automatically. When you configure a server URL without an API key, the client initiates the OAuth flow when needed.
...
The first time you use a tool, the client opens a browser for authentication.
...
You can use Portkey's OAuth for some users and external OAuth for others. Portkey determines the authentication method based on what's provided in the request:
