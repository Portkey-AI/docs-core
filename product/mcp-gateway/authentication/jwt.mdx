...
Your organization has an existing identity provider (Okta, Auth0, Azure AD, Cognito). Users authenticate through it. You want MCP access to work the same way.

With JWT validation:
1. Users get a token from your IdP
2. They include it in MCP requests
3. Portkey validates the token against your IdP
...
Embed public keys directly in the configuration. Use for self-contained deployments or when you can't expose a JWKS endpoint.
...
**Best for:** Opaque tokens, real-time revocation checking, or when you need to validate token status against the IdP on every request.
...
By default, Portkey reads the token from the `Authorization` header. To use a different header:
