...
| `caller.agent.id` | Agent identifier, when the call was made by an authenticated agent |
| `caller.agent.name` | Agent display name |
| `caller.agent.team` | Team the agent belongs to |
| `caller.agent.owner_id` | Identifier of the agent's owner |
| `caller.agent.type` | Agent type (e.g. `CUSTOM`) |

The `caller.agent.*` fields are only present for tokens whose introspection response identifies an agent — `caller.user_id` continues to record the human the agent acts on behalf of, so agent traffic is distinguishable from that person working by hand.
