...
Most external MCP servers use OAuth for authentication. When you add a server with OAuth:
...
Portkey works with any MCP server that supports HTTP transport. Popular options include:
...
<Card title="MCP Server Catalog" icon="grid" href="/integrations/mcp-servers">
  Browse setup guides for popular MCP servers.
</Card>
...
Use shared credentials for services that don't have user-specific data or when you want all users to share the same access level.
