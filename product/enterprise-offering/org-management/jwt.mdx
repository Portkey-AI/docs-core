...
Workspace is determined after the token is verified, using token claims first, then a request header, then deployment settings.
...
**From the request** (attributed, user-identified tokens only — ignored for service-key requests, and never overrides a workspace already present in the token):

- `x-portkey-workspace` header, set to a workspace slug or id

**From deployment settings** (when the token has no workspace claim or header hint):
