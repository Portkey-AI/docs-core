...
When a mapping is deleted, the users provisioned through that mapping are removed from **that workspace only**. Any other workspaces the group is still mapped to are untouched. If the deleted mapping was the only one pointing to a non-default workspace and no users remain after cleanup, the workspace itself may be archived as part of the cleanup; the organization's default/shared workspace is never archived.

<Warning>
Deleting a mapping affects only the workspace it points to. Other workspace mappings for the same SCIM group continue to operate normally.
</Warning>
