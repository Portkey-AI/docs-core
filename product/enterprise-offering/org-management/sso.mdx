...
<Note>
SSO will only provision users whose email belongs to a **valid and verified domain** configured for your organization in the Portkey dashboard (`Admin Settings > Authentication Settings > Allowed Domains`). Users with unverified or unconfigured domains will not be provisioned.
</Note>
...
By default Portkey authenticates to your identity provider's token endpoint with `client_secret_basic`. If your provider requires a different method, set **`token_endpoint_auth_method`** in your OIDC settings (for example `client_secret_post` or `private_key_jwt`).

For `private_key_jwt`, also supply a **`jwks`** object containing your client's private key set; register the matching public JWKS with your identity provider. When a JWKS is supplied without an explicit method, Portkey uses `private_key_jwt` automatically.
