...
Organisation owners/admins can enforce a `rotation_period` on workspace-user API keys via the `user_api_key_rotation_period` setting (with optional per-workspace override). When set, newly-created workspace-user keys without a rotation policy automatically get the enforced period, and any explicit `rotation_period` that differs from the enforced value is rejected at create/update time.
