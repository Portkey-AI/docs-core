...
Portkey supports semantic caching capabilities that allow for intelligent caching based on the semantic meaning of requests. For this functionality, Portkey requires a vector database to efficiently store and retrieve embeddings. Milvus is the supported vector database for Portkey's semantic cache implementation. To set up Milvus as your semantic cache store, you need:
