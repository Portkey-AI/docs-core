...
When you retrieve this secret reference via the API, sensitive fields like `aws_secret_access_key` and `aws_access_key_id` are automatically [masked](#sensitive-field-masking) — the actual values are never exposed in API responses.
...
At runtime, Portkey's data plane fetches the OAuth metadata from AWS Secrets Manager and injects it into the MCP integration configuration — no credentials are stored in Portkey.
