...
Requires Gateway version **2.2.4** or higher, and Backend version **1.12.0** or higher (for air-gapped deployments).
</Info>
...
3. The control plane never fetches or sees the final secret. The secret value lives only in the data plane's memory for the duration of the request.
...
- **Authentication**: Bearer JWT or `x-portkey-api-key` header.
