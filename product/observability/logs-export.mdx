...
<Info>
   Logs export feature is only available on the enterprise plan. For more info reach out to [Portkey support](https://support.portkey.ai/forms/customer-portal-ticket-form)
</Info>
...
7. For completed exports, click the **Download** button to get your logs data file. You can
  ...
  <Note>
      Currently we only support exporting 50k logs per job. For more help reach out to the Portkey team via [Portkey support](https://support.portkey.ai/forms/customer-portal-ticket-form)
  </Note>

## Restricting Exportable Fields

Organisation owners and admins can prevent sensitive log fields — such as `request` and `response` payloads or `metadata` — from leaving Portkey through exports.
...
Individual workspaces can set their own `export_settings` only if the organisation has enabled the `export_settings` [workspace override](/product/administration/enforce-default-config). Otherwise the organisation's list applies to all workspaces.
...
```bash
curl "https://api.portkey.ai/v1/logs/exports/field-restrictions?workspace_id=WORKSPACE_ID" \
...
If you have any questions or need assistance with the logs export feature, reach out to the Portkey team via [Portkey support](https://support.portkey.ai/forms/customer-portal-ticket-form).
