...
A fallback target can itself be a load balancer. This is useful when you want to distribute traffic across multiple providers for throughput, but still have a cross-provider safety net if the entire primary cluster becomes unavailable.
...
<Info>
You can also use this pattern as a named target inside a [conditional routing](/product/ai-gateway/conditional-routing) config — see the [multi-strategy routing guide](/guides/use-cases/combining-routing-strategies) for a complete four-model example.
</Info>
