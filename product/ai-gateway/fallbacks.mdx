...
<CardGroup cols={2}>
  <Card title="Fallback Wrapping a Load Balancer" icon="scale-balanced" href="/guides/use-cases/combining-routing-strategies#distribute-with-a-fallback-safety-net">
  ...
  <Card title="Fallback to a Conditional Router" icon="code-branch" href="/guides/use-cases/combining-routing-strategies#fall-back-to-a-conditional-router">
  ...
  <Card title="Per-Leg Fallbacks in a Load Balancer" icon="layer-group" href="/guides/use-cases/combining-routing-strategies#per-leg-fallbacks-in-a-load-balancer">
    Give each load-balanced slot its own independent fallback chain. Model family A failing doesn't trigger model family B's backup.
  </Card>
