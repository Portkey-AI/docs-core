...
| ---------------------- | -------------------------- | ------------ |
| **[Provider Batch API](#provider-batch-api-mode)** | *Cheapest* for overnight or offline jobs. Uses the provider’s native batch endpoint & limits.| `OpenAI`, `Azure OpenAI`, `Bedrock`, `Vertex`, `Fireworks` |
| **[Portkey Batch API](#portkey-batch-api-mode)**  | *Fastest* and provider‑agnostic. Batches at the Gateway layer; ideal when a provider has no native batch support or you need cross‑provider jobs. | Any provider supported by Portkey |
...
4. Once batch is completed, portkey will create read the batch output for the following details which will be added to your portkey analytics.
    - Token count
    - Cost
