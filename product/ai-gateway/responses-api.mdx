...
For native providers, use `previous_response_id` to chain turns without resending the full conversation history. The server manages state — you send only the new input.

<CodeGroup>
```python Python
# First turn
response1 = portkey.responses.create(
    model="@google-provider/gemini-3.5-flash",
...
response2 = portkey.responses.create(
    model="@google-provider/gemini-3.5-flash",
    input="What is my name and job?",
    previous_response_id=response1.id
)

print(response2.output_text)  # "Your name is Alice and you're a software engineer."
```

```javascript JavaScript
// First turn
const response1 = await portkey.responses.create({
    model: "@google-provider/gemini-3.5-flash",
    input: "My name is Alice and I'm a software engineer.",
    store: true
});
...
// Second turn
const response2 = await portkey.responses.create({
    model: "@google-provider/gemini-3.5-flash",
    input: "What is my name and job?",
    previous_response_id: response1.id
});
...
```sh cURL
# First turn
RESPONSE=$(curl -s https://api.portkey.ai/v1/responses \
  -H "Content-Type: application/json" \
  -H "x-portkey-api-key: $PORTKEY_API_KEY" \
  -d '{
    "model": "@google-provider/gemini-3.5-flash",
...
# Second turn — server remembers the context
curl https://api.portkey.ai/v1/responses \
  -H "Content-Type: application/json" \
  -H "x-portkey-api-key: $PORTKEY_API_KEY" \
  -d "{
...
<Info>
`previous_response_id` is supported by **OpenAI**, **Azure OpenAI**, **Google Gemini**, and **Google Vertex AI**. For Gemini, Portkey maps this to the native Interactions API (`previous_interaction_id`) for server-side conversation state.

For all other providers, use the `input` array to pass full conversation history in each request.
</Info>
...
**Native providers:** OpenAI, Azure OpenAI, Google Gemini, Google Vertex AI, Grok (x.ai), Groq, OpenRouter, Azure AI, Perplexity AI 

**Adapter providers:** Anthropic, AWS Bedrock, Mistral AI, Together AI, and [all other providers](/integrations)
...
- **`previous_response_id`** — Works with OpenAI, Azure OpenAI, Google Gemini, and Google Vertex AI. Gemini uses its native Interactions API (`previous_interaction_id`) for server-side conversation state. For other providers, use [multi-turn conversations](#multi-turn-conversations) to pass history in the `input` array instead
- **`store`** — Silently ignored on adapter providers; responses are not persisted server-side. On Gemini, defaults to `true` (required for `previous_response_id`)
- **Retrieve / Delete** — `GET` and `DELETE` on `/v1/responses/:id` work with OpenAI, Azure OpenAI, Google Gemini, and Google Vertex AI
