...
To pin a `custom_host` (and optional `custom_headers`) on a specific Model Catalog model instead of every request, see [Per-model custom host and headers](/product/model-catalog/custom-models#per-model-custom-host-and-headers).
