...
<Info>
Semantic cache is a superset of simple cache. Portkey checks for an exact match first and only runs semantic search on a miss.
</Info>
...
- The text is too different (similarity below the threshold)
- `model`, `temperature`, or any other parameter differs — **even if the text is identical**
- `x-portkey-metadata` values don't match
- The chat has only a `system` message — at least one user message is required
