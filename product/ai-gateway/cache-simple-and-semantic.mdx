...
<Steps>
  <Step title="Configure the embedding provider">
    Set the following environment variables in your gateway environment:
  ...
  <Step title="Configure the vector database">
    Set the following environment variables to connect to your vector store (Milvus or Pinecone):
...
For more details on configuring the vector database, see [Enterprise components](/product/enterprise-offering/components#semantic-cache-store).
