...
Conditional routing happens on Portkey's on-the-edge stateless AI Gateway. We scan for the given query fields in your request body, apply the query conditions, and route to the specified target based on it.
...
<Accordion title="Routing Based on Request Parameters">
...
When using parameter-based routing, make sure to include the parameter specified in your routing condition in your request. In the example above, you need to include `model` in your request for routing to work properly.
  ...
  <Accordion title="Routing Based on Metadata">
...
5. **Multiple Parameter Types**: You can create sophisticated routing rules by combining conditions on parameters, metadata, or a mix of both.

<Info>
Portkey supports routing based on any parameter that can be sent in LLM requests including `model`, `temperature`, `top_p`, `frequency_penalty`, `presence_penalty`, `max_tokens`, and many others.
