...
Each target in a conditional routing config can itself be a load balancer or fallback — not just a single provider. This enables model-level routing and provider-level distribution in a single config.

This example routes four model aliases to different strategies:
