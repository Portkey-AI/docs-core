...
<Accordion title="Conditional Routing with Load-Balanced Targets">

Each target in a conditional routing config can itself be a load balancer or fallback — not just a single provider. This lets you combine model-level routing with provider-level distribution in a single config.

The example below routes four model aliases to different strategies:

- **`claude-sonnet`** → load balanced across Anthropic, Vertex AI, and AWS Bedrock (triples rate limits)
- **`gpt-4o`** → single OpenAI target
- **`gpt-4o-mini`** → load balanced across Azure OpenAI and two OpenAI accounts
- **`gemini-2.5-pro`** → load balancer wrapped in a fallback (if all Gemini providers fail, routes to GPT-4.1)
...
See the [full guide with code samples and setup instructions](/guides/use-cases/combining-routing-strategies).
</Accordion>
