...
2. **`x-portkey-virtual-key` header** — Used directly as the virtual key.
3. **Request body `model` field** — If the model uses the `@slug/model-name` format (e.g., `@anthropic-dev/claude-sonnet-4-20250514`), the slug is extracted as the virtual key and the model name is passed through to the provider.
4. **URL query parameter** — Virtual key extracted from the query string (if present).
