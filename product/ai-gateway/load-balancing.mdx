...
A load balancer can be a named target inside a [conditional routing](/product/ai-gateway/conditional-routing) config — route by model or metadata first, then distribute traffic across multiple providers within each branch.

**Example: Claude Sonnet load balanced across Anthropic, Vertex AI, and Bedrock**

When `params.model` is `claude-sonnet`, traffic spreads evenly across three providers, each running Claude Sonnet 4.5 with independent rate limit buckets:
...
The outer conditional router picks which load balancer to call; the inner load balancer selects the provider within that model family. The application sends `model: "claude-sonnet"` — all provider selection happens in the gateway.
