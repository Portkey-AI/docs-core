...
## Load Balancing as a Target in Conditional Routing

A load balancer can be a named target inside a [conditional routing](/product/ai-gateway/conditional-routing) config. This lets you route by model or metadata first, then distribute traffic across multiple providers within each branch.

**Example: Claude Sonnet load balanced across Anthropic, Vertex AI, and Bedrock**

When `params.model` is `claude-sonnet`, traffic is spread evenly across three providers — each running the same Claude Sonnet 4.5 model but with independent rate limit buckets:
...
The outer conditional router picks which load balancer to call; the inner load balancer decides which provider within that model family gets the request. Your application only ever sends `model: "claude-sonnet"` — all provider selection happens in the gateway.

See the [multi-strategy routing guide](/guides/use-cases/combining-routing-strategies) for a complete example with four model branches, including a load balancer wrapped in a fallback.
