...
<Note>
Use `strategy.sticky` as the supported configuration shape. Migrate any legacy `strategy.sticky_session` configs to `strategy.sticky` with `enabled`, `hash_fields`, and `ttl`.
</Note>
