...
The `@provider-slug/model-name` format references AI Providers configured in [Model Catalog](https://app.portkey.ai/model-catalog). The outer conditional router picks which load balancer to call; the inner load balancer selects the provider. The application sends `model: "claude-sonnet"` — all provider selection happens in the gateway.
