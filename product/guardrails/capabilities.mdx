...
All of them. Guardrails run at the Gateway layer, so the same Guardrail ID works across every provider Portkey supports — OpenAI, Anthropic, Google, AWS Bedrock, Azure, Mistral, self-hosted models, and more. No provider-specific configuration needed (for supported endpoints).
