...
<Note>
  Decisions requests only expose the `state` field to guardrails — `model`, `questions`, and `criteria` are your own instructions and are never scanned. Typed answers have no free text to evaluate, so only `input_guardrails` apply. See [Guardrails for Decisions](/product/guardrails/decisions-guardrails).
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Guardrails for Decisions" href="/product/guardrails/decisions-guardrails">
