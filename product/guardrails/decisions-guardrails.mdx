...
Portkey's guardrails also cover `/v1/decisions` — the typed-judgment endpoint for models like [TypeSafe's Jev](/integrations/llms/typesafe). This lets you screen the content a decision is made about before it reaches the model, the same way you would for a chat or embedding request.
