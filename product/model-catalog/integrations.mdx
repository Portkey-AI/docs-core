...
When creating an integration via API, `allow_all_models` defaults to **`false`**. This means:
- All catalog models are pre-populated with a **disabled** (archived) status
- No models are accessible until you explicitly enable them
- To allow all models, pass `"allow_all_models": true` in the create request

<Note>
**For API users:** If you previously relied on models being accessible by default after creating an integration without specifying `allow_all_models`, you must now explicitly pass `"allow_all_models": true` in your create integration request. This change ensures integrations follow a secure-by-default approach — no model access is granted unless explicitly configured.
</Note>
