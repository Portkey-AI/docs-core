...
**Model lists are tied to Integrations.** When you create an Integration, the full list of supported models from our catalog is pre-populated. You control which models are available by enabling or disabling them. All Providers created from that Integration will only have access to the models you've enabled.
...
#### Default Behavior (effective August 18, 2026)

When creating an integration via the `POST /v1/integrations` API, the `allow_all_models` parameter now defaults to **`false`**.

**What this means for new integrations created after August 18, 2026:**
- All catalog models are pre-populated with a **disabled** status
- No models are accessible until you explicitly enable them or pass `"allow_all_models": true`

**What the old behavior was (before August 18, 2026):**
- `allow_all_models` defaulted to `true`
- No model list was stored on the integration
- All models were accessible immediately after creation without any additional configuration

<Note>
**Action required for API users:** If you create integrations via the API and previously relied on all models being accessible by default (without explicitly passing `allow_all_models`), you must now include `"allow_all_models": true` in your `POST /v1/integrations` request body to retain the same behavior.

If you do not pass this parameter, your newly created integration will have no accessible models until you explicitly enable them.

This change does **not** affect existing integrations — only new integrations created after August 18, 2026.
</Note>
...
2. Select the configuration options:
   - **Allow All Models**: Provides access to all models offered by the provider (sets `allow_all_models: true`)
   - **Allow Specific Models**: Create an allowlist of approved models (sets `allow_all_models: false`, then enables selected models)
...
#### Updating Model Access via API

When updating models on an existing integration using `PUT /v1/integrations/{slug}/models`, the `override_existing_model_access` flag controls how your update is applied:

| Flag | Behavior |
| :--- | :--- |
| `override_existing_model_access: true` | All existing models are disabled first, then only the models you specify are enabled. Use this to **restrict** access to a specific subset. |
| `override_existing_model_access: false` (default) | Your specified models are merged into the existing list without affecting other models. This is an **additive** update. |

**Example: Restrict to specific models after creation**
...
curl -X PUT "https://api.portkey.ai/v1/integrations/openai-prod/models" \
  ...
  -d '{
    "models": [
      ...
      {"slug": "gpt-4o-mini", "enabled": true}
    ],
    "allow_all_models": false,
    "override_existing_model_access": true
  }'
```
