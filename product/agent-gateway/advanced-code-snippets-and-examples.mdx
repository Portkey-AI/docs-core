...
<Info>
  Modify the API key permissions and turn on the toggle for **Agent Gateway** to enable Agent Gateway for your API key. See the [Quickstart](/product/agent-gateway/quickstart) for where to configure this in the app, including the `agents.invoke` scope.
</Info>
