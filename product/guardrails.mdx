...
Portkey supports hook results in streaming responses for `/chat/completions`, `/completions`, `/embeddings`, and `/messages` endpoints.

- **Input guardrails** (`input_guardrails` / before request hooks) are evaluated before the stream starts. If the check passes, the stream proceeds normally. If it fails with `deny=true`, the stream is blocked with `446` before any tokens are sent.
        ...
        async for chunk in response:
            if chunk.model_extra and chunk.model_extra.get("hook_results"):
                ...
                if len(hook_results.get("after_request_hooks", [])) > 0:
                    # Output guardrail results (informational only, no fallback/retry)
                    pass
