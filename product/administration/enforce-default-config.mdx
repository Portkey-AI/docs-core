...
1. Create one or more [configs](/product/ai-gateway/configs) with your desired settings
2. Note the config ID(s) you want to attach to your API keys
...
<Tabs>
  ...
  <Tab title="Admin API Keys">
    Navigate to **Admin Settings** → **Admin Keys** tab, then:
...
### Attaching Configs via the API

You can also programmatically attach configs when creating or updating API keys using the Portkey API.
