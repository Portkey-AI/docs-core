...
Rate limit policies now support a `target` field to distinguish between LLM and MCP workloads:

- `target: llm` — Applies to LLM API requests (default, existing behavior)
- `target: mcp_tools` — Applies to MCP tool calls through the MCP Gateway

MCP rate limit policies support conditions and grouping by `api_key`, `workspace_id`, `mcp_server`, and `mcp_tool`.

<Note>
`api_key` is not available as a condition or group-by key in OAuth-based MCP flows.
</Note>

For full details on configuring MCP rate limits, see [MCP Gateway Rate Limits](/product/mcp-gateway/rate-limits).
