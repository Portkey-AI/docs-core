...
Rate limit policies support a `target` field to apply limits to MCP tool calls separately from LLM requests:

- `target: llm` — Applies to LLM API requests (default)
- `target: mcp_tools` — Applies to MCP tool calls through the MCP Gateway

MCP policies can be scoped using conditions like `mcp_server` and `mcp_tool`, and grouped by `api_key`, `workspace_id`, `mcp_server`, or `mcp_tool`.

<Note>
`api_key` is not available as a condition or group-by key in OAuth-based MCP flows, since those requests do not carry an API key.
</Note>

For full details, see [MCP Gateway Rate Limits](/product/mcp-gateway/rate-limits).
