...
<Note>
  Saved-only mode gates inline provider **selection**, not credentials. Passing an `Authorization` header on its own is still allowed — credentials by themselves cannot select a provider, and saved providers supply their own credentials.
</Note>
