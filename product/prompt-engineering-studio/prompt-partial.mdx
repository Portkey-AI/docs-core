...
<Frame>
  <img src="/images/product/prompts/prompt-partials-list.png"/>
</Frame>
...
<Frame>
  <img src="/images/product/prompts/prompt-partials-editor-view.png"/>
</Frame>
...
<Frame>
  <img src="/images/product/prompts/prompt-partials-variables-panel.png"/>
</Frame>
...
## Sharing Partials Across Workspaces

Share partials with other workspaces in your organization with read-only access. Recipients can reference the partial in their prompts or fork it into their own workspace to customize.

When forking a shared partial, the destination workspace must have access to any LLM integrations the partial depends on through [Workspace Provisioning](/product/model-catalog/integrations#1-workspace-provisioning). Forking fails with an error if the required integration isn't provisioned.

If a prompt being shared uses partials that aren't shared yet, Portkey prompts you to share those partials in the same flow. See [Sharing prompts and partials](/product/prompt-engineering-studio/prompt-sharing) for the full flow.
