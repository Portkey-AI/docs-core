...
Use `x-portkey-sensitive-headers` when you want to forward a header to the upstream provider (or include it in an OTel export) but avoid persisting its raw value in Portkey logs.
