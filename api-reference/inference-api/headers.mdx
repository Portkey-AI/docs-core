...
<ResponseField name="x-portkey-use-retry-after-header / use_retry_after_header / useRetryAfterHeader" type="boolean">
Controls whether Portkey respects provider's retry wait time recommendations. When set to `true`, Portkey will use the `retry-after` or `retry-after-ms` headers from provider responses instead of applying the default exponential backoff for retries.

By default, this value is set to `false`, meaning Portkey will retry requests using its standard exponential backoff strategy regardless of any `retry-after` headers sent by the provider. Setting this to `true` is especially useful when working with providers that implement rate limiting with specific cool-down periods, as it ensures your application retries requests at the optimal time recommended by the provider.
</ResponseField>
...
curl https://api.portkey.ai/v1/chat/completions \
  -H "x-portkey-api-key: $PORTKEY_API_KEY" \
  -H "x-portkey-virtual-key: openai-virtual-key" \
  -H "x-portkey-use-retry-after-header: true" \
```
