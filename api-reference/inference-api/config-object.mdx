...
| `passthrough` | Defer provider resolution to the incoming request | boolean | No  | -   | When `true`, the target does not require `provider` or `virtual_key`. See [Passthrough targets](/product/ai-gateway/configs#passthrough-targets) |
...
The gateway resolves the provider from the request via `x-portkey-provider` header, `x-portkey-virtual-key` header, the `model` field (using `@slug/model-name` format), or a URL query parameter.
