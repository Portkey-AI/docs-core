...
description: "Connect Gmail's MCP server through Prisma AIRS AI Gateway MCP Gateway for reading, sending, and managing emails"
---

Use the Gmail MCP server to read emails, send messages, and manage labels from AI agents. AI Gateway MCP Gateway adds centralized auth, access control, and observability on top of Google's endpoint.
...
AI Gateway MCP Gateway provides centralized access control, observability, and credential management for Gmail's MCP server. Connect once, and all agents get managed Gmail access.

<Note>
Gmail requires creating a Google OAuth app and passing `client_id`, `client_secret`, `redirect_uri`, and `scope` in the AI Gateway **Advanced Configuration** — same pattern as other providers that need manual OAuth metadata (for example [GitHub](/aigw/integrations/mcp-servers/github-mcp-server#method-1-oauth-with-github-oauth-app) and [Slack](/aigw/integrations/mcp-servers/slack-mcp-server)).
</Note>
...
3. Enter a project name (e.g., `AIGW-Gmail-MCP`) and click **Create**. Ensure this project is active in the top dropdown.
...
3. Complete the 4-part configuration wizard:
   - **App Information:** Enter an app name (e.g., `AI Gateway Gmail MCP`) and select your user support email. Click **Next**.
...
4. Set a name (e.g., `AI Gateway Client`).
...
```json Claude Desktop / Cursor
{
  "mcpServers": {
    "gmail": {
      "url": "https://aigw.portkey.ai/m/gmail/mcp"
    }
  }
}
```
...
On first tool use, the client opens Google's authorization screen. The AI Gateway stores and refreshes tokens automatically.
...
<Note>
Add `"headers": { "Authorization": "Bearer YOUR_PORTKEY_API_KEY" }` when the workspace requires the AI Gateway API key on the MCP request — see [Integrations](/aigw/product/mcp-gateway/integrations). For self-hosted gateways, replace `aigw.portkey.ai/m` with the gateway host.
</Note>
