...
<Note>
Slack requires registering an OAuth app and passing `client_id`, `client_secret`, `redirect_uri`, and `scope` in Portkey **Advanced Configuration**—same pattern as other providers that need manual OAuth metadata (for example [GitHub](/aigw/integrations/mcp-servers/github-mcp-server#method-1-oauth-with-github-oauth-app)).
</Note>
...
<Note>
Add `"headers": { "x-portkey-api-key": "YOUR_PORTKEY_API_KEY" }` when the workspace requires the Portkey API key on the MCP request—see [Integrations](/aigw/product/mcp-gateway/integrations). For self-hosted gateways, replace `mcp.portkey.ai` with the gateway host.
