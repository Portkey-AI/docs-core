...
description: "Connect a Microsoft 365 MCP server (Outlook mail, calendar, Teams, OneDrive) through Prisma AIRS AI Gateway MCP Gateway with centralized auth, access control, and observability."
---

A Microsoft 365 MCP server (for example [`@softeria/ms-365-mcp-server`](https://github.com/softeria/ms-365-mcp-server)) exposes Microsoft Graph—mail, calendar, Teams, OneDrive—as MCP tools. AI Gateway MCP Gateway adds centralized authentication, per-user access control, and observability.
...
- Centralized governance via the AI Gateway access control and observability

## Authentication

- **Method:** OAuth 2.1.
- **Recommended:** let the MS365 MCP server handle Microsoft sign-in itself and **leave AI Gateway Advanced Configuration empty**. The server manages the Microsoft sign-in flow, and the AI Gateway stores and refreshes the resulting tokens per user.
...
```json Claude Desktop / Cursor
{
  "mcpServers": {
    "microsoft-365": {
      "url": "https://aigw.portkey.ai/m/microsoft-365/mcp"
    }
  }
}
```

On first tool use, the client opens Microsoft sign-in. The AI Gateway stores and refreshes tokens per user.
...
<Note>
For self-hosted gateways, replace `aigw.portkey.ai/m` with your gateway host.
</Note>
