...
<Note>
Use `oauth_metadata` (upstream auth), **not** `external_auth_config`. With `client_id` and `client_secret` present, Portkey skips DCR and uses your pre-registered Okta client. See [OAuth Client Metadata](/aigw/product/mcp-gateway/authentication/oauth-client-metadata).

The `oauth_metadata` wrapper above is for the **Advanced Configuration** JSON box. If you use the dedicated **OAuth Metadata** field under **Advanced Settings** instead, paste only the inner object — a doubled `oauth_metadata` key hides your `client_id` from Portkey and DCR resumes.
</Note>
