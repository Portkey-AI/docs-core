...
description: "Connect Snowflake's managed MCP server through Prisma AIRS AI Gateway MCP Gateway to query data, run Cortex Search/Analyst, and execute SQL—with centralized auth, access control, and observability."
---

Snowflake's managed MCP server exposes Cortex Search, Cortex Analyst, and SQL execution as MCP tools. AI Gateway MCP Gateway adds centralized authentication, per-user access control, and observability on top of the Snowflake endpoint.
...
- **Why:** Snowflake's OAuth server **does not support Dynamic Client Registration (DCR / RFC 7591)**. The AI Gateway's default OAuth flow auto-registers a client against the upstream server, which Snowflake rejects with:

  ```
  Incompatible auth server: does not support dynamic client registration
  ```

- **The supported pattern:** front Snowflake with an IdP it trusts (this guide uses **Okta** via Snowflake **External OAuth**), pre-register a confidential OAuth client there, and give the AI Gateway those credentials in `oauth_metadata`. The AI Gateway then skips DCR and uses the pre-registered client for the upstream token exchange.
...
## Connect via AI Gateway MCP Gateway

The setup has three parts: **Snowflake** (trust the IdP, map roles), **Okta** (issue tokens with the role scope), and **the AI Gateway** (register the server with `oauth_metadata`).
...
### Step 3: Register the integration in the AI Gateway

1. In the AI Gateway, go to **MCP Registry** → **Add MCP Integration**.
2. Set:
...
<Note>
Use `oauth_metadata` (upstream auth), **not** `external_auth_config`. With `client_id` and `client_secret` present, the AI Gateway skips DCR and uses your pre-registered Okta client. See [OAuth Client Metadata](/aigw/product/mcp-gateway/authentication/oauth-client-metadata).

The `oauth_metadata` wrapper above is for the **Advanced Configuration** JSON box. If you use the dedicated **OAuth Metadata** field under **Advanced Settings** instead, paste only the inner object — a doubled `oauth_metadata` key hides your `client_id` from the AI Gateway and DCR resumes.
</Note>
...
Gateway URL pattern: `https://aigw.portkey.ai/m/{slug}/mcp`.

```json Claude Desktop / Cursor
{
  "mcpServers": {
    "snowflake": {
      "url": "https://aigw.portkey.ai/m/snowflake/mcp"
    }
  }
}
```

On first tool use, the client opens the Okta authorization screen. After consent, the AI Gateway exchanges the code, calls Snowflake with the resulting JWT, and stores/refreshes tokens per user.
...
|---------|--------------|-----|
| `Incompatible auth server: does not support dynamic client registration` | `oauth_metadata` missing or incomplete → the AI Gateway fell back to DCR against Snowflake | Confirm the field is `oauth_metadata` with **both** `client_id` and `client_secret`, saved on the management plane you actually use |
...
For broader gateway issues, see [MCP Gateway: Common issues and resolutions](/aigw/help-center/mcp-gateway-troubleshooting).
...
<Note>
For self-hosted gateways, replace `aigw.portkey.ai/m` with your gateway host (and use that host in the Okta redirect URI and `redirect_uri`).
</Note>
