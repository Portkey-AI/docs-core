---
title: Cursor
description: Connect your MCP servers to Cursor IDE through Prisma AIRS AI Gateway's secure MCP gateway
---

## Overview

Cursor is an AI-powered IDE that supports MCP (Model Context Protocol) for enhanced code intelligence and tool integration. Through the AI Gateway's MCP Gateway, you can seamlessly connect any MCP server to Cursor without managing infrastructure or authentication.
...
We recommend configuring MCP servers at the **project level** for better organisation and team collaboration.
...
```json
{
  "mcpServers": {
    "your-server-name": {
      "url": "https://aigw.portkey.ai/m/:workspace-id/:server-id/mcp"
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "linear": {
      "url": "https://aigw.portkey.ai/m/ws-abc123/linear-def456/mcp"
    },
    "github": {
      "url": "https://aigw.portkey.ai/m/ws-abc123/github-ghi789/mcp"
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "general-tools": {
      "url": "https://aigw.portkey.ai/m/ws-abc123/tools-xyz789/mcp"
    }
  }
}
```
...
1. **First Connection**: When Cursor first connects to an authenticated MCP server, the AI Gateway will:
