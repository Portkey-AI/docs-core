...
Perplexity AI is a **native Responses API provider** on Portkey — requests are sent directly to Perplexity's Responses API endpoint. You can use the same [Responses API](/aigw/product/ai-gateway/responses-api) format with `@perplexity-ai` models. This is available on Portkey's AI Gateway (cloud and self-hosted).
