...
With Portkey, take advantage of features like fast AI gateway access, observability, prompt management, and more, while securely managing API keys through [Model Catalog](/aigw/product/model-catalog).
...
By default, Portkey routes requests to Databricks' [AI Gateway](https://docs.databricks.com/aws/en/machine-learning/foundation-model-apis/api-reference) endpoint (`/ai-gateway/...`) when no serving endpoint name is specified. This uses Databricks' pay-per-token Foundation Model APIs.
...
<Info>
**Pricing in Logs:** Databricks currently only supports [custom pricing](/aigw/product/model-catalog/custom-models). Cost data will only appear in your Portkey logs if you configure custom pricing for your Databricks models in the Model Catalog.
</Info>
