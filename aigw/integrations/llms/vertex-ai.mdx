...
With Portkey, you can take advantage of features like fast AI gateway access, observability, prompt management, and more, all while ensuring the secure management of your Vertex auth through [Model Catalog](/aigw/product/model-catalog).
...
<Note>
**Vertex Region** accepts regional locations (e.g. `us-central1`), multi-regional locations (`us`, `eu`), or `global`. Portkey automatically routes to the correct Google endpoint. See [Supported Regions](#supported-regions) for details.
</Note>
...
Route Google's native GenAI SDK through Portkey by pointing it at the Portkey gateway and supplying the Portkey provider headers. The SDK handles the request shape; Portkey handles auth, observability, and governance.

<Warning>
This flow currently supports **Gemini models only** on Vertex AI. Other Vertex AI models (Anthropic, Llama, Mistral, self-deployed, etc.) are not supported via the Google GenAI SDK — use the Portkey SDK or OpenAI-compatible endpoint for those.
</Warning>
...
Replace `<gateway-base-url>` with your Portkey gateway URL, `<api-key>` with your Portkey API key, and `<provider-id>` with the Vertex AI provider slug from your Model Catalog.
...
| Location type | Example values | Endpoint Portkey routes to |
| --- | --- | --- |
...
This applies uniformly across all Vertex AI flows in Portkey — Gemini, Anthropic, Llama, Mistral, self-deployed models, [context caching](#explicit-context-caching), and fine-tuned model resolution all honor multi-regional and global endpoints transparently.
