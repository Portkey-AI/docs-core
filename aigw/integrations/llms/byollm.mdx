...
Your private LLM must implement an API specification compatible with one of Portkey's [supported providers](/aigw/integrations/llms) (e.g., OpenAI's `/chat/completions`, Anthropic's `/messages`, etc.).
</Note>
...
<Info>
Portkey blocks requests to private and reserved IP ranges by default. If your private LLM runs on an internal network IP, see [Custom hosts](/aigw/product/ai-gateway/custom-hosts) for blocked patterns and how to allowlist specific hosts.
</Info>
