...
description: "Authenticate Prisma AIRS AI Gateway's Azure OpenAI and Azure AI Foundry integrations using API keys or Entra ID (service principal, managed identity, workload identity, or AWS federated)."
---

The AI Gateway supports four authentication modes for Azure OpenAI and Azure AI Foundry integrations. Pick the one that matches where the gateway runs and how your Azure tenant is configured.
...
| `AZURE_TENANT_ID` | Azure tenant ID. Can be overridden per-integration with `azureWorkloadTenantId`. |
...
```json
{
  ...
  "resourceName": "my-resource",
  "azureWorkloadClientId": "<CLIENT_ID>",
  "azureWorkloadTenantId": "<TENANT_ID>"
}
```

<Note>
Workload identity is supported on both Azure OpenAI and Azure AI Foundry integrations. The management-plane schema for `workload` mode (and `entraFederated` for Azure AI Foundry) requires **Backend `v1.16.0+`**.
</Note>
