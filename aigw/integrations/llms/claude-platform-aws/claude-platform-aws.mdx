...
description: "Integrate Claude models via Claude Platform on AWS with Prisma AIRS AI Gateway"
---
...
AI Gateway provider slug: `claude-platform-aws`
...
<Card title="Setup Assumed Role →" href="/aigw/integrations/llms/claude-platform-aws/setup-assumed-role">
...
Use Anthropic's native `/messages` endpoint through the AI Gateway with full gateway features.
...
<Note>
All AI Gateway features (caching, observability, configs) work with the `/messages` route. Add `x-portkey-config`, `x-portkey-provider`, and other AI Gateway headers as needed.
</Note>
...
<Note>
The AI Gateway handles all authentication and header translation automatically. From the AI Gateway SDK, the experience is identical to using first-party Anthropic -- just use a different provider slug.
</Note>
...
<CardGroup cols={2}>
  <Card title="Files API" icon="file" href="/aigw/integrations/llms/claude-platform-aws/files">
    Upload, list, retrieve, and delete files
  </Card>
  <Card title="Batch Processing" icon="layer-group" href="/aigw/integrations/llms/claude-platform-aws/batches">
    Run large async jobs at 50% lower cost
  </Card>
  <Card title="Assumed Role Setup" icon="shield-halved" href="/aigw/integrations/llms/claude-platform-aws/setup-assumed-role">
    Step-by-step IAM role and trust policy setup
  </Card>
  <Card title="Anthropic Integration" icon="link" href="/aigw/integrations/llms/anthropic">
...
<CardGroup cols={2}>
  ...
  <Card title="Observability" icon="chart-line" href="/aigw/product/observability/traces">
