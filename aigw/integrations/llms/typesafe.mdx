---
title: "TypeSafe (Jev)"
description: "Use TypeSafe's Jev models for typed judgments through the Prisma AIRS AI Gateway's /v1/decisions endpoint."
---

TypeSafe's Jev models return a typed judgment — a probability, a choice, or a score — instead of generated text. The AI Gateway routes these calls through `/v1/decisions`, a separate endpoint from chat completions, while still giving you the gateway's full feature set: authentication, routing, retries, circuit breaking, caching, logging, and cost tracking.

<Note>
  `/v1/decisions` does not support streaming. Only before-request [guardrails](/aigw/product/guardrails/decisions-guardrails) apply, scanning the `state` field only.
</Note>
