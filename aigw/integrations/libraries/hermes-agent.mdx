...
All reliability controls are configured through [Portkey Configs](/aigw/product/ai-gateway/configs) and attached to a scoped API key. Hermes sends the key as a Bearer token — Portkey applies the logic server-side, so Hermes config stays simple.
