...
description: "Add Prisma AIRS AI Gateway to any OpenAI-compatible app—just change 2 settings."
---
...
The AI Gateway works with any tool or application that supports OpenAI-compatible APIs. Add enterprise features—observability, reliability, cost controls—with just 2 configuration changes.

## Quick Start

Find your project's OpenAI settings and update:

1. **Base URL:** `https://aigw.portkey.ai/v1`
2. **API Key:** Your AI Gateway API key

That's it! Your app now routes through the AI Gateway.

<Frame caption="All requests appear in the AI Gateway logs">
...
<CardGroup cols={2}>
  ...
  <Card title="Multi-Provider Access" icon="shuffle">
    Switch between OpenAI, Anthropic, Google, and 3,000+ models without code changes.
  </Card>
...
If your app requires the OpenAI format (just model name), use an AI Gateway config with your default model.
...
1. Create a config in [Strata Cloud Manager](https://stratacloudmanager.paloaltonetworks.com/):
...
```bash
OPENAI_API_BASE=https://aigw.portkey.ai/v1
