...
const model = new ChatOpenAI({
    ...
    configuration: {
        baseURL: "https://aigw.portkey.ai/v1",
        defaultHeaders: {
            "x-portkey-config": "pc_your_config_id",  // Created in Portkey dashboard
        }
    },
    apiKey: "PORTKEY_API_KEY"
});
...
<Info>
The AI Gateway supports OpenAI embeddings via `OpenAIEmbeddings`. For other providers (Cohere, Voyage), use the **AI Gateway SDK directly** ([docs](/api-reference/inference-api/embeddings)).
</Info>
