...
**Solution:** Attach a default config to your Portkey API key. All requests with that key automatically use the config. For different configs per user/use case, create separate API keys. See [Default Configs](/aigw/product/administration/enforce-default-config).
</Warning>
...
The pipe uses Open WebUI's `__user__` context object and formats it as [Portkey metadata](/aigw/product/observability/metadata).
