...
Add trace IDs to group and debug requests in the Portkey Dashboard. Create a [Portkey Config](/aigw/product/ai-gateway/configs) with metadata and attach it to a scoped API key:
...
All advanced routing is configured through [Portkey Configs](/aigw/product/ai-gateway/configs), then attached to a scoped API key used as `apiKey` in `models.json`. Pi passes the key as a Bearer token — Portkey applies the routing logic server-side.
