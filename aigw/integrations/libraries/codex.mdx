...
Codex uses the `model` value in `config.toml` to decide which model to call. With Portkey, set `model` to a [Model Catalog](/aigw/product/model-catalog) slug in the form `@<provider-slug>/<model-name>`. Change providers or models by updating `model` in the config.
...
<Note>
**Fallbacks, load balancing, or caching?** Create a [Portkey Config](/aigw/product/ai-gateway/configs), attach it to your API key, and set `model` to the config’s virtual model. See [Enterprise Governance](#3-enterprise-governance) for examples.
</Note>
