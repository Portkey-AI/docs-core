...
description: "Roll out Claude Desktop with Prisma AIRS AI Gateway across your org with provider routing, metadata-driven governance, scoped API keys, and usage observability."
---
...
- You see every request, every cost, every policy decision in the AI Gateway
...
Providers are the upstream LLM accounts AI Gateway calls on your behalf. Connect every model source your org uses so you can route between them later.

Go to [Model Catalog](https://stratacloudmanager.paloaltonetworks.com/) → **Add Provider**.

<Frame>
  <img src="/images/product/model-catalog/create-provider-page.png" width="500" alt="The Add Provider page in the Model Catalog" />
</Frame>
...
<AccordionGroup>
  <Accordion title="Add fallbacks for reliability" icon="life-ring">
    ...
    See [Fallbacks](/aigw/product/ai-gateway/fallbacks) for details.
  </Accordion>
  ...
  <Accordion title="Add load balancing" icon="scale-balanced">
    ...
    See [Load Balancing](/aigw/product/ai-gateway/load-balancing) for details.
  </Accordion>
  ...
  <Accordion title="Enable caching" icon="database">
    ...
    See [Caching](/aigw/product/ai-gateway/cache-simple-and-semantic) for details.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={3}>
  <Card title="Budget Limits" icon="coins" href="/aigw/product/policies/budget-limits">
    Cap spend by team, environment, or individual user.
  </Card>
  <Card title="Rate Limits" icon="gauge-high" href="/aigw/product/policies/rate-limits">
...
Go to [API Keys](https://stratacloudmanager.paloaltonetworks.com/) and create one key per team or environment.
...
Metadata is how the AI Gateway knows *who* is making each request. Claude Desktop sends it as a JSON header (`x-portkey-metadata`) on every call. The AI Gateway reads those fields and uses them for attribution, filtering, and routing decisions.
...
|---|---|---|
| `tenant` | Organisation or sub-org | `acme` |
...
### What the AI Gateway does with these fields

Once your schema is set, every request appears in the AI Gateway tagged with these values. Here's what they unlock:
...
| Setting | Value |
|---|---|
| Gateway base URL | `https://aigw.portkey.ai` (or your self-hosted gateway URL) |
...
Then point them to the [Developer Guide](/aigw/integrations/libraries/claude-desktop-developers).

## 6. Monitor and iterate

The work isn't done at rollout. Use the AI Gateway to track outcomes and tune the setup based on real usage.

<CardGroup cols={3}>
  <Card title="Logs" icon="list" href="https://stratacloudmanager.paloaltonetworks.com/">
    Verify routing decisions and policy triggers per request.
  </Card>
  <Card title="Analytics" icon="chart-line" href="https://stratacloudmanager.paloaltonetworks.com/">
...
<Steps>
  <Step title="Requests appear in Logs with correct metadata">
    Send a test request from Claude Desktop. Confirm it shows up in [AI Gateway Logs](https://stratacloudmanager.paloaltonetworks.com/) with all the metadata fields you defined in step 5.
  </Step>
  <Step title="Policies trigger correctly">
    Test each policy you configured: budget block, rate limit, guardrail. Confirm the AI Gateway enforces them as expected.
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="What happens if a developer changes their metadata header?">
    They can. Metadata is a hint that AI Gateway trusts. If you need authoritative attribution, scope each developer's API key narrowly so the key itself enforces team and environment.
  </Accordion>
  ...
  <Accordion title="How do I revoke access for a specific user?">
    Revoke their workspace membership in the AI Gateway. Their API key stops working immediately. If multiple users share a key, rotate the key and re-issue to the remaining users.
  </Accordion>

  <Accordion title="Can I see what an individual user is asking Claude?">
    Logs show the prompt content unless you've configured request redaction. For privacy-sensitive deployments, enable [PII redaction guardrails](/aigw/product/guardrails/pii-redaction) before rollout.
  </Accordion>
</AccordionGroup>
