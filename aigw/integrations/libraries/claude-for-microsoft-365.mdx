...
description: "Deploy the Claude add-in for Excel, Word, PowerPoint, and Outlook through Prisma AIRS AI Gateway with governed model routing, scoped API keys, and full observability."
---

The Claude add-in for Microsoft 365 puts Claude in a task pane inside Excel, Word, PowerPoint, and Outlook. Point it at the AI Gateway instead of Anthropic's API and every request from every user routes through one governed gateway.
...
- Every task-pane conversation routes through the AI Gateway
...
Model traffic goes from the user's Office WebView straight to the AI Gateway. Allow `pivot.claude.ai` and your gateway host through the corporate firewall.

## Prerequisites

| Requirement | Notes |
|---|---|
| AI Gateway API key with a config attached | Steps 1–3 |
...
Connect the upstream model source AI Gateway calls on your behalf.

Go to [Model Catalog](https://stratacloudmanager.paloaltonetworks.com/) → **Add Provider**.

<CardGroup cols={3}>
  <Card title="Anthropic" icon="bolt" href="/aigw/integrations/llms/anthropic">
    Direct API access
  </Card>
  <Card title="AWS Bedrock" icon="cloud" href="/aigw/integrations/llms/bedrock/aws-bedrock">
    Cross-region inference
  </Card>
  <Card title="Vertex AI" icon="cloud" href="/aigw/integrations/llms/vertex-ai">
...
<AccordionGroup>
  <Accordion title="Add fallbacks for reliability" icon="life-ring">
    ...
    See [Fallbacks](/aigw/product/ai-gateway/fallbacks).
  </Accordion>
  ...
  <Accordion title="Enable caching" icon="database">
    ...
    See [Caching](/aigw/product/ai-gateway/cache-simple-and-semantic).
  </Accordion>
  ...
  <Accordion title="Route by team with conditional routing" icon="route">
    ...
    See [Conditional Routing](/aigw/product/ai-gateway/conditional-routing).
  </Accordion>
</AccordionGroup>

## 3. Create the API key

Go to [API Keys](https://stratacloudmanager.paloaltonetworks.com/) → create one key per team or environment.
...
<Steps>
  ...
  <Step title="Attach budget and rate limits">
    See [Budget Limits](/aigw/product/model-catalog/budget-limits) and [Rate Limits](/aigw/product/model-catalog/rate-limits).
  </Step>
...
<Note>
  **Use `authorization`, not `x-api-key`.** The add-in supports only those two header schemes, and the AI Gateway rejects its API key on `x-api-key` with `Invalid API Key. Error Code: 03`. The AI Gateway's native `x-portkey-api-key` works with curl but the add-in cannot send it.
</Note>
...
|---|---|
| 1. Connection path | Gateway vs. Vertex/Bedrock/Foundry. **Answer gateway** — the AI Gateway is the gateway, even though it routes to Bedrock or Vertex underneath |
...
node "$PLUGIN/scripts/build-manifest.mjs" office manifest.xml \
  gateway_url=https://aigw.portkey.ai \
...
| Key | Value for the AI Gateway |
|---|---|
| `gateway_url` | `https://aigw.portkey.ai`, or your self-hosted gateway URL |
| `gateway_token` | The API key from step 3 |
| `gateway_auth_header` | `authorization` — the `x-api-key` default does not work |
| `gateway_api_format` | `anthropic` — the AI Gateway exposes the `/v1/messages` route |

<Tip>
  Single-quote the token in shell. AI Gateway keys contain `/` and `+`; the builder percent-encodes them correctly, but an unquoted shell mangles them first.
</Tip>
...
Fully quit and reopen Excel — a backgrounded app won't rescan. The add-in appears under **Insert → My Add-ins**. Send a message and confirm the request lands in [Logs](https://stratacloudmanager.paloaltonetworks.com/).

Remove a sideload with `clear-addin-cache.sh --id <GUID> --apply`. It's dry-run by default and only ever touches that one ID.

### Deploy the add-in for your organisation
...
<Steps>
  ...
  <Step title="Assign users">
    Start with **Just me** or a pilot group. Widen to **Entire organisation** once verified — assignment changes don't require redeployment.
...
Rotating the AI Gateway key, changing the model list, or adding any config key means regenerating and re-uploading. Run the four commands in this order:
...
Add `inference_headers` to the manifest to tag every request with metadata the AI Gateway uses for filtering, cost attribution, and conditional routing:
...
<AccordionGroup>
  ...
  <Accordion title="mcp_servers — attach in-network tools" icon="plug">
    ...
    See [MCP Gateway](/aigw/product/mcp-gateway).
  </Accordion>
  ...
  <Accordion title="otlp_endpoint — send traces to your collector" icon="chart-line">
    ...
    The AI Gateway also exports traces natively. See [OpenTelemetry](/aigw/product/observability/opentelemetry).
  </Accordion>
</AccordionGroup>
...
Mitigate at the AI Gateway layer rather than trying to hide the key:
...
<Steps>
  <Step title="Requests appear in Logs">
    Send a test message from the task pane. Confirm it shows in [Logs](https://stratacloudmanager.paloaltonetworks.com/) with the expected metadata.
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="401 or 403 from the gateway">
    Wrong header scheme. The add-in must use `gateway_auth_header=authorization`; The AI Gateway rejects its key on `x-api-key`. Re-run the step 4 probe.
  </Accordion>
...
<CardGroup cols={3}>
  <Card title="Claude Desktop" icon="desktop" href="/aigw/integrations/libraries/claude-desktop-platform-admins">
    Roll out Claude Desktop across your org
  </Card>
  <Card title="Claude Code" icon="terminal" href="/aigw/integrations/libraries/claude-code">
    Route Claude Code through the AI Gateway
  </Card>
