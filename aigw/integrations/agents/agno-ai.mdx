---
title: "Agno AI"
description: "Use Prisma AIRS AI Gateway with Agno to build production-ready autonomous AI agents"
---

## Introduction

Agno is a powerful framework for building autonomous AI agents that can reason, use tools, maintain memory, and access knowledge bases. The AI Gateway enhances Agno agents with enterprise-grade capabilities for production deployments.

The AI Gateway transforms your Agno agents into production-ready systems by providing:
...
- **Advanced guardrails** for safe and compliant agent behaviour
...
1. Go to [Model Catalog](https://stratacloudmanager.paloaltonetworks.com/) → **AI Providers** tab
...
Sign up at [stratacloudmanager.paloaltonetworks.com](https://stratacloudmanager.paloaltonetworks.com/) to get your API key.

Once you are on the AI Gateway follow this guide to create your first AI provider and integration that will help us connect Agno to any LLM provider.

</Step>

<Step title="Configure the AI Gateway with Agno">
...
# Configure the AI Gateway as the model provider
portkey_model = OpenAILike(
    id="@opeani-provider-slug/gpt-4o",  # You can use any model available on the AI Gateway
    api_key="YOUR_PORTKEY_API_KEY",
    base_url="https://aigw.portkey.ai/v1",
)

# Create an Agno agent with the AI Gateway
agent = Agent(
    model=portkey_model,
...
# Configure the AI Gateway model with fallback config
portkey_model = OpenAILike(
...
# Prefer creating the Config in Strata Cloud Manager and passing the config ID in the client
config = {
...
Track individual users through your Agno agents using the AI Gateway's metadata system.
...
# Configure the AI Gateway with fallbacks and retries
reliability_config = {
    "strategy": {
        "mode": "fallback"
    },
...
portkey_model = OpenAILike(
    ...
    api_key="YOUR_PORTKEY_API_KEY",
    base_url="https://aigw.portkey.ai/v1",
    default_headers={"x-portkey-config": json.dumps(reliability_config), "x-portkey-trace-id": "advanced_trading_agent"}
)
...
The AI Gateway allows you to use 3,000+ LLMs with your Agno AI agents setup, with minimal configuration required. Let's set up the core components in the AI Gateway that you'll need for integration.
...
Create your config on the [Configs page](https://stratacloudmanager.paloaltonetworks.com/) in Strata Cloud Manager. You'll need the config ID for connecting to Cline's setup.
...
<AccordionGroup>
<Accordion title="How does the AI Gateway enhance my Agno agents?">
...
<Accordion title="Can I use any LLM with Agno through the AI Gateway?">
...
<Accordion title="Does Palo Alto Networks support all Agno features?">
