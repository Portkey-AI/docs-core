...
## Using Request Parameters Check in Prisma AIRS AI Gateway
...
<Note>
Guardrail Actions let you orchestrate your guardrail's behaviour (deny, feedback, etc.). Learn more about them [here](/aigw/product/guardrails#there-are-6-types-of-guardrail-actions).
</Note>
...
```sh
curl https://aigw.portkey.ai/v1/guardrails \
  -X POST \
...
* When you save the Guardrail, you'll get an associated Guardrail ID - add this ID to the `input_guardrails` param in your AI Gateway Config
* Create the Config in Strata Cloud Manager, save it, and attach its Config ID to your requests. [More here](/aigw/product/ai-gateway/configs).
...
client = OpenAI(
    ...
    default_headers={"x-portkey-provider": "openai", "x-portkey-config": "CONFIG_ID"}
)
```
  </Tab>
</Tabs>

Your requests are now guarded by the Request Parameters Check, and you can see the verdict and any action taken directly in your AI Gateway logs.
