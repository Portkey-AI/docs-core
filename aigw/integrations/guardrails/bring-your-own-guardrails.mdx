...
description: "Integrate your custom guardrails with Prisma AIRS AI Gateway using webhooks"
---

The AI Gateway's webhook guardrails allow you to integrate your existing guardrail infrastructure with our AI Gateway. This is perfect for teams that have already built custom guardrail pipelines (like PII redaction, sensitive content filtering, or data validation) and want to:
...
1. You add a Webhook as a Guardrail Check in the AI Gateway
2. When a request passes through the AI Gateway:
   - The AI Gateway sends relevant data to your webhook endpoint
   - Your webhook evaluates the request/response and returns a verdict
   - Based on your webhook's response, the AI Gateway either allows the request to proceed, modifies it if required, or applies your configured guardrail actions

## Setting Up a Webhook Guardrail

### Configure Your Webhook in Strata Cloud Manager
...
| **Execute on Proxy** | Also run this webhook on proxy (`/v1/*`) requests | `boolean` |
...
The maximum time the AI Gateway will wait for your webhook to respond before proceeding with a default `verdict: true`.
...
| Custom Headers | Any headers you configured in Strata Cloud Manager |
| Forwarded Headers | Client request headers configured via `parameters.forwardHeaders` on the check |

#### Request Body

The AI Gateway sends comprehensive information about the AI request to your webhook:
...
<Expandable title="Webhook Request Structure">
  ...
  <ParamField body="provider" type="string">AI Gateway provider slug. Example: `openai`, `azure-openai`, etc.</ParamField>
  ...
  <ResponseField name="transformedData" type="object">
    ...
    <Expandable title="Properties">
      <ResponseField name="request" type="object">
        ...
        If this field is found in the Webhook response, the AI Gateway will fully override the existing request body with the returned data.
      </ResponseField>
      <ResponseField name="response" type="object">
        Modified response data (only for afterRequestHook)
        If this field is found in the Webhook response, the AI Gateway will fully override the existing response body with the returned data.
      </ResponseField>
    </Expandable>
  </ResponseField>
</Expandable>
...
You can include additional context with each request using the AI Gateway's metadata feature:

```json
// In your API request to the AI Gateway
"x-portkey-metadata": {"user": "john", "context": "customer_support"}
```

This metadata will be forwarded to your webhook in the `metadata` field. [Learn more about metadata](/aigw/product/observability/metadata).
...
<Card title="Forwarding Headers" href="/aigw/product/guardrails/forwarding-headers" />

## Running on Proxy Requests

By default, webhook guardrails only run on managed `/v1/chat/completions`-style requests where the AI Gateway parses the full request/response body. Enable `executeOnProxy` to also run your webhook on raw proxy (`/v1/*`) pass-through requests.
...
When triggered on a proxy request, the webhook payload includes `method`, `path`, and `content-type` metadata instead of a fully parsed request body. This is useful for blocking or auditing traffic that bypasses the standard chat completions route.
...
Building custom webhooks? Reach out to [Portkey support](https://support.portkey.ai/forms/customer-portal-ticket-form) for support and to share your implementation experiences!
