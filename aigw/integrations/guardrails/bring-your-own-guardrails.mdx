...
By default, webhook guardrails only run on managed `/v1/chat/completions`-style requests where Portkey parses the full request/response body. Enable `executeOnProxy` to also run your webhook on raw proxy (`/v1/*`) pass-through requests.
