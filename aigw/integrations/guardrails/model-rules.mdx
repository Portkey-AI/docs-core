...
Model Rules is a built-in Prisma AIRS AI Gateway guardrail that checks the requested model before the request is sent upstream. It resolves an allowed model list from your configured rules, then either allows or blocks the request based on whether the requested model matches.

With metadata-based mappings, you can dynamically control which models are available based on request context — such as customer tier, team, or workspace. This makes Model Rules especially useful for enforcing model access policies across your organisation.

<Info>
  You can apply Model Rules at the **org level** by attaching it as an [Org Guardrail](/aigw/product/guardrails), giving you complete coverage across all workspaces and API keys.
</Info>
...
## Using Model Rules with the AI Gateway
...
You can learn more about actions in the [Guardrails documentation](/aigw/product/guardrails#there-are-7-types-of-guardrail-actions).

### 3. Attach the guardrail

Once the guardrail is saved, you'll get a Guardrail ID. You can apply it in any of these ways:

- Add it to `input_guardrails` in a [AI Gateway Config](/aigw/product/ai-gateway/configs)
- Attach it to a [Workspace](/aigw/product/enterprise-offering/org-management/workspaces) so it applies to all requests in that workspace
- Attach it as an [Org Guardrail](/aigw/product/guardrails) so it applies across the entire organisation
