...
## Using Prisma AIRS with Prisma AIRS AI Gateway

### 1. Set Up Prisma AIRS

Before integrating with the AI Gateway:
...
* Note your Profile Name or Profile ID if you want the guardrail to target a specific profile

### 2. Add Prisma AIRS Credentials to the AI Gateway
...
* Configure the guardrail parameters (all optional):
  - **Profile Name**: The security profile name from your Prisma AIRS configuration. Leave empty to use the profile linked to your API key in Strata Cloud Manager
  - **Profile ID**: The security profile ID. Can be used instead of, or alongside, Profile Name
  - **AI Model**: The AI model identifier, used for tracking and reporting
  - **Application User**: User identifier for audit logs and user-level analytics
  - **Application Name**: Custom application name for tracking, prefixed with `Portkey-`
  - **Scan Scope**: Controls which messages are scanned (defaults to `last_message`)
  - **Strip Scaffolding**: Removes known agent-harness wrappers before scanning (defaults to `false`)
...
|------------|-------------|------------|-----------------|
| PANW Prisma AIRS Guardrail | Blocks prompt/response when Palo Alto Networks Prisma AI Runtime Security returns action=block | `Profile Name` (string), `Profile ID` (string), `AI Model` (string), `Application User` (string), `Application Name` (string), `Scan Scope` (enum), `Strip Scaffolding` (boolean) | `beforeRequestHook`, `afterRequestHook` |

### Parameter Details

All parameters are optional. If neither `Profile Name` nor `Profile ID` is set, Prisma AIRS uses the profile linked to your API key in Strata Cloud Manager.

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `Profile Name` | string | — | Security profile name from Prisma AIRS |
| `Profile ID` | string | — | Security profile ID, usable instead of or alongside `Profile Name` |
| `AI Model` | string | `unknown-model` | AI model identifier, used for tracking and reporting |
| `Application User` | string | `portkey-gateway` | User identifier for audit logs and user-level analytics |
| `Application Name` | string | — | Custom application name for tracking, prefixed with `Portkey-` (e.g. `Portkey-chatbot`) |
...
Your requests are now protected by Prisma AIRS's comprehensive AI security capabilities, and you can see the verdict and any actions taken directly in your AI Gateway logs!
...
2. **Prompt Injections**: Detects and blocks attempts to manipulate AI behaviour through malicious prompts
...
3. **Monitoring**: Regularly review both AI Gateway logs and Prisma AIRS dashboard for security insights
