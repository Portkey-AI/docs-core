...
<Note>
  The AI Gateway is also available on the Azure Marketplace. You can deploy the AI Gateway directly through your Azure console, which streamlines procurement and deployment processes.
...
| Cache (Prompts, Configs & Providers)    | Built-in Redis or Azure Managed Redis           | Deployed within the same VNet as the AI Gateway.                                                                                                 |
...
* (Optional) If using Workload Identity for Blob Storage access, ensure [OIDC issuer](https://learn.microsoft.com/en-us/azure/aks/use-oidc-issuer) and [Workload Identity](https://learn.microsoft.com/en-us/azure/aks/workload-identity-deploy-cluster#update-an-existing-aks-cluster) are enabled on your AKS cluster.
...
- Go to the [the AI Gateway](https://stratacloudmanager.paloaltonetworks.com/) website.
...
To enable the gateway to work with an Azure Managed cache, ensure connectivity from AKS cluster. [Set up](#azure-managed-redis) access to the Redis instance based on your chosen authentication method and update `values.yaml` with the following configuration.
  ...
  <Tab title="Managed Identity">
    ...
    environment:
      data:
        ...
        AZURE_REDIS_MANAGED_CLIENT_ID: <MANAGED_CLIENT_ID>         ## Defaults to AZURE_MANAGED_CLIENT_ID if not provided
    ```
  </Tab>
  <Tab title="Entra ID">
    ```yaml
    environment:
      data:
        ...
        AZURE_REDIS_ENTRA_CLIENT_ID: <ENTRA_CLIENT_ID>             ## Defaults to AZURE_ENTRA_CLIENT_ID if not provided
        AZURE_REDIS_ENTRA_CLIENT_SECRET: <ENTRA_CLIENT_SECRET>     ## Defaults to AZURE_ENTRA_CLIENT_SECRET if not provided
        AZURE_REDIS_ENTRA_TENANT_ID: <ENTRA_TENANT_ID>             ## Defaults to AZURE_ENTRA_TENANT_ID if not provided
    ```
  </Tab>
</Tabs>
...
2. [Set up](#setting-up-permission) access to the log store. The Gateway supports the following methods for connecting to the Blob Storage:
    ...
    <Tabs>
      <Tab title="Workload Identity">
        To set up Workload Identity authentication for the AI Gateway, follow the [steps](#setting-up-permission) and add the following configuration to `values.yaml`.
        ```yaml
        ## To enable Workload Identity update values.yaml with the following details:-
        serviceAccount:
          create: true
          automount: true
          name: <SERVICE_ACCOUNT_NAME>
...
3. (Optional) Configure log path format using `LOG_STORE_FILE_PATH_FORMAT`. See [Log Object Path Format](/aigw/product/enterprise-offering/components#log-object-path-format) for details.
...
- Test Gateway by sending a cURL request.
    ...
    2. Once port forwarding is active, open a new terminal window or tab and send a test request by running:
      ...
      PORTKEY_API_KEY=<PORTKEY_API_KEY>                         # Replace <PORTKEY_API_KEY> with Gateway API Key which can be created from Strata Cloud Manager(https://stratacloudmanager.paloaltonetworks.com/).
...
The AI Gateway supports the following methods for integrating the Data Plane with the Management Plane for outbound connectivity:
...
#### Azure Private Link
Establishes a secure, private connection between the Management Plane and Data Plane within the Azure network.

**Steps to establish Azure Private Link connectivity:**
1. Contact the AI Gateway and provide your Azure Subscription Id so it can be whitelisted in the AI Gateway's Management Plane.
2. Once you get confirmation from the AI Gateway that your Azure account is whitelisted, create Azure Private Endpoint connection to Management Plane.
  ```sh
...
3. Retrieve Private IP of created endpoint.
  ...
  ```
4. Contact the Palo Alto Networks team and share the endpoint name with them.
5. Once the connection request is approved, create Private Hosted Zone and link it to Gateway VNet.
  ...
  ```
6. Create a record in the PHZ pointing to the AI Gateway's Private Endpoint IP.
  ```sh
...
6. Under **Access security**, select **Restricted by subscription** and whitelist the Management Plane subscription ID (`
1645c6d9-d399-4c91-a4b9-2673952ae803`).

7. Leave all other settings as default and click **Create** to provision the Private Link Service.

8. Contact the Palo Alto Networks team and share the **alias** of the created Private Link Service, along with the **Gateway URL**.

9. Once the Palo Alto Networks team has raised the connection request from their end, approve it by navigating to the PLS in the Azure Portal, then go to **Private Endpoint connections** and click **Approve**.

#### IP Whitelisting
Allows management plane to access the Data Plane over the internet by restricting inbound traffic to specific IP address of Management Plane. This method requires the Data Plane to have a publicly accessible endpoint.
To whitelist, add an inbound rule to the Azure NSG or Firewall allowing connections from the AI Gateway Management Plane's IPs (`54.81.226.149`, `34.200.113.35`, `44.221.117.129`) on required port.
...
- Go to [the AI Gateway website](https://stratacloudmanager.paloaltonetworks.com/) -> **Logs**.
...
#### Azure Blob Storage
To allow the AI Gateway to access Azure Blob Storage for log storage, permissions must be granted.

Follow the steps below to set up these permissions according to your selected access method.
  <Tabs>
    <Tab title="Workload Identity">
    **Prerequisites:** Ensure [OIDC issuer](https://learn.microsoft.com/en-us/azure/aks/use-oidc-issuer) and [Workload Identity](https://learn.microsoft.com/en-us/azure/aks/workload-identity-deploy-cluster#update-an-existing-aks-cluster) are enabled on your AKS cluster.

    1. Specify the details:
      ```sh
      CLUSTER_NAME=<CLUSTER_NAME>                             # Specify name of AKS cluster
      RESOURCE_GROUP=<RESOURCE_GROUP>                         # Specify the name of the resource group
      ...
      MANAGED_IDENTITY_NAME=<MANAGED_IDENTITY_NAME>           # Specify a name for the User-Assigned Managed Identity (e.g., portkey-gateway-identity)
      SERVICE_ACCOUNT_NAME=<SERVICE_ACCOUNT_NAME>             # Must match the service account name used in values.yaml
      NAMESPACE=<NAMESPACE>                                   # Namespace where the gateway is deployed
      ```
    ...
    3. Create a federated identity credential to link the Kubernetes service account to the managed identity.
      ```sh
    ...
    4. Grant the managed identity a role that allows it to access Blob Storage.
      ```sh
    ...
    3. [Assign the managed identity to the AKS cluster VMSS](https://learn.microsoft.com/en-us/azure/active-directory/managed-identities-azure-resources/qs-configure-cli-windows-vmss#user-assigned-managed-identity) so pods can use it.
      ```sh
...
**Prerequisites:** [Microsoft Entra Authentication](https://learn.microsoft.com/en-us/azure/azure-cache-for-redis/cache-azure-active-directory-for-authentication) must be enabled on the Azure Managed Redis instance.

Follow the steps below to set up these permissions according to your selected access method.
  <Tabs>
    <Tab title="Workload Identity">
    Complete steps 1 to 3 from the [Azure Blob Storage > Workload Identity](#azure-blob-storage) section if not already done.

    1. Assign a data access policy to the managed identity on the Redis instance.
      ```sh
      REDIS_NAME=<REDIS_NAME>                                   # Name of the Azure Managed Redis instance
      RESOURCE_GROUP=<RESOURCE_GROUP>
      MANAGED_IDENTITY_NAME=<MANAGED_IDENTITY_NAME>             # Same identity created in Blob Storage setup
    ...
    <Tab title="Managed Identity">
    Complete steps 1 to 3 from the [Azure Blob Storage > Managed Identity](#azure-blob-storage) section if not already done.
    ...
    <Tab title="Entra Identity">
    Complete steps 1 to 3 from the [Azure Blob Storage > Entra Identity](#azure-blob-storage) section if not already done.

    1. Assign a data access policy to the service principal on the Redis instance.
      ```sh
      REDIS_NAME=<REDIS_NAME>                                   # Name of the Azure Managed Redis instance
      RESOURCE_GROUP=<RESOURCE_GROUP>
...
<Tabs>
  ...
  # Replace <RESOURCE_ID> with the full Resource ID of Azure OpenAI or Azure AI Foundry resource.
  # e.g., /subscriptions/<SUB_ID>/resourceGroups/<RG>/providers/Microsoft.CognitiveServices/accounts/<RESOURCE_NAME>
  ...
  <Tab title="Managed Identity">
  Create a User-Assigned Managed Identity if not already done, and [assign it to the AKS cluster or VMSS](https://learn.microsoft.com/en-us/azure/aks/use-managed-identity#bring-your-own-managed-identity).
...
The following sample `values.yaml` shows how to configure Azure Managed Redis (Workload Identity) and Azure Blob Storage (Workload Identity) using a shared User-Assigned Managed Identity, with Private Link for outbound connectivity to the Management Plane.
```yaml
images:
  gatewayImage:
    repository: "docker.io/portkeyai/gateway_enterprise"
...
environment:
  ...
  data:
    ...
    # Private Link outbound connectivity to Control Plane
...
**Azure Managed Redis (Access Key) with Managed Identity for Blob Storage**

The following sample `values.yaml` shows how to configure Azure Managed Redis with an access key and Azure Blob Storage using the AKS Managed Identity.
```yaml
images:
  gatewayImage:
    repository: "docker.io/portkeyai/gateway_enterprise"
...
environment:
  ...
  data:
    ...
    # Azure Managed Redis with Access Key
    CACHE_STORE: azure-redis
    REDIS_URL: "redis://<Azure_Redis_Endpoint>:<Port>"
    REDIS_TLS_ENABLED: "true"
...
**AI Gateway + MCP Gateway with NGINX Ingress**

The following sample `values.yaml` shows how to deploy both AI Gateway and MCP Gateway with host-based routing using the NGINX Ingress controller, built-in Redis, and Workload Identity for Blob Storage.
```yaml
images:
  gatewayImage:
    repository: "docker.io/portkeyai/gateway_enterprise"
