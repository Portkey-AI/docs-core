...
To whitelist, add an inbound rule to the Azure NSG or Firewall allowing connections from the AI Gateway Management Plane's IPs on required port. Contact the Palo Alto Networks team for the current address list.
...
The following sample `values.yaml` shows how to configure Azure Managed Redis (Workload Identity) and Azure Blob Storage (Workload Identity) using a shared User-Assigned Managed Identity.
```yaml
images:
  gatewayImage:
    repository: "docker.io/portkeyai/gateway_enterprise"
