...
| Cache (Prompts, Configs & Providers)    | Built-in Redis, Amazon ElastiCache for Redis OSS or Valkey                | Deployed within the same VPC as the AI Gateway.                                                                                                                  |
...
<Tabs>
  ...
  <Tab title="IRSA">
    ...
    ```
    **Note:** The IAM role must have `elasticache:Connect` permission. See [ElastiCache IAM authentication](https://docs.aws.amazon.com/AmazonElastiCache/latest/dg/auth-iam.html) for details.
  </Tab>
  <Tab title="EKS Pod Identity">
    ...
    environment:
      data:
        ...
        REDIS_USERNAME: <ELASTICACHE_USER_ID>                 ## ElastiCache user ID configured for IAM authentication
    ```
    **Note:** The IAM role must have `elasticache:Connect` permission and a Pod Identity association must be created. See [ElastiCache IAM authentication](https://docs.aws.amazon.com/AmazonElastiCache/latest/dg/auth-iam.html) and [EKS Pod Identity](https://docs.aws.amazon.com/eks/latest/userguide/pod-id-agent-setup.html) for details.
  </Tab>
</Tabs>
...
The Data Service is a component of the AI Gateway deployment responsible for batch processing, fine-tuning, and log exports.
...
ingress:
  ...
  annotations:
    ...
    alb.ingress.kubernetes.io/inbound-cidrs: <X.X.X.X/Y>                                                # Allowed inbound CIDR ranges
...
#### AWS PrivateLink
Establishes a secure, private connection between the Management Plane and Data Plane within the AWS network.

**Steps to establish AWS PrivateLink connectivity:**

1. Contact the AI Gateway and provide AWS account ARN so it can be whitelisted in the AI Gateway's Management Plane.
2. Once you get confirmation from the AI Gateway that your AWS account is whitelisted, go to the [VPC Console](https://console.aws.amazon.com/vpc/).
3. Select the AWS Region where the AI Gateway is deployed.
...
- In the top-right corner of the AWS Console, select the region where the AI Gateway is deployed.
...
**Authorize the AI Gateway's Management Plane to initiate connection requests**

- Open to Endpoint Service > click on **Actions** > select **Allow principals**, and enter the Management Plane's ARN(`arn:aws:iam::299329113195:root`).
...
- Wait for the Palo Alto Networks team to initiate a connection request from the management plane's AWS account to your Gateway AWS account. Navigate to the **Endpoint connections** section and once the request appears, approve it.
...
To whitelist, add an inbound rule to the Load Balancer's security group allowing connections from the AI Gateway Management Plane's IPs (`54.81.226.149`, `34.200.113.35`, `44.221.117.129`) on NLB listener port.
...
<Tabs>
  ...
  2. Create a trust policy and IAM role.
    ```sh
  ...
  <Tab title="EKS Pod Identity">
  **Prerequisites:** If the EKS Pod Identity Agent is not already installed on your cluster, install it before proceeding. For detailed instructions, refer to the AWS [documentation](https://docs.aws.amazon.com/eks/latest/userguide/pod-id-agent-setup.html).
...
**ElastiCache (IAM Auth) with S3 using IRSA and PrivateLink**

The following sample `values.yaml` shows how to configure Amazon ElastiCache with IAM authentication, Amazon S3 for log storage using IRSA, and AWS PrivateLink for outbound connectivity to the Management Plane.
```yaml
images:
  gatewayImage:
    repository: "docker.io/portkeyai/gateway_enterprise"
...
**ElastiCache (Auth Token) with S3 using EKS Pod Identity**

The following sample `values.yaml` shows how to configure Amazon ElastiCache with auth token, Amazon S3 for log storage using EKS Pod Identity, and an Application Load Balancer.
```yaml
images:
  gatewayImage:
    repository: "docker.io/portkeyai/gateway_enterprise"
...
ingress:
  ...
  annotations:
    ...
    alb.ingress.kubernetes.io/target-type: ip
    alb.ingress.kubernetes.io/healthcheck-path: /v1/health
    alb.ingress.kubernetes.io/inbound-cidrs: <X.X.X.X/Y>
    alb.ingress.kubernetes.io/manage-backend-security-group-rules: "true"
```

**AI Gateway + MCP Gateway with ALB and Host-Based Routing**

The following sample `values.yaml` shows how to deploy both AI Gateway and MCP Gateway with host-based routing using an Application Load Balancer, built-in Redis, and IRSA for S3.
```yaml
images:
  gatewayImage:
    repository: "docker.io/portkeyai/gateway_enterprise"
