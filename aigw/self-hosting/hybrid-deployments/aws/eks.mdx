...
To whitelist, add an inbound rule to the Load Balancer's security group allowing connections from the AI Gateway Management Plane's IPs on NLB listener port. Contact the Palo Alto Networks team for the current address list.
...
**ElastiCache (IAM Auth) with S3 using IRSA**

The following sample `values.yaml` shows how to configure Amazon ElastiCache with IAM authentication, and Amazon S3 for log storage using IRSA.
```yaml
images:
  gatewayImage:
    repository: "docker.io/portkeyai/gateway_enterprise"
