...
| Cache (Prompts, Configs & Providers)    | Built-in Redis or Amazon ElastiCache for Redis OSS or Valkey              | Deployed within the same VPC as the AI Gateway.                                                                                                                |
...
| Layer-4 pass-through, AWS PrivateLink (Inbound Management Plane) | NLB         |
...
Establishes a secure, private connection between the Data Plane and the Management Plane within the AWS network.

**Steps to establish AWS PrivateLink connectivity:**

1. Contact the AI Gateway and provide your AWS account ARN so it can be whitelisted in the AI Gateway's Management Plane.
2. Once you receive confirmation from the AI Gateway that your AWS account is whitelisted, go to the [VPC Console](https://console.aws.amazon.com/vpc/).
...
11. Update the `main.tf` to point the Gateway to the private Management Plane endpoint:
...
Establishes a secure, private connection between the Management Plane and the Data Plane within the AWS network.
...
- Open the Endpoint Service > click on **Actions** > select **Allow principals**, and enter the Management Plane's ARN (`arn:aws:iam::299329113195:root`).
- Reach out to the Palo Alto Networks team and share the following details:
    - **Service name**
    - **DNS names**
    - **Private DNS name**
    - **Region** selected while creating the Endpoint Service
    - Port number on which the Load Balancer is listening for connections
- Wait for the Palo Alto Networks team to initiate a connection request from the Management Plane's AWS account to your Gateway's AWS account. Navigate to the **Endpoint connections** section, and once the request appears, approve it.
...
To whitelist, add an inbound rule to the Load Balancer's security group allowing connections from the AI Gateway Management Plane's IPs (`54.81.226.149`, `34.200.113.35`, `44.221.117.129`) on the listener port. Alternatively, set `allowed_lb_cidrs` in the module configuration:
...
- Go to the [the AI Gateway website](https://stratacloudmanager.paloaltonetworks.com/) -> **Logs**.
