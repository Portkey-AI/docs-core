...
- **Management Plane** — the Dashboard, Backend, MySQL, ClickHouse, Redis, and Blob Store.
  ...
  logging. It communicates with the Customer Environment over a private link.
...
2. **Auth and config hydration** — the gateway checks Redis for a cached copy of the API key,
   ...
   a cache miss the gateway calls the Backend over the private link; the Backend reads from MySQL
