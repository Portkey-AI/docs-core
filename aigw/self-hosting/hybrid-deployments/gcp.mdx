...
| Cache (Prompts, Configs & Providers)    | Built-in Redis, Google Memorystore Redis or Valkey                        | Deployed within the same VPC as the AI Gateway.                                                                                                                  |
...
<Tabs>
  ...
  To set up IAM-based authentication for AI Gateway to GCP Memorystore, follow the [steps](#setting-up-iam-permission) and add the following configuration to `values.yaml`.
  ```yaml
  serviceAccount:
    create: true
    automount: true
...
2. Set up access to the log store. The Gateway supports the following methods for connecting to GCS bucket for log storage:
    ...
    <Tabs>
      <Tab title="Workload Identity Federation">
        To set up IAM-based authentication for AI Gateway to GCP bucket, follow the [steps](#setting-up-iam-permission) and add following configuration in `values.yaml`.
...
Establishes a secure, private connection between the Management Plane and Data Plane within the GCP network.
...
2. Create an IP address for the PSC endpoint.
  ...
  ```
3. Contact the Palo Alto Networks team, share your GCP Project ID, and request private connectivity.
4. Once your project has been whitelisted and the AI Gateway has shared the `SERVICE_ATTACHMENT_URI`, create the forwarding rule for the Management Plane's private endpoint.
  ```sh
  # Replace <VPC_NAME> with name of vpc in which your GKE cluster is created.
...
5. **(Optional)** If your GKE cluster is in a different region from `us-east4`, enable global access on the PSC endpoint.
  ...
  ```
6. Fetch Connection ID of PSC endpoint and share it with the AI Gateway for connection approval.
  ```sh
  gcloud compute forwarding-rules describe portkey-cp-pvt-endpoint-rule \
      --region=us-east4 \
      --format="value(pscConnectionId)"
  ```
7. Once the AI Gateway confirms approval, verify the connection status:
  ```sh
  gcloud compute forwarding-rules describe portkey-cp-pvt-endpoint-rule \
    --region=us-east4 \
...
8. Fetch Private IP of PSC endpoint.
  ...
  echo ${PSC_IP}
  ```
9. Create a Private DNS Zone for DNS resolution of Management Plane PSC endpoint.
  ```sh
  ...
  gcloud dns managed-zones create portkey-control-plane-pdz \
    ...
    --networks="<VPC_NAME>"
  ```
10. Create a DNS record to point to AI Gateway PSC endpoint IP.
  ```sh
...
- AI Gateway must be exposed via either:
...
3. Select the AI Gateway Gateway's Load Balancer and the forwarding rule associated with it.
...
6. Once the PSC service is created, copy the **Service attachment** and share it with the Palo Alto Networks team so they can initiate a connection request. In addition to that also share AI Gateway URL (e.g., https://gateway.example.com).

7. Once the connection is initiated from the Management Plane, go to your PSC Published Service and approve the connection request.

8. To verify connectivity from the Management Plane to the Data Plane, send a test request to the Gateway and check if you can view the full log details on Strata Cloud Manager after clicking a log entry.
...
To whitelist, add an inbound rule to the VPC Firewall allowing connections from the AI Gateway Management Plane's IPs (`54.81.226.149`, `34.200.113.35`, `44.221.117.129`) on Load Balancer listner port.
...
To allow the AI Gateway to authenticate with Google Memorystore using IAM, grant `roles/redis.dbConnectionUser` to the GSA.
...
#### GCS Bucket
To allow the AI Gateway to access a GCS bucket for log storage, grant `roles/storage.objectAdmin` (or a custom role with `storage.objects.create` and `storage.objects.get`) to the GSA.
...
#### Vertex AI (Optional)
To allow the AI Gateway to invoke Vertex AI models, grant `roles/aiplatform.user` to the GSA.
...
The following sample `values.yaml` shows how to configure Google Memorystore for caching, GCS for log storage using Workload Identity Federation, and GCP Private Service Connect for outbound connectivity to the Management Plane.
```yaml
images:
  gatewayImage:
    repository: "docker.io/portkeyai/gateway_enterprise"
