...
All custom metrics are automatically labelled with:
...
The AI Gateway Enterprise Gateway exposes 15 custom metrics designed to provide deep visibility into LLM gateway operations, performance characteristics, and business metrics.

### Universal Label Schema

All custom metrics share a common labelling schema enabling multi-dimensional analysis:
...
- `config_name`: Config slug from the request — **opt-in**, disabled by default (set `PROMETHEUS_INCLUDE_CONFIG_NAME_LABEL = true`). Only populated when a config slug is passed via headers; defaults to `N/A` for inline config objects
- `api_key_name`: API key name from the request — **opt-in**, disabled by default (set `PROMETHEUS_INCLUDE_API_KEY_NAME_LABEL = true`)
...
- **Model Popularity**: Track adoption of different AI models across your organisation
...
### 4. AI Gateway Processing Time (Excluding Streaming Latency)
...
**Technical Description**:
Measures the AI Gateway's internal processing time excluding the time spent waiting for the final byte of streamed responses from LLM providers. This metric isolates the gateway's computational overhead from provider streaming characteristics, enabling precise performance optimization of internal operations.
...
- **Model Behaviour Analysis**: Different models exhibit different streaming characteristics
...
### 6. Total AI Gateway Request Duration
...
**Technical Description**:
Comprehensive timing metric measuring the complete duration of AI Gateway request processing from initial request receipt to final response transmission. This provides the most complete view of gateway performance and serves as the authoritative metric for end-to-end processing analysis.
...
- **Workspace/Organisation Context**: Multi-tenant authorization processing
...
**Technical Description**:
Tracks the performance of hierarchical rate limiting checks across organisation, workspace, and user levels. This metric monitors the computational overhead of the multi-level rate limiting system and identifies performance impacts of complex rate limiting policies.

**Rate Limiting Hierarchy**:
- **Organisation Level**: Global rate limits across entire organisation
...
The AI Gateway Enterprise Gateway allows you to completely disable Prometheus metrics collection if not needed for your deployment. When disabled, both the metrics middleware and the `/metrics` endpoint are deactivated, reducing overhead in environments where Prometheus monitoring is not required.
...
### Config Name and API Key Name Labels

Add `apiKeyName` and/or `configName` to the `PROMETHEUS_EXTRA_LABELS` variable to expose these as labels on every custom metric.

| Environment Variable | Type | Default | Description |
|----------------------|------|---------|-------------|
| `PROMETHEUS_EXTRA_LABELS` | `string` (comma-separated list) | `""` (empty) | Additional labels to include. **Supported values:** `apiKeyName`, `configName` |

```bash
PROMETHEUS_EXTRA_LABELS=apiKeyName,configName
```

This exports:
- `apiKeyName` — the name of the AI Gateway API key used (as configured in Strata Cloud Manager)
- `configName` — the name of the AI Gateway config used for the request

### Dynamic Metadata Label System

The AI Gateway Enterprise Gateway supports dynamic metadata labelling through request-specific metadata injection. This powerful feature enables fine-grained observability across custom dimensions specific to your organisation's structure and use cases.
...
- [Analytics Dashboard](/aigw/product/observability/analytics) - SaaS monitoring and analytics
...
- [Observability](/aigw/product/observability) - General observability features
