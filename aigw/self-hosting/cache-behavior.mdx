...
LLM response caching is opt-in and must be explicitly enabled per request or via an AI Gateway Config. TTL only applies when caching is active. The [Cache (Simple & Semantic)](/aigw/product/ai-gateway/cache-simple-and-semantic) doc covers how to enable caching, set TTL via `max_age`, configure org-level default TTL, and use force refresh.

---

## Sync: Management Plane → Gateway

Every **minute**, the Gateway sends a sync request to the Management Plane carrying a stable `syncIdentifier` (a UUID generated once per Gateway instance and persisted in cache). The Management Plane uses this identifier to return only the objects that have changed since the last successful sync for that Gateway instance.
...
For each object in the delta, the Gateway **deletes** its cache entry. The updated data is not pushed into the cache at this point. On the next incoming request that needs that object, the Gateway fetches the latest version from the Management Plane and re-populates the cache with a fresh 7-day TTL.

---

## Resync: Gateway → Management Plane

Separately, a **resync** process also runs every **minute**. Its direction is the opposite of sync: it pushes data from the Gateway back to the Management Plane.

The only data pushed back is **usage counters** (token usage and cost usage). Rather than writing to the Management Plane on every request, the Gateway accumulates these counters locally in cache as requests are processed. The resync worker reads the accumulated values and flushes them to the Management Plane in batches. After a successful flush, the local counter keys are deleted from cache.
...
No other cached data (configs, prompts, guardrails, or LLM responses) is ever pushed back to the Management Plane.
...
|:--------|:-------------|
| Delta sync (every minute) | The Gateway deletes cache entries for any object the Management Plane reports as changed. The next request for that object fetches the latest version and re-caches it with a fresh 7-day TTL. |
| TTL expiry (7 days) | The entry is removed automatically. The next request triggers a fresh fetch from the Management Plane. |
...
See [Cache (Simple & Semantic)](/aigw/product/ai-gateway/cache-simple-and-semantic) for full details on force refresh and TTL configuration.
...
In each case, an evicted entry behaves the same as an expired one: the next request for that object triggers a fresh fetch from the Management Plane (for config objects) or a live LLM call (for response cache entries).
...
<CardGroup cols={2}>
  <Card title="Enterprise Architecture" icon="sitemap" href="/aigw/self-hosting/hybrid-deployments/architecture">
    Overview of the Data Plane / Management Plane split and data flow between them
  </Card>
  <Card title="Enterprise Components" icon="database" href="/aigw/product/enterprise-offering/components">
