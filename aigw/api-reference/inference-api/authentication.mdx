...
<Note>
This key authenticates inference requests only. The Admin API does not accept it. Those requests are authorised with a Strata Cloud Manager access token issued to a service account. See [Admin API Authentication](/aigw/api-reference/admin-api/authentication).
</Note>
