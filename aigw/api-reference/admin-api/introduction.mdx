...
graph LR
    ...
    APP["Your script, CI job<br/>or platform"]
    ADMIN["Admin API<br/>api.apps.paloaltonetworks.com/ai_gw"]
    ORG["Organisation resources<br/>integrations, credentials, deployments"]
    WS["Workspace resources<br/>configs, providers, guardrails"]

    SA -->|"grant_type=client_credentials<br/>scope=tsg_id:TSG_ID"| AUTH
    AUTH -->|"access token, valid 15 minutes"| APP
    APP -->|"Authorization: Bearer"| ADMIN
    ADMIN --> ORG
    ADMIN --> WS
```
...
```mermaid
graph TD
    T["Access token<br/>carries one tsg_id"] --> ORG["Organisation<br/>one TSG is one organisation"]

    ORG --> OL["Organisation level<br/>/ai_gw/admin/v2"]
    ORG --> WL["Workspace level<br/>/ai_gw/v2 with workspace_id"]

    OL --> O1["Integrations"]
    OL --> O2["MCP integrations"]
    OL --> O3["Secret references"]
    OL --> O4["Deployments"]
    OL --> O5["Organisation guardrails"]

    WL --> W1["Configs"]
    WL --> W2["Providers"]
    WL --> W3["Workspace guardrails"]
    WL --> W4["Analytics"]
```
...
The split matches the base URLs: organisation-level resources are served from the `admin/v2` base. API keys and limit policies span both levels, and [`POST /api-keys/{type}/{sub-type}`](/aigw/api-reference/api-keys/post-api-keys-by-type-by-sub-type) takes the level as a path parameter.
...
<AccordionGroup>
  ...
  <Accordion title="Issue and rotate an API key" icon="key">
    1. [`POST /api-keys/{type}/{sub-type}`](/aigw/api-reference/api-keys/post-api-keys-by-type-by-sub-type) creates the key with the scopes it needs
