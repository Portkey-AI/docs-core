...
sequenceDiagram
    ...
    participant App as Your script or CI job
    participant Auth as auth.apps.paloaltonetworks.com
    participant API as Admin API

    Admin->>SCM: Create a service account
    SCM-->>Admin: Client ID, Client Secret, TSG ID
    Admin->>App: Store the three values as secrets

    App->>Auth: POST /oauth2/access_token<br/>basic auth, scope=tsg_id:TSG_ID
    Auth-->>App: access_token, expires_in 900

    loop Within the next 15 minutes
        App->>API: Authorization: Bearer access_token
        API-->>App: Resources of that tenant
    end
```
