...
<Info>
  This feature is available only on the [Enterprise Plan](/aigw/product/enterprise-offering) of Portkey.
</Info>
...
| User attribution in logs | The gateway uses `email_id`, `sub`, or `uid` from the token for per-user logging and metrics. When the token's email matches a Portkey user in the target workspace, the request is attributed to that user (and inherits their workspace role) instead of a generic workspace service key. |
