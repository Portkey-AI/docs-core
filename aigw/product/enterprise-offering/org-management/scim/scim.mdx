---
title: "Overview"
description: "SCIM integration with Prisma AIRS AI Gateway."
---

# SCIM Integration Guide

The AI Gateway supports **SCIM (System for Cross-domain Identity Management)** to automate user provisioning and deprovisioning.
...
SCIM is an open standard that allows organisations to automate the management of user identities and groups across applications. By integrating with SCIM, you can:

- Automatically provision and update user accounts.
- Deprovision users when they leave your organisation.
- Sync user attributes and workspace memberships.

<Note>
SCIM will only provision users whose email belongs to a **valid and verified domain** configured for your organisation in Strata Cloud Manager (`Admin Settings > Authentication Settings > Allowed Domains`). Users with unverified or unconfigured domains will not be provisioned.
</Note>

## SCIM Base URL

To integrate SCIM with our platform, get the SCIM Base URL from Management Plane.
...
You need to generate an **API token** from Management Plane (``Admin Settings > Authentication Settings > SCIM Provisioning`) and use it as a bearer token in the SCIM requests.
...
For further assistance, please contact our support team via [Portkey support](https://support.portkey.ai/forms/customer-portal-ticket-form).
