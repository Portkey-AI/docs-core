...
description: "Map SCIM groups to Prisma AIRS AI Gateway workspaces and roles without naming restrictions."
---

# SCIM Group Management

The AI Gateway now supports flexible group-to-workspace mapping, allowing you to provision groups from your identity provider (Okta or Azure Entra) with any naming convention and then map them to one or more AI Gateway workspaces and roles directly from the Management Plane.

---

## Overview

Previously, SCIM group provisioning required groups to follow a specific naming format (`ws-{group}-role-{role}`) to automatically map to AI Gateway workspaces. This restriction has been removed.
...
- **Manage mappings** directly from Management Plane
...
2. **Map the group** to an AI Gateway workspace and assign a role from Management Plane
3. **Users are automatically assigned** to the workspace with the specified role when added to the group

<Warning>
Groups must be provisioned from your identity provider **first** before they can be mapped in the AI Gateway. You cannot map a group that hasn't been provisioned yet.
</Warning>
...
Before mapping groups in the AI Gateway, ensure the groups are provisioned from your identity provider.

### For Okta Users

1. Navigate to your Okta application settings
2. Go to the **Push Groups** tab
3. Push the groups you want to map to the AI Gateway
4. Verify the groups appear in the AI Gateway after provisioning

For detailed instructions, refer to the [Okta Group Provisioning](/aigw/product/enterprise-offering/org-management/scim/okta#group-provisioning-with-okta) section.
...
4. Verify the groups are provisioned to the AI Gateway

For detailed instructions, refer to the [Azure Entra Group Provisioning](/aigw/product/enterprise-offering/org-management/scim/azure-ad#group-workspace-provisioning) section.

---

## Configuring Group Naming Format (Optional)

If you prefer automatic group-to-workspace mapping based on naming conventions, you can configure a custom prefix and separator to match your organisation's group naming format.

### Default Format

By default, the AI Gateway expects groups to follow this format:
...
1. Navigate to **Admin Settings > Authentication Settings > SCIM Provisioning** in Management Plane
...
Once groups are provisioned from your identity provider, you can map them to AI Gateway workspaces. A single SCIM group can be mapped to **multiple workspaces**; every member of the group is automatically provisioned into each mapped workspace with the configured role.
...
4. Select the appropriate fields from the dropdowns:
    ...
    - **AI Gateway Workspace**: The workspace to map the group to
...
<Info>
Role names are **case-insensitive**. For example, `Admin`, `admin`, and `ADMIN` are all treated as the same role. This applies to both organisation roles and workspace roles in SCIM provisioning.
</Info>
...
By default, when a SCIM group update includes an archived (deprovisioned) user, the AI Gateway will **not** reactivate that user. This is because some identity providers (like Okta) send the full member list on every group update, which could unintentionally reactivate users that were removed.
...
Deleting a mapping from the AI Gateway only unlinks the SCIM group from that workspace. **Users already provisioned into the workspace remain as workspace members**, and the workspace itself is **not** archived. Future updates to the SCIM group will simply stop affecting that workspace. Other workspaces the group is still mapped to continue to operate normally.

<Info>
Workspace and user cleanup only happen when the SCIM group is **deleted at the identity provider** (which sends `DELETE /v1/scim/Groups/{id}` to the AI Gateway). In that path:
...
- The workspace itself is archived only if it is **not** the organisation's default/shared workspace
...
- **No naming restrictions** - Use any group naming convention that fits your organisation
- **Flexible mapping** - Map groups to workspaces after provisioning
- **One group, many workspaces** - Map a single SCIM group to multiple AI Gateway workspaces in one go, with one role applied uniformly across all of them
- **Simplified management** - Manage all mappings from Management Plane
