...
description: "Discover how Admin and Workspace API Keys are used to manage access and operations in Prisma AIRS AI Gateway."
---

## API Keys

<Warning>
API keys authenticate requests to the **inference API**. They are not accepted on the Admin API, which is authorised with a Strata Cloud Manager access token issued to a service account — see [Admin API Authentication](/aigw/api-reference/admin-api/authentication).

The scopes below describe what an API key may do on the inference path, and what the Admin API may do on that key's behalf.
</Warning>

The AI Gateway uses two types of API keys to manage access to resources and operations: **Admin API Keys** and **Workspace API Keys**. These keys play crucial roles in authenticating and authorizing various operations within your [organisation](/aigw/product/enterprise-offering/org-management/organizations) and [workspaces](/aigw/product/enterprise-offering/org-management/workspaces).

### Admin API Keys

Admin API Keys operate at the organisation level and provide broad access across all workspaces within an organisation.
...
<AccordionGroup>
  <Accordion title="Organisation Management" icon="building">
    | Scope | Description |
    |-------|-------------|
    | `organisation_users.create` | Create new users in the organisation |
    | `organisation_users.read` | View organisation user details |
    | `organisation_users.update` | Modify organisation user settings |
    | `organisation_users.delete` | Remove users from the organisation |
    | `organisation_users.list` | List all organisation users |
    | `organisation_service_api_keys.create` | Create new organisation service API keys |
    | `organisation_service_api_keys.update` | Modify organisation service API keys |
    | `organisation_service_api_keys.read` | View organisation service API key details |
    | `organisation_service_api_keys.delete` | Delete organisation service API keys |
    | `organisation_service_api_keys.list` | List all organisation service API keys |
    | `organisation_service_api_keys.rotate` | Regenerate organisation service API keys |
    | `organisation_settings.read` | View organisation settings, defaults, workspace overrides, and properties |
    | `organisation_settings.update` | Modify organisation settings, defaults, workspace overrides, and properties |
    | `organisation_guardrails.create` | Create organisation-level guardrails |
    | `organisation_guardrails.read` | View organisation guardrail details |
...
Both types of API keys play important roles in the AI Gateway's security model, enabling secure and efficient access to resources while maintaining proper separation of concerns between organisation-wide administration and workspace-specific operations.

### Related Topics

<Card title="Admin API Authentication" href="/aigw/api-reference/admin-api/authentication"/>
