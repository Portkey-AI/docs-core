...
Prisma AIRS AI Gateway supports following authentication protocols for enterprise customers.
...
<Info>
**Auto-Provisioning**: First-time SSO users are automatically provisioned to your organisation when they log in. If a user has a pending invite, it will be automatically accepted during their first SSO login.

To restrict access to invited users only, enable **`disable_auto_provision_user`** in your authentication settings. Users without a pending invite are then denied at login, and invited users must still belong to a verified allowed domain.
</Info>

<Note>
SSO will only provision users whose email belongs to a **valid and verified domain** configured for your organisation in Strata Cloud Manager (`Admin Settings > Authentication Settings > Allowed Domains`). Users with unverified or unconfigured domains will not be provisioned.
</Note>
...
By default the AI Gateway authenticates to your identity provider's token endpoint with `client_secret_basic`. If your provider requires a different method, set **`token_endpoint_auth_method`** in your OIDC settings (for example `client_secret_post` or `private_key_jwt`).

For `private_key_jwt`, also supply a **`jwks`** object containing your client's private key set; register the matching public JWKS with your identity provider. When a JWKS is supplied without an explicit method, the AI Gateway uses `private_key_jwt` automatically.

<Note>
Self-hosted deployments running in single-organisation SSO mode configure the same behaviour through the `OIDC_TOKEN_ENDPOINT_AUTH_METHOD` and `OIDC_CLIENT_JWKS` environment variables on the backend service.
</Note>

### Setup Steps
<Info>
    Following scopes are required for the AI Gateway to work with OIDC:
    - openid
    - profile
    - email
    - offline_access
...
- Once the application is created, please note the following details:
    ...
    - `Client Id`
    - `Client Secret`
- Update the above details in Management Plane in `Admin Settings > Authentication Settings > OIDC`.
...
- On the next step, fill in the required fields. The `signin redirect URI` should be https://stratacloudmanager.paloaltonetworks.com/ and the `Grant Type` should have `Authorization code` and `Refresh Token` as checked
- Create Application
...
- Add https://stratacloudmanager.paloaltonetworks.com/ as the `initiate login URI`
...
- Enter a name.
- Select one of the Supported account types that best reflects your organisation requirements.
- Under `Redirect URI`,
    - Select `Web` as the platform
    - Enter https://stratacloudmanager.paloaltonetworks.com/ as redirect url
- Click on Register
...
- Update the above details in Management Plane in `Admin Settings > Authentication Settings > SAML`.

#### Okta
- Go to `Applications` tab on okta dashboard and `create a new app integration`.
- Select `SAML 2.0` as the signin method.
- In `Configure SAML`, update
    - `Single sign-on URL` with Saml redirect url. You can find the Saml redirect url from the `Admin Settings > Authentication Settings > SAML Redirect/Consumer Service URL` from Management Plane.
    - `Audience URI (SP Entity ID)` with SAML Entity ID from Management Plane.
- Create Application
...
- Under `Redirect URI`,
    - Select `Web` as the platform
    - Enter the `SAML Redirect/Consumer Service URL` from Management Plane as redirect url
- Select `Register`.
...
- To the right of `Application ID URI`, select `Add`.
    - Enter `SAML Entity ID` from Management Plane as the `App ID URI`.
- Select `Save`.
- Once everything is set up, please note the following details
    - Copy the `Federation metadata document URL` and paste it in Management Plane in `Admin Settings > Authentication Settings > SAML > Provider Metadata URL`

---

## Allowed Domains

In `Admin Settings > Authentication Settings`, you can configure **Allowed Domains** to restrict which email domains are permitted to authenticate with your organisation. You can add one or more domains (e.g., `example.com`) to control access.
