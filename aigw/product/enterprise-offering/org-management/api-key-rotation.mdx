...
**Endpoint:** [`POST /v2/api-keys/:apiKeyId/rotate`](/aigw/api-reference/api-keys/post-api-keys-by-id-rotate)
...
A rotation policy can be attached to any API key at [**creation**](/aigw/api-reference/api-keys/post-api-keys-by-sub-type) or via [**update**](/aigw/api-reference/api-keys/put-api-keys-by-id).

| Field | Type | Required | Description |
|---|---|---|---|
| `rotation_period` | `"weekly"` \| `"monthly"` | One of `rotation_period`, `rotation_period_days`, or `next_rotation_at` | Recurring schedule. Weekly rotates every Monday 00:00 UTC; monthly rotates on the 1st of each month 00:00 UTC |
| `rotation_period_days` | integer (1–365) | One of `rotation_period`, `rotation_period_days`, or `next_rotation_at` | Custom rotation cadence in days. Mutually exclusive with `rotation_period`. _(Backend `v1.16.0+`)_ |
| `next_rotation_at` | ISO 8601 datetime | One of `rotation_period`, `rotation_period_days`, or `next_rotation_at` | Explicit one-time or override date for the next rotation (normalized to UTC midnight) |
...
Organisation owners/admins can enforce a rotation cadence on workspace-user API keys via the `user_api_key_rotation_period` setting (with optional per-workspace override). The setting accepts an integer between `1` and `365` (days). _(Backend `v1.17.0+` stores and returns days only; `weekly`/`monthly` input is normalized to `7`/`30`.)_
...
[`GET /v2/api-keys/:apiKeyId`](/aigw/api-reference/api-keys/get-api-keys-by-id) returns the rotation policy alongside the key details:
...
<CardGroup cols={3}>
  <Card title="Rotate API Key" icon="rotate" href="/aigw/api-reference/api-keys/post-api-keys-by-id-rotate" />
  <Card title="Create API Key" icon="plus" href="/aigw/api-reference/api-keys/post-api-keys-by-sub-type" />
  <Card title="Update API Key" icon="pen" href="/aigw/api-reference/api-keys/put-api-keys-by-id" />
  <Card title="Retrieve API Key" icon="eye" href="/aigw/api-reference/api-keys/get-api-keys-by-id" />
  <Card title="API Keys (AuthN & AuthZ)" icon="key" href="/aigw/product/enterprise-offering/org-management/api-keys-authn-and-authz" />
  <Card title="User Roles & Permissions" icon="shield" href="/aigw/product/enterprise-offering/org-management/user-roles-and-permissions" />
</CardGroup>
