...
Prisma AIRS AI Gateway can push complete LLM request/response logs to any OpenTelemetry-compatible endpoint, following the [experimental semantic conventions for Generative AI](https://opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-spans/). This enables integration with external observability platforms like LangSmith, Datadog, Honeycomb, or any OTLP-compatible collector.

Unlike the [Analytics Export](/aigw/product/enterprise-offering/otel/analytics), this feature exports **complete logs** including:
...
- **External Analysis**: Send AI Gateway logs to specialized GenAI observability tools like LangSmith
...
- Metrics export (use [Analytics Export](/aigw/product/enterprise-offering/otel/analytics) for that)
...
1. LLM request is processed through AI Gateway
...
<Note>
  The endpoint should be the base OTLP URL. The AI Gateway automatically appends `/v1/traces` when sending data.
</Note>
...
### AI Gateway Tenant Attributes

These attributes are set on all spans and identify the AI Gateway organisation, workspace, and user associated with the request:

| Attribute | Source | Description |
|-----------|--------|-------------|
| `portkey.organisation.id` | Metrics | Organisation ID in the AI Gateway |
...
- **Trace ID**: Uses the AI Gateway's trace ID if valid (32 hex chars), otherwise generates a new one
- **Span ID**: Uses the AI Gateway's span ID if valid (16 hex chars), otherwise generates a new one
...
The AI Gateway exports spans using the semconv 1.40.0 structured message format (`gen_ai.input.messages`, `gen_ai.output.messages`, `gen_ai.system_instructions`), which is compatible with LangSmith and other modern GenAI observability tools.
...
2. Make an LLM request through AI Gateway
