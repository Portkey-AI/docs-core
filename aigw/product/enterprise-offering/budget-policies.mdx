...
Policies are the API-driven mechanism. For the simpler case — a single budget or rate limit set on an Integration in the dashboard and cascading to every provider created from it — see [Budget Limits](/aigw/product/policies/budget-limits) and [Rate Limits](/aigw/product/policies/rate-limits).
...
<Note>
Backend `v1.16.0+` allows updating `conditions` on existing usage-limit and rate-limit policies via the corresponding `PUT` endpoint. Earlier versions only supported updating other fields.
</Note>
...
<Note>
When `model` is used as a condition or group_by key, Prisma AIRS AI Gateway uses the **request model name** (the model you sent in the request body) for both matching and bucketing — not the model name returned by the provider in the response. This is important for providers like Azure OpenAI that return versioned model names in responses (e.g. `gpt-4.1-2025-06-01`), which differ from the request model name (e.g. `gpt-4.1`).
</Note>
...
<Note>
When a usage limit is exceeded, the AI Gateway returns a **412 Precondition Failed** HTTP status code.
</Note>

### Alert thresholds: audit logs and notifications

If you set an **`alert_threshold`**, the AI Gateway records when usage crosses that threshold in **[audit logs](/aigw/product/enterprise-offering/audit-logs)**. Email and other proactive alerts for threshold crossings are **work in progress**; they are not sent today.

### Policy Types

- **`cost`**: Limit based on total cost (in dollars)
- **`tokens`**: Limit based on total tokens consumed
- **`requests`**: Limit based on total number of requests _(Backend `v1.16.0+`)_
...
- **Minimum value**:
  ...
  - For `requests` type: `1` request
- **Units**:
  - For `cost` type: Value is in USD (dollars)
  - For `tokens` type: Value is in tokens
  - For `requests` type: Value is the number of requests
...
- **Behaviour**:
  - When usage reaches this threshold, an event is written to [audit logs](/aigw/product/enterprise-offering/audit-logs)
...
#### `periodic_reset_days` (optional)

Configures a custom reset cadence in days as an alternative to the calendar-based `periodic_reset`.

- **Type**: Integer (1–365)
- **Behaviour**: The usage counter resets every N days at 00:00 UTC, with the next reset computed from the policy's start date (or `next_usage_reset_at` if provided)
- **Mutually exclusive** with `periodic_reset` — setting both is rejected

#### `next_usage_reset_at` (optional)

Override the timestamp of the next scheduled reset.

- **Type**: ISO 8601 datetime (normalized to UTC midnight)
- **Behaviour**: Sets/overrides the next reset timestamp. Subsequent resets follow the configured `periodic_reset` or `periodic_reset_days` cadence.
...
<Note>
When a rate limit is exceeded, the AI Gateway returns a **429 Too Many Requests** HTTP status code.
</Note>

### Target

Rate limit policies support a `target` field that determines whether the policy applies to LLM requests or MCP tool calls:

- **`llm`** (default) — Applies to LLM API requests
- **`mcp_tools`** — Applies to MCP tool calls through the MCP Gateway

All existing policies default to `target: llm`. The target is **immutable** — it cannot be changed after creation. Archive and recreate the policy if you need a different target.

<Note>
MCP tool call rate limits require **Gateway v2.18.0+** and **Backend v1.24.0+**.
</Note>
...
The workload type this policy applies to.

- **Type**: String (enum)
- **Valid values**:
  - `"llm"` (default) - Applies to LLM API requests
  - `"mcp_tools"` - Applies to MCP tool calls
- **Behaviour**: Determines which condition and group-by keys are valid. Cannot be changed after creation.
...
### Supported Condition and Group-By Keys by Target

#### LLM Target (`target: llm`)
...
| `virtual_key` | Match by virtual key slug | `"virtual key slug"` |
| `provider` | Match by provider | `"openai"`, `"anthropic"` |
| `config` | Match by config slug | `"config slug"` |
| `prompt` | Match by prompt slug | `"prompt slug"` |
| `model` | Match by model (with wildcard support) | `"@openai/gpt-4o"`, `"@anthropic/*"` |
| `endpoint_type` | Match by endpoint type | `"chatComplete"`, `"embed"`, `"imageGenerate"` |

#### MCP Tools Target (`target: mcp_tools`)
...
1. **Conditions**: Each condition must have `key` and `value` fields.
2. **Group By**: Each group must have a `key` field.
3. **Valid Keys (LLM)**: `api_key`, `virtual_key`, `provider`, `config`, `prompt`, `model`, `endpoint_type`, or any key starting with `metadata.`
4. **Valid Keys (MCP)**: `api_key`, `workspace_id`, `mcp_server`, `mcp_tool`, or any key starting with `metadata.`
5. **Value**: Must be a numeric value.
6. **Workspace**: Workspace ID is required (can be provided via API key or request body).
...
### Use Case 16: Global MCP Rate Limit
...
### Use Case 17: Per-User MCP Rate Limit

Limit each user to 50 MCP tool calls per minute using metadata.
...
### Use Case 18: Per-Server Token Limit
...
### Use Case 19: Per-API-Key Limits Grouped by MCP Server
...
### Use Case 20: Exclude Specific Tools from a Server Limit

Apply rate limit to all Slack MCP tools except `list_channels`.
...
```
GET https://aigw.portkey.ai/v1/policies/usage-limits/{policyId}/entities?page_size=50
```
...
```
PUT https://aigw.portkey.ai/v1/policies/usage-limits/{policyId}/entities/{entityId}/reset
```
...
2. **Model Format**: Models are specified as `@provider/model-name`. Use `@provider/*` for wildcard matching. Both condition matching and group_by bucketing use the **request model name** passed, not the provider's response model name.
...
6. **Usage Limit Types**: `"cost"` (in dollars), `"tokens"`, or `"requests"` (Backend `v1.16.0+`).
...
### Usage Limits Policies
<Card title="Create Usage Limits Policy" href="/aigw/api-reference/usage-limit-policies/create-usage-limits-policy"/>
<Card title="List Usage Limits Policies" href="/aigw/api-reference/usage-limit-policies/list-usage-limits-policies"/>
<Card title="Retrieve Usage Limits Policy" href="/aigw/api-reference/usage-limit-policies/get-usage-limits-policy"/>
<Card title="Update Usage Limits Policy" href="/aigw/api-reference/usage-limit-policies/update-usage-limits-policy"/>
<Card title="Delete Usage Limits Policy" href="/aigw/api-reference/usage-limit-policies/delete-usage-limits-policy"/>
<Card title="List Usage Limits Policy Entities" href="/aigw/api-reference/usage-limit-policies/list-usage-limits-policy-entities"/>
<Card title="Reset Usage Limits Policy Entity" href="/aigw/api-reference/usage-limit-policies/reset-usage-limits-policy-entity"/>

### Rate Limits Policies
<Card title="Create Rate Limits Policy" href="/aigw/api-reference/rate-limit-policies/create-rate-limits-policy"/>
<Card title="List Rate Limits Policies" href="/aigw/api-reference/rate-limit-policies/list-rate-limits-policies"/>
<Card title="Retrieve Rate Limits Policy" href="/aigw/api-reference/rate-limit-policies/get-rate-limits-policy"/>
<Card title="Update Rate Limits Policy" href="/aigw/api-reference/rate-limit-policies/update-rate-limits-policy"/>
<Card title="Delete Rate Limits Policy" href="/aigw/api-reference/rate-limit-policies/delete-rate-limits-policy"/>
