...
description: Reference secrets stored in external secret managers like AWS Secrets Manager, Azure Key Vault, and HashiCorp Vault instead of storing them directly in Prisma AIRS AI Gateway.
---
...
Secret References let you point the AI Gateway to credentials stored in your external vault. Instead of entering keys directly in the AI Gateway, you create a **reference**, that tells the AI Gateway where to fetch the value at runtime.
...
Secret references use a split architecture between the AI Gateway's **management plane** and **data plane**:

1. You create a **secret reference** in the AI Gateway via the API. The management plane stores only the reference configuration (manager type, secret path, auth config) — never the actual secret value.
...
3. The management plane never fetches or sees the final secret. The data plane caches the fetched secret value for **5 minutes** to avoid hitting your secret manager on every request. After the TTL expires, the next request triggers a fresh fetch.
...
1. From your admin panel, go to [**Secret References**](https://stratacloudmanager.paloaltonetworks.com/) and click **Create**.
...
<Tabs>
  <Tab title="AWS Secrets Manager">
    ...
    Use this when the AI Gateway should assume an IAM role in your account.
    ...
    Uses the AI Gateway's own service role. Requires your secret's resource policy to grant the AI Gateway access.
...
When your secret value is a JSON object (not a plain string), use `value_format: "json"` to have the AI Gateway parse the value as JSON before injecting it into the configuration.
...
This walkthrough shows how to store GitHub OAuth credentials in AWS Secrets Manager and reference them in the AI Gateway for an MCP integration.
...
<Tip>
AWS Secrets Manager natively supports JSON values. Store your OAuth metadata as a JSON object directly — you do **not** need to `JSON.stringify()` it first. The AI Gateway (with `value_format: "json"`) will parse the value correctly either way, but storing as native JSON makes it easier to read and edit in the AWS console.
</Tip>

#### 2. Create a secret reference in the AI Gateway

In Strata Cloud Manager, go to [**Secret References**](https://stratacloudmanager.paloaltonetworks.com/) and click **Create**:
...
This example uses the `serviceRole` auth type, where the AI Gateway authenticates using its own service role — your secret's resource policy must grant the AI Gateway access. When you retrieve a secret reference via the API, any sensitive `auth_config` fields are automatically [masked](#sensitive-field-masking).
...
At runtime, the AI Gateway's data plane fetches the OAuth metadata from AWS Secrets Manager and injects it into the MCP integration configuration — no credentials are stored in the AI Gateway's management plane.
...
<Card title="Create Secret Reference" href="/aigw/api-reference/secret-references/create-secret-reference"/>
<Card title="List Secret References" href="/aigw/api-reference/secret-references/list-secret-references"/>
<Card title="Retrieve Secret Reference" href="/aigw/api-reference/secret-references/get-secret-reference"/>
<Card title="Update Secret Reference" href="/aigw/api-reference/secret-references/update-secret-reference"/>
<Card title="Delete Secret Reference" href="/aigw/api-reference/secret-references/delete-secret-reference"/>
