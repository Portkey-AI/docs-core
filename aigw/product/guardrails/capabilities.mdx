...
description: "Which endpoints, providers, input types, and execution modes Prisma AIRS AI Gateway Guardrails support."
---
...
| `/v1/decisions` | ✓ (state only) | — |

<Note>
  Embeddings have no model-generated output to evaluate, so only `input_guardrails` apply. See [Guardrails for Embeddings](/aigw/product/guardrails/embedding-guardrails).
</Note>

<Note>
  Decisions requests only expose the `state` field to guardrails — `model`, `questions`, and `criteria` are your own instructions and are never scanned. Typed answers have no free text to evaluate, so only `input_guardrails` apply. See [Guardrails for Decisions](/aigw/product/guardrails/decisions-guardrails).
</Note>
...
Guardrails are only evaluated on **LLM inference** endpoints where the AI Gateway can extract a text input and/or text output.
...
<Info>
  With `async=false`, the AI Gateway returns `246` (failed, but allowed) or `446` (failed, denied) instead of the standard provider status codes. See [Guardrail behaviour on the gateway](/aigw/product/guardrails#guardrail-behaviour-on-the-gateway).
</Info>
...
With output guardrails on a streaming request: After the stream completes and the `[DONE]` chunk is sent, the AI Gateway evaluates the full assembled response against your output guardrails. The results are delivered as an additional `hook_results` chunk. Output guardrail results on streaming are **informational only** — no fallback or retry actions are triggered.
...
See [Streaming Hook Results](/aigw/product/guardrails#streaming-responses) for SDK examples.
...
| `/decisions` | ✓ (input only, `state` field) |
...
<CardGroup cols={2}>
  <Card title="Guardrails Overview" href="/aigw/product/guardrails">
    Create guardrails, configure actions, attach to requests
  </Card>
  <Card title="Guardrails for Embeddings" href="/aigw/product/guardrails/embedding-guardrails">
    Guardrails on embedding workflows
  </Card>
  <Card title="Guardrails for Decisions" href="/aigw/product/guardrails/decisions-guardrails">
    Guardrails on typed-judgment (Decisions) requests
  </Card>
  <Card title="List of Guardrail Checks" href="/aigw/product/guardrails/list-of-guardrail-checks">
    All built-in and partner checks
  </Card>
  <Card title="Raw Guardrails in JSON" href="/aigw/product/guardrails/creating-raw-guardrails-in-json">
