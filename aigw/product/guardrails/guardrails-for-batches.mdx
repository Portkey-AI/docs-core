...
description: "Secure batch inference with Prisma AIRS AI Gateway Guardrails"
---
...
The AI Gateway supports running [guardrails](/aigw/product/guardrails) on [provider batch requests](/api-reference/inference-api/batch/create-batch). You pass guardrails through a [config](/aigw/product/ai-gateway/configs),
and the AI Gateway applies them asynchronously — pre-processing inputs before sending to the provider, and post-processing outputs after the provider completes.
...
flowchart LR
    ...
    subgraph postprocess["AI Gateway"]
        direction TB
...
Upload a JSONL file in OpenAI chat completions format to the AI Gateway.
...
The response returns an AI Gateway file ID (`pk_file_*`):
...
Guardrails are the **only** supported way to pass guardrail checks to a provider batch request. Define them in a [AI Gateway config](/aigw/product/ai-gateway/configs) [here](https://stratacloudmanager.paloaltonetworks.com/):
...
Save this config in Strata Cloud Manager and note the config ID (e.g., `pc-sync-g-1009a3`).
...
The response contains an AI Gateway batch ID (`pk_batch_*`) and the initial guardrail results in `hook_results`:
...
```sh
curl --location 'https://aigw.portkey.ai/v1/batches/pk_batch_cd7f0e97-37b6-48cb-abd7-1bb699300585?include_provider_details=true' \
  --header 'Authorization: Bearer {{PORTKEY_API_KEY}}'
```
...
Once the batch status is `completed`, retrieve the output using the gateway-exclusive file content endpoint:

```sh
curl --location 'https://aigw.portkey.ai/v1/files/{{OUTPUT_FILE_ID}}/content' \
  --header 'Authorization: Bearer {{PORTKEY_API_KEY}}' \
  --header 'x-portkey-provider: portkey'
```
