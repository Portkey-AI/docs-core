...
The Prisma AIRS AI Gateway's guardrails also cover `/v1/decisions` — the typed-judgment endpoint for models like [TypeSafe's Jev](/aigw/integrations/llms/typesafe). This lets you screen the content a decision is made about before it reaches the model, the same way you would for a chat or embedding request.
...
flowchart LR
    ...
    Guardrails -->|Pass| LLM[TypeSafe / Jev]
...
2. The AI Gateway's before-request guardrails scan `state` — never `model`, `questions`, or nested `criteria`.
3. If all checks pass, the request (with `state` possibly redacted by a mutator) is forwarded to the provider.
4. If a check fails, the configured [guardrail action](/aigw/product/guardrails) applies: log only, soft-deny with a `200`, or hard-deny with a `446`.

<Note>
  There are no after-request (output) guardrails on `/v1/decisions`. A successful answer is a typed judgment — a choice, a score, or a probability — with no free text to scan, so only `input_guardrails` apply. This mirrors [Guardrails for Embeddings](/aigw/product/guardrails/embedding-guardrails).
</Note>
...
- If a mutator's output is no longer valid JSON for an object/array `state`, the AI Gateway keeps the original `state` unchanged rather than forwarding corrupted content.
...
<CardGroup cols={2}>
  ...
  <Card title="Supported Endpoints & Capabilities" href="/aigw/product/guardrails/capabilities">
    Where guardrails run across the gateway
  </Card>
  <Card title="TypeSafe (Jev)" href="/aigw/integrations/llms/typesafe">
