---
title: "Prisma AIRS AI Gateway Models"
description: "Open-source pricing and configuration database for 2,300+ LLMs across 35+ providers"
api: "GET https://aigw.portkey.ai/model-configs/pricing/{provider}/{model}"
authMethod: "none"
---

<Info>
**AI Gateway Models** is a free, open-source pricing database for LLMs. No API key required — just query any model's pricing directly.
</Info>
...
```bash cURL
curl https://aigw.portkey.ai/model-configs/pricing/openai/gpt-4o
```

```javascript JavaScript
const response = await fetch(
  'https://aigw.portkey.ai/model-configs/pricing/anthropic/claude-3-5-sonnet-20241022'
);
...
pricing = requests.get(
    'https://aigw.portkey.ai/model-configs/pricing/google/gemini-2.0-flash-001'
).json()
...
```
GET https://aigw.portkey.ai/model-configs/pricing/{provider}/{model}
```
...
```
GET https://aigw.portkey.ai/model-configs/general/{provider}/{model}
```
...
<ResponseField name="disablePlayground" type="boolean">
  If `true`, model is not available in the AI Gateway playground.
</ResponseField>
...
The models database powers automatic cost tracking for all requests through the AI Gateway. When you make requests via the AI Gateway, costs are calculated automatically using this pricing data.

<CardGroup cols={2}>
  <Card title="Cost Analytics" icon="chart-line" href="/aigw/product/observability/analytics">
    View real-time spend across all your LLM usage
  </Card>
  <Card title="Budget Limits" icon="dollar-sign" href="/aigw/product/model-catalog/integrations#3-budget--rate-limits">
...
<Card title="Custom Pricing" icon="pen-to-square" href="/aigw/product/model-catalog/model-overrides">
