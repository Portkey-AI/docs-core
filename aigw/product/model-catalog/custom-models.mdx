...
description: 'Learn how to add custom and fine-tuned models to your Prisma AIRS AI Gateway Model Catalog for seamless integration.'
---
...
Configure via the [Update Model Access](/api-reference/admin-api/control-plane/integrations/models/update-model-access) API (`models[].configurations`), or set the fields when editing the model in Model Provisioning.

```bash
curl -X PUT "https://api.portkey.ai/v1/integrations/YOUR_INTEGRATION_SLUG/models" \
...
<Note>
`custom_headers` must be a flat string-to-string object. See [Custom hosts](/aigw/product/ai-gateway/custom-hosts) for URL validation rules on `custom_host`.
</Note>
