---
title: "Feedback"
description: "The Prisma AIRS AI Gateway's Feedback APIs provide a simple way to get weighted feedback from customers on any request you served, at any stage in your app."
---
...
The AI Gateway adds trace ids to all incoming requests. You can find this in the `x-portkey-trace-id` response header.

To use your own trace IDs, send them as part of the request headers - [Adding a trace ID to your requests](/aigw/product/observability/traces#how-to-enable-request-tracing)
...
```sh
curl --location 'https://aigw.portkey.ai/v1/feedback' \
...
You can see the `Feedback Count` and `Value: Weight` pairs for each `trace-id` on the logs page. You can also view the feedback details on [Analytics](/aigw/product/observability/analytics#feedback) and on the Prompt Eval tabs.
