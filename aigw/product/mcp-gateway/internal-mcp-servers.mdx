...
description: Add your internal MCP servers to Prisma AIRS AI Gateway. Get enterprise-grade auth, access control, and logging without building it.
---
...
<Note>
On a self-hosted gateway, servers on private IPs or internal hostnames are blocked by the gateway's SSRF policy until you add the server's hostname to `TRUSTED_CUSTOM_HOSTS`. See [Connect to internal services](/aigw/self-hosting/private-network-access).
</Note>
