...
description: Connect third-party MCP servers like Linear, GitHub, Slack, and more through Prisma AIRS AI Gateway.
---
...
**Add external servers to the AI Gateway.** Get centralized access control, logging, and credential management without building infrastructure.

---

## Why Use the AI Gateway for External MCP Servers

- **Centralized credential management.** OAuth tokens for Linear, GitHub, Slack—all managed in one place. Users authenticate once, the AI Gateway handles token refresh.
...
- **Unified authentication.** Your agents authenticate to the AI Gateway with a single API key. The AI Gateway handles the complexity of OAuth flows with each external service.
...
| **Server** | The AI Gateway authenticates to external service | OAuth 2.1, Client Credentials, Headers |

Users authenticate to the AI Gateway. The AI Gateway authenticates to external services. Users never see external service credentials.
...
1. The AI Gateway registers as an OAuth client with the service
2. Users complete OAuth consent when they first use the server
3. The AI Gateway stores and refreshes tokens automatically
...
Many MCP servers support OAuth 2.1 with Dynamic Client Registration. For these servers, just select **OAuth 2.1** as the auth type—the AI Gateway handles client registration automatically.
...
2. Set the callback URL to `https://aigw.portkey.ai/m/oauth/upstream-callback`
3. Add the `client_id`, `client_secret`, and `redirect_uri` in the AI Gateway's **Advanced Configuration**

See the [GitHub MCP server guide](/aigw/integrations/mcp-servers/github-mcp-server#connect-via-portkey-mcp-gateway) for a detailed walkthrough.
...
The API key determines permissions. When a user first calls a tool, the AI Gateway initiates OAuth with the external service if needed.
...
1. User calls a tool through the AI Gateway
2. If no token exists, the AI Gateway returns an authorization URL
3. User completes OAuth consent
4. The AI Gateway stores the token
...
- **Client Credentials**: The AI Gateway fetches tokens using client ID/secret
...
<CardGroup cols={2}>
  <Card title="Authentication" icon="lock" href="/aigw/product/mcp-gateway/authentication">
