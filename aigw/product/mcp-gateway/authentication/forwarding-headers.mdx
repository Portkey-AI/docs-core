...
Without header forwarding, these headers stop at Prisma AIRS AI Gateway. With it, they pass through to the MCP server.
...
The same behaviour with explicit configuration. Entries in the `headers` array can be:
...
| `x-portkey-api-key` | AI Gateway authentication |
...
In `all-except` mode, these are automatically added to the blocklist. You cannot override this behaviour.
...
If you use [Identity Forwarding](/aigw/product/mcp-gateway/authentication/identity-forwarding), identity headers are also protected:
...
| 4 (highest) | Identity headers | Headers from [Identity Forwarding](/aigw/product/mcp-gateway/authentication/identity-forwarding) |
...
| **Identity Forwarding** | Pass authenticated user claims | The AI Gateway (from validated token) |
...
- `X-User-Claims` from the AI Gateway (for user identity)
...
| Topic | Description |
|-------|-------------|
| [Identity Forwarding](/aigw/product/mcp-gateway/authentication/identity-forwarding) | Pass authenticated user claims to MCP servers |
| [Custom Auth](/aigw/product/mcp-gateway/authentication/custom-auth) | Configure static headers for MCP servers |
