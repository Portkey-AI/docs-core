...
When Prisma AIRS AI Gateway connects to OAuth-protected MCP servers using **OAuth Auto**, it registers as an OAuth client with the server's authorization endpoint. By default, the AI Gateway uses standard client metadata. For compliance or branding requirements, you can customize this.

## When to Use

Customize OAuth client metadata when:
- The OAuth server requires a pre-registered `software_id` or specific scopes
- You need custom branding during OAuth consent screens (your company name/logo instead of the AI Gateway's)
...
When a user first accesses an OAuth-protected MCP server, the AI Gateway initiates the OAuth flow. As part of this process, the AI Gateway presents client metadata to the authorization server (per RFC 7591 - OAuth 2.0 Dynamic Client Registration).
...
<Warning>
Do **not** wrap this in an outer `oauth_metadata` key when using the **OAuth Metadata** field. The field's contents are stored as your OAuth metadata directly, so an extra wrapper nests your settings one level too deep and the AI Gateway ignores them silently — including a `client_id`, which causes the AI Gateway to fall back to Dynamic Client Registration.
...
| `client_uri` | string | Organisation's homepage URL |
...
The `redirect_uris` field is **never customizable**. The AI Gateway always uses its own callback URL for OAuth flows:

```
<gateway-url>/oauth/upstream-callback
```

This ensures OAuth tokens are delivered securely to the AI Gateway's gateway, not to an arbitrary URL.
...
`authorization_params` cannot override the parameters the AI Gateway and the MCP SDK own, because doing so would break PKCE or the callback correlation:
...
When users authorize access, they see "Acme Corp MCP Gateway" with your logo instead of generic the AI Gateway branding.
...
Some MCP servers like GitHub don't support OAuth Dynamic Client Registration (DCR). For these servers, create an OAuth App manually and provide the credentials to the AI Gateway.
...
Providing a `client_id` disables Dynamic Client Registration entirely for that integration — the AI Gateway presents the client you configured instead of trying to register a new one.
...
<Note>
Set the OAuth App's **Authorization callback URL** to the AI Gateway's callback, `<gateway-url>/oauth/upstream-callback`. On the AI Gateway's managed gateway that is `https://mcp.portkey.ai/oauth/upstream-callback`; on a self-hosted or hybrid deployment, use your own gateway host (the value of `MCP_GATEWAY_BASE_URL`). This URL is always controlled by the gateway, so a `redirect_uri` set in your OAuth metadata is ignored.
</Note>

See the [GitHub MCP server guide](/aigw/integrations/mcp-servers/github-mcp-server#connect-via-portkey-mcp-gateway) for complete setup instructions.
...
Without a refresh token, the AI Gateway has nothing to renew the connection with, so the user is pushed back through consent once the access token expires (one hour for Google).
...
The AI Gateway appends every key in `authorization_params` to the upstream authorization URL on each auth flow. Values must be strings; non-string values are ignored.
...
**the AI Gateway still attempts Dynamic Client Registration even though I set a `client_id`.**
The AI Gateway resolved no client ID from your configuration, so it fell back to registration. Check that the JSON in the **OAuth Metadata** field has `client_id` at the top level, with no outer `oauth_metadata` wrapper. If you configured it through the **Advanced Configuration** box instead, the wrapper is required there — but it must appear exactly once.

**Consent succeeds, then the token exchange fails.**
The client ID is being presented correctly and the failure is now on the provider side. In order of likelihood: the OAuth App is registered as a confidential/web client while the AI Gateway defaults to `token_endpoint_auth_method: "none"` (set it explicitly, and supply `client_secret`); the callback URL registered on the provider doesn't exactly match `<gateway-url>/oauth/upstream-callback`; or the app is missing a granted scope or the `refresh_token` grant, which AI Gateway requests by default.
...
| Topic | Description |
|-------|-------------|
| [Authentication Overview](/aigw/product/mcp-gateway/authentication) | How authentication layers work |
| [External MCP Servers](/aigw/product/mcp-gateway/external-mcp-servers) | Adding OAuth-protected external servers |
