...
JWT validation enables Prisma AIRS AI Gateway to verify tokens from your identity provider before processing MCP requests. Use with [External OAuth](/aigw/product/mcp-gateway/authentication/external-oauth) to bring your own IdP.

## When to Use

Organisations with existing identity providers (Okta, Auth0, Azure AD, Cognito) can use JWT validation. Users authenticate through the IdP, and MCP access works the same way.

Flow:
1. Users get a token from the IdP
2. Token included in MCP requests
3. The AI Gateway validates the token against the IdP
4. Valid requests proceed with user identity attached

The AI Gateway never handles user credentials. Your IdP remains the source of truth for identity.
...
The AI Gateway fetches keys and caches them (default: 24 hours). When a key rotates, the AI Gateway automatically refetches the JWKS if verification fails with the cached key.

**Best for:** Most production deployments. Minimal configuration, automatic key rotation.

#### Multiple JWKS URIs

When your infrastructure uses multiple identity providers or key sets, pass a comma-separated list of JWKS endpoints:

```json
{
  "jwt_validation": {
    "jwksUri": "https://idp-1.example.com/.well-known/jwks.json, https://idp-2.example.com/.well-known/jwks.json",
    "algorithms": ["RS256"]
  }
}
```

The AI Gateway fetches all endpoints in parallel, merges their key sets, and finds the matching key by `kid`. If one endpoint is unavailable, tokens are still verified against the keys from the remaining endpoints.

<Note>
URLs that contain commas in query strings (e.g., `?tenants=a,b`) are handled correctly — the AI Gateway only splits on commas that separate distinct URLs.
</Note>
...
For opaque tokens that require real-time validation. AI Gateway calls your IdP's introspection endpoint.
...
By default, the AI Gateway reads the token from the `Authorization` header. For a different header:
...
If a token is missing `sub`, `email`, or `groups`, the AI Gateway returns:
...
- If signature verification fails with a cached key, the AI Gateway refetches the JWKS (handles key rotation)
...
The AI Gateway optimizes JWT validation for production workloads:
...
2. The AI Gateway validates token, extracts claims
3. The AI Gateway forwards claims to MCP server
...
| Topic | Description |
|-------|-------------|
| [External OAuth](/aigw/product/mcp-gateway/authentication/external-oauth) | Overview of using external IdPs |
