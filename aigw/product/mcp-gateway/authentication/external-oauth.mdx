...
External OAuth enables using your existing identity provider (Okta, Auth0, Azure AD, Descope, Cognito) instead of Prisma AIRS AI Gateway's built-in authentication. Users authenticate with corporate credentials—no Strata Cloud Manager account needed.

## When to Use

Organisations with existing IdPs can use External OAuth. Users authenticate through the IdP for other services, and MCP access works the same way.
...
- **No Strata Cloud Manager account.** Users who aren't in the AI Gateway need MCP access
...
Step 1 establishes your organisation context; the `jwt_validation` in step 4 is a per-server check layered on top of it. The AI Gateway never handles user credentials—your IdP remains the source of truth for identity.

---

## Configuration

Configure JWT validation for your MCP server. The AI Gateway validates incoming tokens using your IdP's public keys or introspection endpoint.

### Option 1: JWKS URI (Recommended)

The most common setup. The AI Gateway fetches public keys from your IdP's JWKS endpoint.
...
**How it works:**
- The AI Gateway fetches and caches your IdP's public keys
- Keys are cached for 24 hours by default
- If a key rotates, the AI Gateway automatically refetches
...
**How it works:**
- The request authenticates to the gateway first (AI Gateway API key or org-claim JWT); introspection runs **after** that step.
- The AI Gateway reads the end-user's opaque token from the header named by `headerKey` (default: `Authorization`). Use a dedicated header such as `X-Auth-Token` so it doesn't collide with the gateway credential.
- AI Gateway POSTs to your `introspectEndpoint` per [RFC 7662](https://datatracker.ietf.org/doc/html/rfc7662): the token in the request body as `token=…`, and—when `introspectClientId`/`introspectClientSecret` are set—an `Authorization: Basic base64(client_id:client_secret)` header to identify the AI Gateway to your endpoint.
...
<Note>
Token introspection validates the **end-user's** token; it does **not** authenticate the gateway. You still need an AI Gateway API key or an org-claim JWT on the request (see the callout at the top of this page). An opaque token on its own is not sufficient.
</Note>

See [JWT Validation](/aigw/product/mcp-gateway/authentication/jwt) for full configuration options.

---

## Client Configuration

Every request needs a **gateway credential** (an AI Gateway API key or an org-claim JWT) so the AI Gateway can resolve your organisation. When you use token introspection, the **end-user token** goes in a separate header—the one you set as `headerKey`:
...
If your IdP issues **JWTs that already carry AI Gateway org claims** (`portkey_oid` / `organisation_id`, `portkey_workspace` / `workspace_slug`), you can pass that JWT in `x-portkey-api-key` and it serves as the gateway credential on its own—see [Organisation JWT Authentication](/aigw/product/enterprise-offering/org-management/jwt). In that case no separate AI Gateway API key is required.
...
headers = {
    "Authorization": "Bearer YOUR_PORTKEY_API_KEY",  # gateway credential
    "X-Auth-Token": token,                         # end-user token (introspected)
}

async with streamablehttp_client(
    "https://aigw.portkey.ai/m/linear/mcp",
    headers=headers
) as (read, write, _):
    # MCP operations...
```
...
External OAuth pairs naturally with identity forwarding. The AI Gateway validates the incoming JWT, extracts user claims, and forwards them to MCP servers.
...
The AI Gateway couldn't fetch keys from your JWKS URI. Verify the URL is accessible and returns valid JWKS JSON.
...
| Topic | Description |
|-------|-------------|
| [JWT Validation](/aigw/product/mcp-gateway/authentication/jwt) | Full configuration reference for JWT validation |
| [Identity Forwarding](/aigw/product/mcp-gateway/authentication/identity-forwarding) | Pass validated claims to MCP servers |
| [Authentication Overview](/aigw/product/mcp-gateway/authentication) | Understanding gateway authentication |
