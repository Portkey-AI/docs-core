...
Without identity forwarding, the MCP server sees requests coming from Prisma AIRS AI Gateway, not from individual users. Identity forwarding bridges that gap.

Common scenarios:

- **Audit logging.** MCP server logs user identity for compliance
- **Authorization.** MCP server enforces per-user permissions
- **Multi-tenancy.** MCP server scopes data by user or organisation
...
The MCP server doesn't need to validate tokens or implement OAuth. It receives trusted user identity from the AI Gateway.
...
**Best for:** Internal MCP servers in trusted networks that trust the AI Gateway's claims without cryptographic verification.
...
The AI Gateway generates a new JWT containing user claims, signed with the AI Gateway's private key.
...
**Best for:** Cryptographic proof that AI Gateway issued the claims. MCP servers verify the signature without trusting the network.
...
If you don't specify `include_claims`, the AI Gateway includes:
...
For `jwt_header`, MCP servers verify tokens using the AI Gateway's public keys.

### Fetch Public Keys

```
GET https://aigw.portkey.ai/m/.well-known/jwks.json
```
...
jwks_client = PyJWKClient("https://aigw.portkey.ai/m/.well-known/jwks.json")
...
const JWKS = jose.createRemoteJWKSet(
  new URL('https://aigw.portkey.ai/m/.well-known/jwks.json')
);
...
The AI Gateway adds identity headers (`X-User-Claims`, `X-User-JWT`) to the protected headers list. This means:
...
This ensures MCP servers can trust identity information from the AI Gateway.

### JWT Security Properties

Signed JWTs provide:
- **Integrity**: Claims cannot be modified without invalidating the signature
- **Authenticity**: Only the AI Gateway can sign with its private key
...
Signing JWTs involves expensive cryptographic operations. The AI Gateway caches signed JWTs to avoid this overhead:
...
Cache the AI Gateway's JWKS on the MCP server. Most JWT libraries handle this automatically with configurable TTL.

---

## Self-Hosted Setup

Self-hosted AI Gateway deployments using `jwt_header` require a signing key.
...
The AI Gateway automatically exposes the public key at `/.well-known/jwks.json` for MCP servers to verify.
...
2. User sends request to the AI Gateway with IdP token
3. The AI Gateway validates token against your IdP
4. The AI Gateway extracts claims and forwards to MCP server
...
| Topic | Description |
|-------|-------------|
| [External OAuth](/aigw/product/mcp-gateway/authentication/external-oauth) | Use your own IdP for gateway authentication |
| [JWT Validation](/aigw/product/mcp-gateway/authentication/jwt) | Validate tokens from external IdPs |
| [Forwarding Headers](/aigw/product/mcp-gateway/authentication/forwarding-headers) | Pass request headers to MCP servers |
