---
title: OAuth
description: The Prisma AIRS AI Gateway's built-in OAuth 2.1 authentication for MCP Gateway.
---

The AI Gateway provides built-in OAuth 2.1 authentication for MCP Gateway. When users access MCP servers without an API key, the AI Gateway acts as the OAuth provider and handles the authentication flow.

## When to use

Use the AI Gateway's OAuth when:
...
The AI Gateway's OAuth is enabled by default. No configuration required.

When a request arrives without an API key or bearer token, the AI Gateway automatically initiates the OAuth flow.
...
```
https://aigw.portkey.ai/m/oauth/authorize?
  response_type=code&
...
```bash
curl -X POST https://aigw.portkey.ai/m/oauth/token \
...
The AI Gateway handles token lifecycle:
...
The AI Gateway's OAuth works alongside external OAuth. The AI Gateway determines the authentication method based on what's in the request:

| Request contains | Authentication method |
|------------------|----------------------|
| AI Gateway API key | API key authentication |
| External IdP token | External OAuth 2.0 validation |
| Nothing | AI Gateway OAuth 2.1 flow |
...
| [API Keys](/aigw/product/mcp-gateway/authentication#api-key) | Simpler authentication for programmatic access |
