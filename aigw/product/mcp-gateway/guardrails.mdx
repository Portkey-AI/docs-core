...
| Target | Description |
|--------|-------------|
| `llm` | Applied to LLM API requests (default, existing behaviour) |
...
```bash cURL
curl -X POST "https://aigw.portkey.ai/v1/guardrails" \
...
```bash cURL
curl -X PUT "https://aigw.portkey.ai/v1/guardrails/{guardrailId}/mcp-servers" \
  -H "Authorization: Bearer $PORTKEY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "mcp_servers": {
...
```bash cURL
curl -X PUT "https://aigw.portkey.ai/v1/guardrails/{guardrailId}/mcp-servers/{mcpServerId}" \
  -H "Authorization: Bearer $PORTKEY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "run_on": ["input", "output"],
...
```bash cURL
curl "https://aigw.portkey.ai/v1/guardrails/{guardrailId}/mcp-servers" \
  -H "Authorization: Bearer $PORTKEY_API_KEY"
```
...
<CardGroup cols={2}>
  <Card title="Guardrail Checks" icon="shield-check" href="/aigw/product/guardrails/list-of-guardrail-checks">
