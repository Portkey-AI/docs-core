...
- **Customize OAuth behaviour** when the upstream server's defaults don't work
...
| [`external_auth_config`](#external-auth-config) | Use an external OAuth provider instead of Prisma AIRS AI Gateway's | Enterprise SSO with your own IdP |
...
<Card title="Forwarding Headers" icon="arrow-right" href="/aigw/product/mcp-gateway/authentication/forwarding-headers">
...
Override the OAuth endpoints and parameters that AI Gateway discovers from the upstream server's `/.well-known/oauth-authorization-server`. Only relevant when Auth Type is **OAuth 2.1**.
...
<Card title="OAuth Client Metadata" icon="key" href="/aigw/product/mcp-gateway/authentication/oauth-client-metadata">
...
After authenticating a user, the AI Gateway can forward their identity to the upstream MCP server. The server gets to know *who* is making the request — without implementing its own auth.
...
| `jwt_header` | gateway-signed JWT with claims in `X-User-JWT` header |
...
<Card title="Identity Forwarding" icon="id-card" href="/aigw/product/mcp-gateway/authentication/identity-forwarding">
...
<Card title="JWT Validation" icon="shield-check" href="/aigw/product/mcp-gateway/authentication/jwt">
...
<Note>Available for self-hosted deployments.</Note>
...
<Card title="Bring Your Own Auth" icon="building" href="/aigw/product/mcp-gateway/authentication/external-oauth">
