...
Authorization determines what authenticated users can access. While [authentication](/aigw/product/mcp-gateway/authentication) verifies *who* a user is, authorization controls *what* they can do.
...
Control access to MCP servers based on JWT claims. This is configured through [JWT Validation](/aigw/product/mcp-gateway/authentication/jwt) using `requiredClaims` and `claimValues`.
...
See [JWT Validation](/aigw/product/mcp-gateway/authentication/jwt) for complete configuration options.
...
Authorization works alongside [Team Provisioning](/aigw/product/mcp-gateway/access-control):
...
<CardGroup cols={2}>
  <Card title="JWT Validation" icon="key" href="/aigw/product/mcp-gateway/authentication/jwt">
