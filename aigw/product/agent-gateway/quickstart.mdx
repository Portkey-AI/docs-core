...
<Info>
  Modify the API key permissions and turn on the toggle for **Agent Gateway** to enable Agent Gateway for your API key. Create or edit a key on the [API Keys](https://stratacloudmanager.paloaltonetworks.com/) page, and enable the `agents.invoke` scope when you will invoke agents with that key. For how Prisma AIRS AI Gateway API keys and scopes work, see [API keys, authentication, and authorization](/aigw/product/enterprise-offering/org-management/api-keys-authn-and-authz).
</Info>
...
<Steps>
  ...
  <Step title="Add agent to registry">
    <Steps>
      <Step title="Configure the agent">
        Access the [Agent
        Registry](https://stratacloudmanager.paloaltonetworks.com/) and
  ...
  <Step title="Verify your integration">
    <Steps>
      <Step title="Grab your AI Gateway API key">
        Head over to the [API Keys](https://stratacloudmanager.paloaltonetworks.com/) page and
      ...
      <Step title="Fetch the agent server URL from the agents tab">
        Open the [Agents](https://stratacloudmanager.paloaltonetworks.com/) tab and fetch the
        corresponding agent server URL. This is the URL you will use to connect
        your agent to the AI Gateway Agent Gateway instead of the agent server URL.
        It looks something like this (you can replace the agent slug with your
        server's slug):
        `https://aigw.portkey.ai/agent/header-based-authentication`
      </Step>
    </Steps>
  </Step>
</Steps>
...
<CodeGroup>
```shell cURL
curl --location --request POST 'https://aigw.portkey.ai/agent/{agent-slug}/.well-known/agent.json' \
...
BASE_URL = "https://aigw.portkey.ai/agent/{agent-slug}"
...
const BASE_URL = "https://aigw.portkey.ai/agent/{agent-slug}";
...
<CardGroup cols={2}>
  ...
  <Card title="Agent Servers" icon="server" href="/aigw/product/agent-gateway/servers">
