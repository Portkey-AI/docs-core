...
<Info>
  Modify the API key permissions and turn on the toggle for **Agent Gateway** to enable Agent Gateway for your API key. Create or edit a key on the [API Keys](https://app.portkey.ai/api-keys) page, and enable the `agents.invoke` scope when you will invoke agents with that key. For how Portkey API keys and scopes work, see [API keys, authentication, and authorization](/aigw/product/enterprise-offering/org-management/api-keys-authn-and-authz).
</Info>
