...
<Info>
  Modify the API key permissions and turn on the **Agent Gateway** toggle for your API key. See the [Quickstart](/aigw/product/agent-gateway/quickstart) for where to configure this in the app. Make sure the `agents.invoke` scope is enabled.
</Info>

## Introduction

The [Quickstart](/aigw/product/agent-gateway/quickstart) guide provides a basic example of how to use the Agent Gateway.
...
```shell cURL
curl --location 'https://aigw.portkey.ai/agent/{agent-slug}' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer {PORTKEY_API_KEY}' \
--data '{
    "jsonrpc": "2.0",
...
BASE_URL = "https://aigw.portkey.ai/agent/header-based-authentication"
...
```javascript JavaScript
const BASE_URL =
  process.env.BASE_URL || "https://aigw.portkey.ai/agent/{agent-slug}";
