...
Exclusions are managed per guardrail direction through the Admin API, so the call is authorised with a [Strata Cloud Manager access token](/aigw/api-reference/admin-api/authentication) whose service account can administer the organisation.
...
```bash
curl -X PUT "https://api.apps.paloaltonetworks.com/ai_gw/admin/v2/workspace-exclusions/input-guardrails" \
