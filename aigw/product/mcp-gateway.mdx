...
The MCP Gateway is Prisma AIRS AI Gateway's solution for managing access to MCP servers. It acts as a proxy between MCP Clients and MCP servers, handling authentication, access control, and logging.

When connecting to MCP servers directly, each agent needs its own credentials and configuration for every server. With the MCP Gateway, clients authenticate once to the AI Gateway. The Gateway handles credential injection, permission checks, and request logging for all configured servers.
...
<CardGroup cols={2}>
  ...
  <Card title="Integrations" icon="plug" href="/aigw/product/mcp-gateway/integrations">
...
<CardGroup cols={2}>
  ...
  <Card title="Access Control" icon="users" href="/aigw/product/mcp-gateway/access-control">
  ...
  <Card title="Rate Limits" icon="gauge" href="/aigw/product/mcp-gateway/rate-limits">
    Throttle MCP tool calls by request count or token consumption per API key, server, or tool.
  </Card>
  <Card title="Guardrails" icon="shield-check" href="/aigw/product/mcp-gateway/guardrails">
    Content filters and approval workflows per server.
  </Card>
  <Card title="Troubleshooting" icon="wrench" href="/aigw/help-center/mcp-gateway-troubleshooting">
...
Add an MCP server to your organisation's registry, configure its authentication, and provision access to workspaces. Then connect any MCP client to the AI Gateway's endpoint:

```json
{
  "mcpServers": {
    "portkey": {
      "url": "https://aigw.portkey.ai/m/{server-slug}/mcp",
      "headers": {
...
The API key determines which servers and tools are accessible. See [Integrations](/aigw/product/mcp-gateway/integrations) for setup guides for Claude Desktop, Cursor, VS Code, and custom agents.
...
<Card title="MCP Registry" icon="layer-group" href="/aigw/product/mcp-gateway/mcp-registry">
