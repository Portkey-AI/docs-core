...
1. **Portkey Gateway execution** — Prefix the MCP server with `@portkey-mcp` in the [Responses API](#responses-api) or [Messages API](#messages-api), and send the following header `x-portkey-beta: server-side-mcp-2026-06-01`. Portkey fetches and executes tools on your behalf. See [Beta Features](/aigw/product/ai-gateway/beta-features) for the full header reference.
...
If you have MCP servers registered on Portkey's [MCP Gateway](/aigw/product/mcp-gateway), you can connect to them through the Responses API by passing the appropriate authentication in the tool definition.
