...
Using AI Gateway, you can route your requests to different provider targets based on custom conditions you define. These can be conditions like:
...
Conditional routing is one of the _strategies_ in the AI Gateway's [Gateway Configs](/aigw/product/ai-gateway/configs). (others being `fallback` and `loadbalance`). To use it in your app,

1. You need to create a `conditional` config in Strata Cloud Manager.
...
2. When a condition evaluates to `false` or is malformed, the AI Gateway moves on to the next condition until it finds a successful one.
3. If no conditions pass, then the `default` target name is called
4. Since the AI Gateway iterates through the queries sequentially, the order of your conditions is important
5. If a referenced key is missing (in `metadata`, `params`, or `url`), the condition evaluates to `false`; it does not throw an error.
</Info>

## 2. Getting Config ID

Based on the `conditions` and the Config structure described above, you can create your [Config in Strata Cloud Manager](https://stratacloudmanager.paloaltonetworks.com/), and save it to get Config ID. The UI also helps you autofill and autoformat your Config.

## 3. Implementing Conditional Routing

Conditional routing is enabled through the AI Gateway's [Gateway Configs](/aigw/product/ai-gateway/configs) by following these steps:

1. Create a conditional config in Strata Cloud Manager ([stratacloudmanager.paloaltonetworks.com](https://stratacloudmanager.paloaltonetworks.com/))
...
<Note>
    The AI Gateway only supports params routing for primitive types (string, number, boolean). Complex types like arrays and objects are not supported.
</Note>
...
Palo Alto Networks support [metadata](/aigw/product/observability/metadata)-based routing, which requires sending custom metadata with your request.
...
<Info>
Guardrails are referenced by their IDs in the configuration. You can create guardrails in Strata Cloud Manager and then reference them in your conditional routing config.

Learn more about [AI Gateway Guardrails](/aigw/product/guardrails) to understand how to create and manage different types of guardrails for your LLM applications.
</Info>
</Accordion>
...
5. **Multiple Parameter Types**: The AI Gateway supports routing based on any parameter that can be sent in LLM requests including `model`, `temperature`, `top_p`, `frequency_penalty`, `presence_penalty`, `max_tokens`, and many others.
...
<Info>
[Reach out to us](https://support.portkey.ai/forms/customer-portal-ticket-form) to share your thoughts on this feature or to get help with setting up advanced routing scenarios.
</Info>
