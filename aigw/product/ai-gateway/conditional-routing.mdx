...
Conditional routing is one of the _strategies_ in Portkey's [Gateway Configs](/aigw/product/ai-gateway/configs). (others being `fallback` and `loadbalance`). To use it in your app,
...
Conditional routing is enabled through Portkey's [Gateway Configs](/aigw/product/ai-gateway/configs) by following these steps:
...
Portkey support [metadata](/aigw/product/observability/metadata)-based routing, which requires sending custom metadata with your request.
...
Learn more about [Portkey Guardrails](/aigw/product/guardrails) to understand how to create and manage different types of guardrails for your LLM applications.
</Info>
</Accordion>
