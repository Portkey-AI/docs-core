...
<Info>
Available on all Prisma AIRS AI Gateway [plans](https://portkey.ai/pricing).
</Info>
...
<Note>
When `use_retry_after_headers` is set to true and the provider includes Retry-After or Retry-After-ms headers in their response, the AI Gateway will use these values to determine the wait time before the next retry attempt, overriding the exponential backoff strategy.
If the provider doesn't include these headers in the response, the AI Gateway will fall back to the standard exponential backoff strategy.
