...
Add `cache` to your [config object](/aigw/api-reference/inference-api/config-object#cache-object-details):
...
<Info>
  Semantic cache is a superset of simple cache. The Prisma AIRS AI Gateway checks for an exact
...
<Steps>
  ...
  <Step title="Configure the vector database">
    ...
    **Pinecone**
    ...
    - In the Pinecone console, create or use an index with **cosine** as the similarity metric so it matches the AI Gateway's semantic cache behaviour.

  </Step>
  <Step title="Enable semantic caching per request">
    Set the cache mode to `semantic` in your [config object](/aigw/api-reference/inference-api/config-object#cache-object-details) for each LLM request:
...
1. Go to **Admin Settings** → **Organisation Properties** → **Cache Settings**
...
**Logs** → Status column shows: `Cache Hit`, `Cache Semantic Hit`, `Cache Miss`, `Cache Refreshed`, or `Cache Disabled`. [Learn more →](/aigw/product/observability/logs)
