...
<Info>
Available on all Prisma AIRS AI Gateway plans.
</Info>
...
<Info>
The `@provider-slug/model-name` format automatically routes to the correct provider. Set up providers in [Model Catalog](https://stratacloudmanager.paloaltonetworks.com/).
</Info>

[Create](/aigw/product/ai-gateway/configs#creating-configs) and [use](/aigw/product/ai-gateway/configs#using-configs) configs in your requests.

## How It Works

1. **Define targets & weights** — Assign a `weight` to each target. Weights represent relative share of traffic.
2. **Weight normalization** — the AI Gateway normalizes weights to sum to 100%. Example: weights 5, 3, 1 become 55%, 33%, 11%.
...
- Ensuring consistent model behaviour for A/B testing
...
|-----------|------|-------------|
| `enabled` | boolean | Turns sticky routing on or off. Must be `true` to activate sticky behaviour. If omitted or `false`, sticky routing is disabled and normal weighted load balancing is used. |
...
1. **Identifier Generation**: When a request arrives, the AI Gateway generates a hash from the specified `hash_fields` values
