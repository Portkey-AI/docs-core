---
title: "Configs"
description: This feature is available on all Prisma AIRS AI Gateway plans.
---
<Info>
...
Navigate to the ‘Configs’ page in Strata Cloud Manager and click 'Create' to start writing a new config.

## Using Configs

Configs are supported across all integrations.

* Through the config parameter of the AI Gateway SDK client(Directly or via [frameworks](/aigw/integrations/llms))
* Through the config headers in the OpenAI SDK
* Via the REST API through the `x-portkey-config` header

### Applying Gateway Configs

Gateway [configs](/aigw/product/ai-gateway/configs) allow you to unlock the gateway superpowers of the AI Gateway. You can create a config in the UI and attach it's config id in the OpenAI client.
...
const openai = new OpenAI({
  ...
  defaultHeaders: {
      ...
      "x-portkey-config": "CONFIG_ID" // Fetched from the UI,
  }
});
```
  </Tab>
...
If you want to attach the configuration to only a few requests instead of modifying the client, you can send it in the request headers for OpenAI or in the config parameter while using the AI Gateway SDK.
...
```js
let reqHeaders = { "x-portkey-config": "CONFIG_ID" };
...
```py
reqHeaders = {"x-portkey-config": "CONFIG_ID"}
...
- **Enforcing organisation-wide governance policies** without requiring code changes
...
1. Navigate to [API Keys](https://stratacloudmanager.paloaltonetworks.com/) in Strata Cloud Manager
2. Create a new API key or edit an existing one
3. In the API key settings, select a config from the **Default Config** dropdown
4. Save the API key

See the [Default Configs documentation](/aigw/product/administration/enforce-default-config) for detailed instructions and use cases.
...
The AI Gateway shows your Config usage smartly on the logs page with the **Status column** and gives you a snapshot of the Gateway activity for every request. [Read more about the status column here](/aigw/product/observability/logs#request-status-guide).
...
<Card title="Config Schema" horizontal="true" href="/aigw/api-reference/inference-api/config-object">
</Card>
