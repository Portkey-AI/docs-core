...
flowchart TB
    subgraph Azure["Your Azure Environment"]
        ...
        RedisExt["Azure Managed Redis<br/>(Cache - recommended)"]
    ...
    LB <-->|"HTTPS<br/>(Config Sync & Metrics)"| CP
```
...
flowchart TB
    subgraph Azure["Your Azure Environment (Fully Airgapped)"]
        subgraph AKS["AKS Cluster"]
            ...
            RedisInt["Redis<br/>(in-cluster or managed)"]
            MySQLInt["MySQL<br/>(in-cluster or managed)"]
        ...
        LB["Azure Load Balancer"]
        Blob["Azure Blob Storage<br/>(Log Store)"]
        RedisExt["Azure Managed Redis<br/>(Recommended)"]
        MySQLExt["Azure Database for MySQL<br/>(Recommended)"]
