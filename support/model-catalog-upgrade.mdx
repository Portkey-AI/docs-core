...
We are thrilled to introduce the **Model Catalog**, a powerful new feature designed to give your organization unprecedented control, governance, and visibility over your AI models. The Model Catalog is the evolution of our Virtual Keys feature, built to handle the complexities of enterprise-scale AI operations.
...
The Model Catalog fundamentally upgrades the Virtual Key experience by introducing a centralized, organization-level management layer. It addresses key challenges our customers have faced, allowing for:

1.  **Centralized Provider Management:** Create a single provider integration (e.g., for Azure, OpenAI, Bedrock) at the organization level, and securely provision it to multiple workspaces without re-entering credentials.
...
3.  **Granular Model Provisioning:** For any given provider integration, you can specify exactly which models (e.g., `gpt-4o`, `claude-3-sonnet`) are available, preventing users from accessing unapproved, expensive, or deprecated models.
...
| **Budgeting** | Budgets are set per virtual key. Managing shared provider costs across teams is difficult. | Set a master budget/rate limit at the Org integration level, then assign specific budgets/limits **per workspace**. |
| **Model Access** | A virtual key grants access to all models available under that provider key. Control requires complex configs or guardrails. | Define an explicit allow-list of models for each Org **Integration**. Workspaces only see what you've enabled. |
...
You also get a "Workspace Created" tab to view and manage any integrations created by Workspace Admins themselves, giving you full visibility.
...
Of course! This is a crucial addition. Explaining how existing workflows map to the new system is key to a smooth transition.

Here is the new section, formatted and expanded with code examples, ready to be added to **Documentation 1: The Upgrade Guide**.
...
<CardGroup cols={2}>
  ...
  <Card title="The New Way" icon="rocket-launch">
    You now create a central **Integration** at the Org level and provision it to many workspaces. You can also still create workspace-only providers, or "inherit" from an Org integration to create providers with stricter limits.
  </Card>
</CardGroup>
...
We've renamed "Virtual Keys" to **AI Providers** at the workspace level. To reference one in a request, you simply prefix its slug with an `@` symbol.
...
1.  **Contact Us:** Reach out to the Portkey team, and we will enable the Model Catalog for your organization.
