...
To enable Portkey cache, just add the `cache` params to your [config object](/docs/api-reference/config-object#cache-object-details).
...
### [Read more how to set cache in Configs](/docs/product/ai-gateway/cache-simple-and-semantic#how-cache-works-with-configs).
...
On the Logs page, the cache status is updated on the Status column. You will see `Cache Disabled` when you are not using the cache, and any of `Cache Miss`, `Cache Refreshed`, `Cache Hit`, `Cache Semantic Hit` based on the cache hit status. Read more [here](/docs/product/observability/logs).
