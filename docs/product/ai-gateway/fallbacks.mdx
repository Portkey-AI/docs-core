---
title: "Fallbacks"
description: "With an array of Language Model APIs available on the market, each with its own strengths and specialties, wouldn't it be great if you could seamlessly switch between them based on their performance or availability? Portkey's Fallback capability is designed to do exactly that."
---
...
To enable fallbacks, you can modify the [config object](/docs/api-reference/config-object) to include the `fallback` mode.
...
[Using Configs in your Requests](/docs/product/ai-gateway/configs#using-configs)
