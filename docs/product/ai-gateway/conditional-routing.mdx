...
I apologize for the error. It seems that the issue might be related to how the JSON code blocks are formatted within the tabs. Let's try to simplify the structure and see if it resolves the issue. Here's a revised version of that section:


## More Examples Using Conditional Routing

Here are some examples of how you can leverage conditional routing to handle real-world scenarios:
