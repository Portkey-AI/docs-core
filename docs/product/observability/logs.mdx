...
Each log entry provides useful data such as the timestamp, request type, LLM used, tokens generated, and cost. For [multimodal models](/docs/product/ai-gateway/multimodal-capabilities), Logs will also show the image sent with vision/image models, as well as the image generated.
...
Portkey’s gateway features—[Cache](/docs/product/ai-gateway/cache-simple-and-semantic), [Retries](/docs/product/ai-gateway/automatic-retries), [Fallback](/docs/product/ai-gateway/fallbacks), [Loadbalance](/docs/product/ai-gateway/load-balancing)—are all tracked here with their exact states (`disabled`, `triggered`, etc.), making it a breeze to monitor and optimize your usage.
...
As you're viewing logs, you can also add manual feedback on the logs to be analysed and filtered later. This data can be viewed on the [feedback analytics dashboards](/docs/product/observability/analytics#feedback).
