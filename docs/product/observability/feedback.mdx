...
To use your own trace IDs, send them as part of the request headers - [Adding a trace ID to your requests](/docs/product/observability/traces#how-to-enable-request-tracing)
...
You can see the `Feedback Count` and `Value: Weight` pairs for each `trace-id` on the logs page. You can also view the feedback details on [Analytics](/docs/product/observability/analytics#feedback) and on the Prompt Eval tabs.
