...
<Info>
  You'll notice that I'd already created [virtual keys](/docs/product/ai-gateway/virtual-keys) for OpenAI and Google in my account. You can create them by going to the **Virtual Keys** tab and adding your API keys to Portkey's vault - this also ensures that your original API keys remain secure.
</Info>
...
To run the experiment, lets create a [config](/docs/product/ai-gateway/configs) in Portkey that can automatically route requests between these 2 prompts.
...
Lets use this config to start making requests from our application. We will use the [prompt completions API](/docs/portkey-endpoints/prompts/prompt-completion) to make the requests and add the config in our headers.
