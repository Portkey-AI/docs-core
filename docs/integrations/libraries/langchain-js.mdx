...
<Info>
  ...
  This guide covers the integration for the **Javascript / Typescript** flavour of Langchain. Docs for the Python Langchain integration are [here](/docs/integrations/libraries/langchain-python).
</Info>
...
Portkey supports [Virtual Keys](/docs/product/ai-gateway/virtual-keys) which are an easy way to store and manage API keys in a secure vault. Lets try using a Virtual Key to make LLM calls.
...
You can find more config examples [here](/docs/api-reference/config-object#examples).
